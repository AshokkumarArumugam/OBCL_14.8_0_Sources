...
Copyright © 2013-2016 by Oracle Financial Services Software Limited..
**  Modified By          : Neethu Sreedharan
**  Modified On          : 22-Aug-2016
**  Modified Reason      : Changes done to calculate the page screen based on the dashboard space height 
                           instead of the menu browser window height
**  Retro Source         : 9NT1606_12_0_3_COMMONWEALTH_BANK_OF_AUSTRALIA
**  Search String        : 9NT1606_12_2_RETRO_12_0_3_23655941
----------------------------------------------------------------------------------------------------
...
**  Modified Reason      : Fix provided to refer to the relevant tag on the parent screen so as 
**						   to popultate Physical file name post upload of the file in GIDIFPRS function ID.                  
**  Search String        : FCUBS_14.1_CTCB_28988274

**  Modified By          : Ajeet Kumar
**  Modified On          : 02-Sep-2022
**  Modified Reason      : Fix provided to make OK and EXIT button color as per FCUBS's theme in file upload window.                  
**  Search String        : Bug_34408080
--------------------------------------------------------------------------------------------------------- -
*/%>
   ...
   <!--<link href="Theme/Ext<%=StringEscapeUtils.escapeURL(logintheme)%>" rel="stylesheet" type="text/css"/>--><!--HTML5 Changes-->
    ...
    <script type="text/javascript">
	  ...
	  function fnFileUpload(){
	    //FCUBS_14.1_CTCB_28988274 Starts
    	  parent.document.getElementById('BLK_GITM_INTERFACE_TRIGGER__PHY_FILE_NAME').value=fileName;
          parent.document.getElementById("BLK_GITM_INTERFACE_TRIGGER__FILE_NAME").readOnly = true;
          parent.document.getElementById("BLK_GITM_INTERFACE_TRIGGER__FILE_NAME").setAttribute("class","oj-read-only");
          parent.document.getElementById("BLK_GITM_INTERFACE_TRIGGER__PROCESS_CODE").setAttribute("class","oj-read-only");
          parent.document.getElementById("BLK_GITM_INTERFACE_TRIGGER__INTERFACE_CODE").readOnly = true;
          parent.document.getElementById("BLK_GITM_INTERFACE_TRIGGER__INTERFACE_CODE").setAttribute("class","oj-read-only");
		  //FCUBS_14.1_CTCB_28988274 ends
          fnExitUpload();
        }
			function fnExitUpload(){
				...
				if (parent.seqNo) {
					...
					if((imgUploadDIVHeight - document.getElementById("WNDtitlebar").offsetHeight) < parent.document.getElementById("ChildWin").offsetHeight){
						parent.parent.document.getElementById(containerDIV).style.height = imgUploadDIVHeight + "px";
						parent.parent.document.getElementById(containerDIV).children[0].style.height = imgUploadIFHeight + "px";
					}
				}            
...
//                                iFrameBody += '<link type="text/css" rel="stylesheet" href="https://static.oracle.com/cdn/fnd/gallery/2110.1.0/images/iconfont/ojuxIconFont.min.css"/>';
                ...
                iFrameBody += "<FORM id='fileUploadForm' method='post' action='GIFileUploadServlet?actionType=UPLOAD&uploadPath' enctype='multipart/form-data' onsubmit='createXML();'>";
	    ...
	    strFormData += "<UPLOADPATH></UPLOADPATH>";
            strFormData += "<FUNCTIONID>GIDIFPRS</FUNCTIONID>";
            strFormData +="<CURRENTBRANCH>000</CURRENTBRANCH>";
            strFormData += "<VALUE></VALUE>";
            strFormData +="<ENTITY>ENTITY_ID1</ENTITY>"; 
           // strFormData +="<CURRENTBRANCH><%=StringEscapeUtils.escapeJavaScript(currBranch)%></CURRENTBRANCH>";
			...
			iFrameFormDocument.getElementsByName("FILENAME")[0].value = fileName;
...
function createRequestIFrame(height,width) {
		...
		function responseHandler(e) {
            ...
            if (getNodeText(selectSingleNode(responseXml, "//ECODE")) == 'GI-PRS-023') {
					fileName= getNodeText(selectSingleNode(responseXml, "//FNAME"));
                    errorDesc = getNodeText(selectSingleNode(responseXml, "//EDESC"));
                    ecode = getNodeText(selectSingleNode(responseXml, "//ECODE"));
                    errorText = errorDesc;
                    mask();
					mainWin.showAlerts(fnBuildAlertXML(ecode, 'I', errorText), 'I');
                    unmask();
                }/*9NT1606_14_0_RETRO_12_0_2_27453370 ends*/ 
				if(getNodeText(selectSingleNode(responseXml, "//ECODE"))=='IN-HEAR-120'){
					errorDesc = getNodeText(selectSingleNode(responseXml, "//EDESC"));
					ecode     = getNodeText(selectSingleNode(responseXml, "//ECODE")); 
					mask();
					mainWin.showAlerts(fnBuildAlertXML(ecode,'E',errorDesc), 'E');
					unmask();
				} 
				if(getNodeText(selectSingleNode(responseXml, "//ECODE"))=='GI-PRS-027'){
					errorDesc = getNodeText(selectSingleNode(responseXml, "//EDESC"));
					ecode     = getNodeText(selectSingleNode(responseXml, "//ECODE")); 
					errorText = errorDesc;
					mask();
        ...
        <DIV  slot="body" id="WNDcontent">
                    <div id="ResTree" style="width:100%" >
                        <div class="oj-flex-bar">
                                <div class="oj-flex-bar-start ">
                                <div>
					<INPUT TYPE="File" class="oj-sm-padding-1x" LABEL_VALUE="Select" ID="" DBT="" DBC="" NAME="FILEUPLOAD" ID="FILEUPLOAD" DTYPE="" SIZE="" REQUIRED="0">
                                 </div>      
                ...
                <div id="DIV_AUDIT_BUTTONS" >
                 <div><div class="oj-flex-bar oj-sm-align-items-center oj-sm-padding-5x-horizontal">
                        <div class="footer-btn-container oj-flex-bar-end">
							...
							VALUE="Ok" DEFAULT="Ok" 
                                                        on-oj-action="[[function() {fnFileUpload()}.bind(null)]]"
