                ...
                <td>
                    <span class="SPNtext" tabindex='0'>
                        <xsl:choose>
                            ...
                            <xsl:when test ="StageStatus ='MTS'"><xsl:value-of select="$multi_auth"/></xsl:when>
	...
	<xsl:template name="dispXREF">
		...
		<xsl:choose>
                    ...
                    <xsl:when test ="('AUTREJ' = $StageStatus)">
                     ...
                     <xsl:when test ="('MTS' = $StageStatus)">
                        <A class="Astd" href="javascript:loadScreenForMultiAuth('{$xref}','{$txnBranch}','{$funcid}')"><xsl:value-of select="$xref"/></A><!--multi auth Changes -->
                    </xsl:when>
                   <xsl:when test ="('DIS' = $StageStatus)">
                        <A class="Astd" href="javascript:loadScreenForView('{$xref}','{$txnBranch}','{$funcid}')"><xsl:value-of select="$xref"/></A><!--multi auth Changes -->
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template name="dispXREF1"><!-- my change-->
		...
		<xsl:choose>
                    ...
                    <xsl:when test ="('AUTREJ' = $StageStatus)">
                     ...
                     <xsl:when test ="('MTS' = $StageStatus)">
                        <A class="Anorm" href="javascript:loadScreenForMultiAuth('{$xref}','{$txnBranch}','{$funcid}')"><xsl:value-of select="$xref"/></A>	<!-- FCUBS11.1 Authorizer assignment for approval Changes -->
                    </xsl:when>
                    <xsl:when test ="('DIS' = $StageStatus)">
                        <A class="Anorm" href="javascript:loadScreenForView('{$xref}','{$txnBranch}','{$funcid}')"><xsl:value-of select="$xref"/></A>	<!-- FCUBS11.1 Authorizer assignment for approval Changes -->
                    </xsl:when>
