...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <oj-navigation-list drill-mode="sliding" root-label="Menu">
        <a name="hrefMenu"></a>
		...
		<xsl:if test="$screen='SMBPLORIG'">
			<textarea style="display:none" id = "finalFunctionRightsOrig">
    	...
    	<xsl:template name="DisplayNodes">
			...
			<xsl:variable name="TreeMenu3Id" select="generate-id(current())"/> <!--18501338 Changes-->
			...
			<xsl:if test="$NodeType='root'">
	                <!--<div class="MenuLDiv">-->
                        <li>
					  <a  tabIndex ="0" href="#" id = "F{$NodeId}A"> 
					...
					<xsl:if test="$screen!='SMBPLORIG'">  <!-- 12.0.2 SOATEAM -->
                    <ul ><!-- 1203 oghag fix ID treemenu1 REMOVED-->
        ...
        <xsl:template name="DisplayLeafs">
    		...
    		<xsl:param name="txnType" select="." />
                ...
                <xsl:if test="$screen='SMBPLBRW'">
                 ...
                 <!-- 12.0.2-->           
				<a id ="{$winHref}|{$cusFunc}"   title="{$winHref} | {$cusFunc}" href="javascript:dispHref('{$winHref}','{$UIName}','{$finalRights}','{$txnType}');">
				<xsl:value-of select="@LABEL" />
                                </a>
                            </xsl:if>
                            <xsl:if test="($cusFunc='null') or ($cusFunc ='')">
                              <!-- 12.0.2-->           
				<a id ="{$winHref}" title="{$winHref}" href="javascript:dispHref('{$winHref}','{$UIName}','{$finalRights}','{$txnType}');">
				<xsl:value-of select="@LABEL" />
                                </a>
                            </xsl:if>
                  <!--</div>-->
