    ...
    <xsl:template name="sprtHandler">
        ...
        <xsl:if test="$footer != 'Y'">
            ...
            <xsl:if test="$SPRT_Index &lt;= $subpartCount">
                ...
                <oj-form-layout onclick="mainWin.fnUpdateScreenSaverInterval();" label-edge="start">
          ...
          <div id="{./BLOCK}_tableContainer">
                 <!--<oj-bind-if test="{{showTable}}">-->
                    <oj-table cellspacing="0"   ID="{./BLOCK}" caption="{./LBL}" type="ME"
                              summary="{./LBL}" role="presentation" display="grid" class="oj-sm-width-full oj-sm-margin-4x-vertical" style="height:250px">
                        <xsl:attribute name="selection-mode">
                                                    <xsl:text>{"row": "multiple"}</xsl:text>
                                            </xsl:attribute>
                        <!--<xsl:if test="count(./HEIGHT) > 0 and ./HEIGHT != ''">
                            <xsl:attribute name="style">
                                <xsl:text>height:</xsl:text><xsl:value-of select="./HEIGHT"/><xsl:text>px;max-height:</xsl:text><xsl:value-of select="./HEIGHT"/><xsl:text>px</xsl:text>
                            </xsl:attribute>
                        </xsl:if>-->
...
<xsl:template name="generateMEHeaderScript">
    ...
    <xsl:for-each select="$multipleEntryNode/FIELD">
          ...
          <xsl:if test="count(VALUE) > 0 and VALUE != ''">
             multipleEntryFieldList['<xsl:value-of select="$multipleEntryNode/BLOCK"></xsl:value-of>']['<xsl:value-of select="NAME"/>'] ='<xsl:value-of select="VALUE"/>';
          </xsl:if>
        ...
        <div>
            ...
            <xsl:if test="count(./FLDSET) !=0">
                <xsl:if test="position()!=last()">
                    <xsl:attribute name="class">partitionPanel oj-sm-padding-5x-horizontal partitionRightMargin oj-sm-padding-2x-bottom</xsl:attribute>
                </xsl:if>
                <xsl:if test="position()=last()">
                    <xsl:attribute name="class">partitionPanel oj-sm-padding-5x-horizontal oj-sm-padding-2x-bottom</xsl:attribute>
                </xsl:if>
    ...
    <xsl:template match="SECTION">
        ...
        <div>
            ...
            <xsl:choose>
             ...
             <oj-collapsible class="oj-sm-padding-5x-horizontal" id="BtnSectionNav{@ID}Expand" on-oj-expand="[[ojExpandHandler]]" on-oj-collapse="[[ojCollapseHandler]]">
             <h4 slot="header">
                <xsl:value-of select="LBL"/>
             </h4>
                <!--<xsl:attribute name="class">
...
<!--<xsl:text>amountConverter</xsl:text><xsl:value-of select="$amountNode/../BLOCK"/><xsl:text>__</xsl:text> <xsl:value-of select="$amountNode/NAME"/>= getFormattedAmount(this,  '<xsl:value-of select="$amountNode/RELATED_FIELD"/>',true);-->    
    ...
    <xsl:attribute name="NAME">
     ...
     <xsl:if test="($curr_fld/../@TYPE = 'ME' and $curr_fld/../@VIEW != 'SE') and   $curr_fld/TYPE!='SELECT'">
     ...
     <xsl:if test="($curr_fld/../@TYPE != 'ME' or $curr_fld/../@VIEW = 'SE' or $curr_fld/TYPE ='SELECT')">
    <xsl:attribute name="ID">
        <xsl:value-of select="concat($curr_fld/../BLOCK,'__',$curr_fld/NAME)"/>
    </xsl:attribute>
    </xsl:if>
...
<!--input text width changes end-->
   ...
   <oj-input-text slot="value" type="TEXT">
    ...
    <oj-input-date slot="value" title="{../LBL}"  placeholder="{$dateFormat}" day-formatter="[[dayFormatter]]">
            <!--HTML5 Changes -->
 ...
 <xsl:if test="count($selectNode/@CONTROL) = 0 or $selectNode/@CONTROL = 'N'">
  ...
  <!--  <label class="LBLstd" for=""></label>-->
   ...
   <OJ-INPUT-TEXT style="display:none;" onpropertychange="displayValue(this)">
    ...
    <xsl:if test="count(@MULTIFUNC) > 0 and @MULTIFUNC= 'Y'">
		<div class="DIVHeaderBodyContainer" id="DIVMainTmp">
			<xsl:attribute name="style">
				<xsl:text>width:</xsl:text>
				<xsl:value-of select="$screenWidth + 4"/>
				<xsl:text>px</xsl:text>
			</xsl:attribute>
			<xsl:if test="count(BODY/TAB) > 1">
                <xsl:call-template name="DisplayTemplateTabsCust"/>
                <xsl:call-template name="DisplayTemplateTabsContent"/>
      </xsl:if>
            
		</div>
    </xsl:if>
    <xsl:if test="count(@MULTIFUNC) = 0 and count(@SUBSYS) = 0">        
		<div class="DIVHeaderBodyContainer" id="DIVMainTmp">
        ...
        <xsl:if test="count(@SUBSYS) > 0 and @SUBSYS= 'Y'">
            <div class="DIVfooter" id="DIVFooter">
                <h4>
                    <xsl:value-of select="/FORM/SCREEN[@NAME = $screen]/@TITLE"/>
                </h4>
                <!--<fieldset>
                    <legend>
                        <xsl:value-of select="/FORM/SCREEN[@NAME = $screen]/TITLE"/>
    ...
    <xsl:template name="SubSysBtns_tmp">
        <div>
                ...
                <xsl:if test="count(//*[name() = 'SCREEN' and @NAME =  $screen]/*[name() ='SUBSCREEN']/FORM) > 0">
                <oj-conveyor-belt id="subSystemConveyorBelt" class="convyorBeltContainer oj-md-9 oj-sm-12 oj-flex-bar-start" arrow-visibility="visible" data-oj-binding-provider="none">
   ...
   <xsl:template name="DisplayTemplateTabsContent">
        <!--<div class="DIVThreeColSectionContainer">-->
            <div>
            <xsl:variable name="l_scr_type" select="/FORM/SCREEN[@NAME=$screen]/@TMP_SCR_TYPE"/>
            ...
            <div>
                <div id="tabContent">
                    <xsl:for-each select="BODY/TAB[@ID != 'All']">
                      <xsl:call-template name="TABContentHandler">
                        <xsl:with-param name="currentTab" select="."/>
       ...
       <xsl:template name="DisplayTemplateTabsCust">
        <!--<div class="DIVThreeColSectionContainer">-->
            <div class="oj-sm-width-full oj-flex-item">
            <!--<xsl:variable name="l_scr_type" select="/FORM/SCREEN[@NAME=$screen]/@TMP_SCR_TYPE"/>
            <xsl:if test="$l_scr_type != 'L'">
			...
			<xsl:attribute name="style">
				...
				<h2 class="hh4" style="padding-left:5px">
                    <xsl:value-of select="/FORM/SCREEN[@NAME = $screen]/@TITLE"/>
                </h2>-->
                <div id="SYS_TBL_TABS" class="oj-flex-bar oj-sm-align-items-center oj-sm-padding-5x-horizontal">
                    <!--<xsl:attribute name="style">
                      <xsl:text>width:</xsl:text>
                    ...
                    <oj-conveyor-belt class="convyorBeltContainer oj-sm-padding-5x-horizontal oj-flex-item oj-sm-12" arrow-visibility="visible" data-oj-binding-provider="none">
                        <oj-navigation-list drill-mode="none" selection="" edge="top">
                            <ul id="tablist">
                                ...
                                <!--<xsl:apply-templates select="BODY/TAB[@ID != 'All']" mode="template"/>-->
                                <xsl:for-each select="BODY/TAB[@ID != 'All']">
                                  <xsl:call-template name="TABHandler">
                                    <xsl:with-param name="currentTab" select="."/>
                                    <xsl:with-param name="index" select="position()"/>
                                  </xsl:call-template>
                                </xsl:for-each>
                    ...
                    <!--<xsl:call-template name="HorzFldSet">
                      <xsl:with-param name="." select="BODY/FIELDSET[count(@HREQ)>0 and @HREQ='Y']"/>
                    </xsl:call-template>-->
                    <div class="footer-btn-container oj-flex-bar-end">
                        <xsl:apply-templates select="BODY/FLDSET">
                            <xsl:with-param name="sprtReqd" select="'N'"/>
                            <xsl:with-param name="subpartCount" select="'0'"/>
                            <xsl:sort select="./@INDEX" data-type="number" order="ascending"/>
                        </xsl:apply-templates>
   ...
   <xsl:template name="TABHandler">
    <xsl:param name="currentTab" select="."/>
    <xsl:param name="index" select="position()"/>
    <xsl:variable name="tpage">
            <xsl:value-of select="@ID"/>
            </xsl:variable>
    <li>            
        <a href="#href{@ID}" onkeydown="return handleTabKeys(this,event)" objClicked="false">
            <xsl:variable name="sc">
            <xsl:text>'</xsl:text>
        </xsl:variable>
        <xsl:if test="position()=1">
         <xsl:attribute name="class">
            <xsl:text>Htabsel</xsl:text>
        </xsl:attribute>
        </xsl:if>
        <xsl:if test="position()!=1">
         <xsl:attribute name="class">
            <xsl:text>Htaball</xsl:text>
        </xsl:attribute>
        </xsl:if>
        <xsl:attribute name="id">
        <xsl:value-of select="@ID"/>
            ...
            <xsl:attribute name="onClick">
            <xsl:text>highlightSelTab('</xsl:text><xsl:value-of select="$tpage"/><xsl:text>')</xsl:text>
        </xsl:attribute>
            <span>
                <xsl:if test="count(LBL) > 0">
                <xsl:call-template name="dispLabelCaption_tmp">
   ...
   <xsl:template name="TABContentHandler">
    <xsl:param name="currentTab" select="."/>
     <xsl:param name="index" select="."/>
    <div>
    <xsl:attribute name="id">
      <xsl:value-of select="concat('tabCN_',@ID)"></xsl:value-of>
    </xsl:attribute>
    <div><!--Fix for 20918429 -->
     <xsl:attribute name="id">
	...
	<!-- Fix for 20805413 -->
      <iframe allowtransparency="true" frameborder="0" scrolling="no" title="" style="z-index:10">
		<xsl:attribute name="id">
			<xsl:text>ifr_LaunchWin</xsl:text>
			<xsl:value-of select="@ID"/>
		 </xsl:attribute>
       <xsl:if test="$index = 1">
        <xsl:attribute name="src">
		  <xsl:value-of select="concat('TempForward.jsp?action=SMSStartLogServlet&amp;funcid=',./FUNCID,'&amp;srcType=',./TYPE,'&amp;sectionName=',@ID,'&amp;tabId=',@ID,'&amp;uiName=&amp;msgType=WORKFLOW&amp;actionType=initiate&amp;timestamp=&amp;numeric=&amp;parentArgs=&amp;dashboardArgs=&amp;inTime=&amp;txnBranch=&amp;debugFlag=N&amp;userLevelDbgEnabled=N')"/>        
        </xsl:attribute>
        </xsl:if>       
