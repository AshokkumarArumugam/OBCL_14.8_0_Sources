   ...
   <xsl:variable name ="screenWidth">
        <xsl:choose>
            <xsl:when test="$l_scr_type = 'L'">
                <xsl:value-of select="number($largeScreenWidth) - 10"></xsl:value-of>
            </xsl:when>
            <xsl:otherwise>
...
<xsl:template name="sprtHandler">
        ...
        <div class="DIVpage" id="DIVVerisonBtns">
            ...
            <xsl:for-each select="./FIELD">
                <xsl:if test="./NAME = 'VERNO'">
                    <label class="LBLstd" for="{../BLOCK}__{./NAME}" style="width:{$labelWidth}px;"></label>
    ...
    <xsl:template match="FIELD/TYPE[text()='CHECKBOX']" mode="template">
        <xsl:if test="../../@TYPE != 'ME' or ../../@VIEW = 'SE'">
             <xsl:if test="../../@ID != 'FLD_AUDIT1'">
                <b class="LBLstd" id="groupidpymt" style="width:{$labelWidth}px;">
                    <xsl:text disable-output-escaping="yes">&#160;</xsl:text>
    ...
    <xsl:template match="FIELD/TYPE[text()='BUTTON']" mode="template">
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="../../@TYPE != 'ME' or ../../@VIEW = 'SE'">              
                    <div class="DIVText" > <label class="LBLstd" style="width:{$labelWidth}px;"></label>
                        <xsl:call-template name="dispButtonField_tmp"/>
                    </div>
                </xsl:if>
            </xsl:otherwise>
    ...
    <xsl:template match="FIELD/TYPE[text()='PASSWORD']" mode="template">
     ...
     <xsl:template match="FIELD/TYPE[text()='LINK_TYPE']" mode="template">
        <xsl:if test="../../@TYPE != 'ME' or ../../@VIEW = 'SE'">  
            <label class="LBLstd" style="width:{$labelWidth}px;"></label>
    ...
    <xsl:template name="dispNormalLabel_tmp" >
        <xsl:if test="count(../REQD) = 0 or ../REQD = '0'">        
            ...
            <xsl:if test="../LBL = ''">
                <label class="LBLstd LBLinv2" style="width:{$labelWidth}px;">
                    <xsl:attribute name="FOR">
                        <xsl:if test="../../BLOCK != ''">
                             <xsl:value-of select="concat(../../BLOCK,'__',../NAME)"></xsl:value-of>
                        </xsl:if>
        ...
        <xsl:if test="count(../REQD) &gt; 0 and ../REQD = '-1'">
            <label class="LBLstd star" style="width:{$labelWidth}px;">
                <xsl:attribute name="FOR">
                    <xsl:if test="../../BLOCK != ''">
                         <xsl:value-of select="concat(../../BLOCK,'__',../NAME)"></xsl:value-of>
                    </xsl:if>
    ...
    <xsl:template name="dispRadioLabel_tmp" >
        <xsl:if test="count(../REQD) = 0 or ../REQD = '0'">
        <b class="LBLstd" id="groupidpercentage" style="width:{$labelWidth}px;">
            <xsl:value-of select="../LBL"/>
        </b>
        </xsl:if>
        <xsl:if test="count(../REQD) &gt; 0 and ../REQD = '-1'">
...
<xsl:template name="dispMaskField_tmp">
    ...
    <label class="LBLstd" for="" style="width:{$labelWidth}px;"></label><INPUT TYPE="TEXT" class="TXTro" mask="{../MASK}" onblur="validateInputValue('{../NAME}');" readOnly="true" >
        <xsl:call-template name="ATTR_InputEntity_Handler_tmp">
            <xsl:with-param name="curr_fld" select=".." />
        </xsl:call-template>
