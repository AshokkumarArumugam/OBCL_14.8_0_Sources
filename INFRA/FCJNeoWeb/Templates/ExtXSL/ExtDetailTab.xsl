...
**  Change Description   : Text editor icon is not showing for Text fields when Popup Edit Required is Yes
**  Search String        : Redwood_35358400
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:param name="globalVariables"/><!--12.0.3 Defaulting global variables-->
...
<xsl:template name="sprtHandler">
        ...
        <xsl:if test="$footer = 'Y'">
            <xsl:if test="$SPRT_Index &lt;= $subpartCount">
                <!--<td>-->
                <div class="oj-sm-width-1/4 oj-flex-item " block="{./BLOCK}" type="{@TYPE}" view="{@VIEW}">
             <xsl:if test="LBL != ''">
                <xsl:attribute name = "class">
                    <xsl:value-of select="''"/>
                </xsl:attribute>
    ...
    <xsl:template name="MEHandler">
        ...
        <xsl:variable name="multipleHeight">
         ...
         <div name="dataContainer" id="dataContainer_{./BLOCK}">
          ...
          <div id="{./BLOCK}_tableContainer" class="oj-sm-padding-5x-horizantal">
                 <oj-bind-if test="{{showTable}}">
                    <oj-table display="grid" cellspacing="0" cellpadding="0" ID="{./BLOCK}" caption="{./LBL}" type="ME"
                              summary="{./LBL}" role="presentation" class="oj-sm-width-full oj-sm-margin-2x-vertical oj-density-compact" >
...
<xsl:template name="generateMEHeaderScript">
    ...
    <xsl:template name="MEHandler_OLD">
        ...
        <div name="dataContainer" id="dataContainer_{./BLOCK}">
			...
			<xsl:value-of select="((number($partWidth) div 100) * $screenWidth) - ((18 * (number($partWidth) div 100) )+ 10)"></xsl:value-of>
			<xsl:text>px</xsl:text>
			</xsl:attribute>-->
        	<!--Changes for header in ME screen-->
            <div class="DIVmultiplebox" onkeydown="return addRowShortcut('{./BLOCK}', event);">
                <!--Fix for 18532714 starts-->
                <xsl:if test="count(./LBL) &gt; 0 and ./LBL != ''"><!--OHAG issue-->
                ...
                <div name="MEContainer" class="MEContainer">
                  <!--9NT1606_14_0_RETRO_12_3_27342835 starts-->
                  <!--<div name="headerContainer"  style="overflow:hidden;" class="DIVMultipleSmallInnerH">-->
                  <div name="headerContainer"  style="overflow-x:hidden;overflow-y:scroll;" class="DIVMultipleSmallInnerH">
                  <!--9NT1606_14_0_RETRO_12_3_27342835 ends-->
	            ...
	            <div name="tableContainer" style="Height:{$multipleHeight}px;" onscroll="toggleSelectBoxes(this,'{./BLOCK}Header')">
	                <xsl:choose>
	                    <xsl:when test="$partWidth = '100' and $l_scr_type='L'"> 
	                        <xsl:attribute name="class">
					...
					<!--Change for header in ME screen-->
                                        <oj-bind-if test="{{showTable}}">
	               <oj-table display="grid" cellspacing="0" cellpadding="0" ID="{./BLOCK}"       caption="{./LBL}"   type="ME" summary="{./LBL}" role="presentation">
					<!--<xsl:attribute name="style">
						<xsl:text>width:</xsl:text>
						<xsl:value-of select="((number($partWidth) div 100) * $screenWidth) - ((18 * (number($partWidth) div 100) )+ 10)"/>
						<xsl:text>px;Height:</xsl:text>
    ...
    <xsl:template name="MultipleHandler_Nav_tmp" >
        ...
        <!--<div class="oj-flex-bar oj-sm-align-items-center">
            <span class="oj-flex-bar oj-sm-align-items-centerH">-->
        <!-- Changes for Header in ME screen start-->
       ...
       <xsl:param name="curr_blk" select="."/>
        ...
        <tbody>
          <tr>
            ...
            <td class="THgrid1" scope="col"><div>
                    ...
                    <td class="THgrid"  scope="col">                        
                            <xsl:if test="count(./DD) &gt; 0 and ./DD = -1">
                                    <xsl:if test="count(./REQD) = 0 or ./REQD != -1" >
                                        <div><span class="SPNtext"><xsl:value-of select="./LBL"/>
                                        </span></div><!--static header change -->
                                    </xsl:if>
                                    <xsl:if test="./REQD = -1">
                                        <div><span class="SPNtext star"><xsl:value-of select="./LBL"/>
                                        </span></div><!--static header change -->
                                    </xsl:if>                      
                            </xsl:if>
                            <xsl:if test="count(./DD) = 0">
                                    <xsl:if test="count(./REQD) = 0 or ./REQD != -1" >
                                        <div><span class="SPNtext"><xsl:value-of select="./LBL"/></span></div><!--static header change -->
                                    </xsl:if>
                                    <xsl:if test="./REQD = -1">
                                        <div><span class="SPNtext star"><xsl:value-of select="./LBL"/></span></div><!--static header change -->
                                    </xsl:if>                      
                            </xsl:if>
                    </td>
                </xsl:if>
            </xsl:for-each>
            <td class="THgrid" width="99%"><div></div></td>
        </tr>
        </tbody>
    ...
    <xsl:template name="MultipleHandler_tmp">
        ...
        <tbody>
            <tr>
                <td class="TDgrid1" scope="row"><div><!--Static header change-->
                ...
                <td class="TDgrid" width="99%"><div><xsl:text disable-output-escaping="yes">&#160;</xsl:text></div></td><!--Static header change-->
            </tr>
        </tbody>
        <tfoot>
<!-- Fix for bug no 19618264  -->
            <tr><td colspan="{$spanCnt}" ><a class="vHidden" href="#"><xsl:value-of select="$end_table"/></a><!--HTML5 changes 2/NOV/2016 fix for 24940888-->
...
<!-- Fix for bug no 19618264  -->
    ...
    <xsl:template name="VCHandler">
        <div class="oj-flex-bar oj-sm-align-items-center" id="DIVVerisonBtns">
            <div class="oj-flex-bar-end">
            ...
            <xsl:text disable-output-escaping="yes">&#160;</xsl:text>
            <oj-label slot="label" class="LBLstd" for="" ></oj-label>
    ...
    <xsl:template match="FIELD/TYPE[text()='CHECKBOX']" mode="template">
        ...
        <xsl:if test="(../../@TYPE = 'ME' and ../../@VIEW != 'SE') or (../../@TYPE = 'ME' and count(../../@VIEW) = 0)">
            <td class="TDgrid" nowrap="nowrap"><div><!--Static header change-->
                <xsl:call-template name="dispCheckboxField_tmp"/>
            </div></td><!--Static header change-->
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="FIELD/TYPE[text()='ROSELECT']" mode="template">
        <xsl:if test="(../../@TYPE = 'ME' and ../../@VIEW != 'SE') or (../../@TYPE = 'ME' and count(../../@VIEW) = 0)">                     
        <td class="TDgrid"><div><!--Static header change-->
    ...
    <oj-label-value label-edge="start" label-width="{$labelWidth}px">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="(../../@TYPE = 'ME' and ../../@VIEW != 'SE') or (../../@TYPE = 'ME' and count(../../@VIEW) = 0)"> 
                    <td class="TDgrid" nowrap="nowrap" onclick="mainWin.fnUpdateScreenSaverInterval();"><div><!--Static header change-->
    ...
    <xsl:template match="FIELD/TYPE[text()='PASSWORD']" mode="template">
     ...
     <xsl:template match="FIELD/TYPE[text()='LINK_TYPE']" mode="template">
        ...
        <xsl:if test="(../../@TYPE = 'ME' and ../../@VIEW != 'SE') or (../../@TYPE = 'ME' and count(../../@VIEW) = 0)"> 
            <td class="TDgrid" nowrap="nowrap"><div></div><!--Static header change-->
            </td>
        </xsl:if>    
    </xsl:template>
...
<!--UDF Changes Start -->
  ...
  <xsl:otherwise>
            ...
            <!--<div class="oj-flex">-->
                <fieldset block="{./BLOCK}" type="{./@TYPE}" view="{./@VIEW}">
                    ...
                    <xsl:if test="(LBL != '') and (@TYPE = 'SE' or @VIEW = 'SE')">
                        ...
                        <h4 class="oj-sm-padding-5x-start">
                            <xsl:value-of select="LBL"/>
                        </h4>
                    </xsl:if>
                    <xsl:if test="$sprtReqd = 'Y'">
    ...
    <xsl:template match="FLDSET" mode="footer">
        ...
        <xsl:if test="count(../SPRTCNT) > 0 and ../SPRTCNT != '0'">
            <oj-form-layout label-edge="start"  user-assistance-density="compact">
                <div class="oj-flex oj-sm-width-full oj-panel oj-panel-shadow-xl oj-sm-margin-1x">
            <xsl:call-template name="sprtHandler">
                <xsl:with-param name="footer" select="'Y'"/>
                <xsl:with-param name="SPRT_Index" select="1"/>
    ...
    <xsl:template name="HorzFldSet">
        ...
        <fieldset class="oj-flex-item oj-sm-padding-2x-top" block="{./BLOCK}" type="{@TYPE}" view="{@VIEW}">
         <oj-form-layout onclick="mainWin.fnUpdateScreenSaverInterval();" label-edge="start"  user-assistance-density="compact">
			<!--change for holiday maint screen fields alignment-->
            <!--<xsl:attribute name="class">
            ...
            <div class="oj-sm-width-full" onclick="mainWin.fnUpdateScreenSaverInterval();">
                <!-- <table border="0" cellpadding="0" cellspacing="0" summary="" style="display:inline-block;" class="hfieldSetTable">
                    <tbody role="group" aria-labelledby="{./BLOCK}"> -->
                       <!--fcis CHANGE for horizontal fields-->
...
<xsl:template match="EVENT" mode="template">
    ...
    <xsl:if test="(../TYPE = 'SELECT' or ../TYPE = 'ROSELECT')  and ($eventname = 'onblur' or $eventname = 'onBlur')">
                <xsl:attribute name="onchange">
                <xsl:value-of select="./FUNCTION"/>
            </xsl:attribute>
    </xsl:if>
...
<xsl:template name="calcTextMaxWidth">
  <xsl:param name="curr_node"/>
   <xsl:if test="(../../@TYPE != 'ME' or ../../@VIEW = 'SE') and ../../HREQ != '-1'">    
    <!--<xsl:attribute name="style">
     <xsl:text>max-width:</xsl:text>
      <xsl:variable name="partWidth">
        <xsl:choose>
        <xsl:when test="count(../../../SPRTCNT) = 1">
          <xsl:value-of select="(number(../../../@WIDTH) div number(../../../SPRTCNT)) - 1"/>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:variable name="lovWidth">
        <xsl:choose>
        <xsl:when test="count(../LOV) > 0 and count(../INPUT_LOV) = 0">
      ...
      <xsl:variable name="popUpWidth">
        <xsl:choose>
        <xsl:when test="count(../POPUPEDIT) > 0 or (number(../MAXLENGTH) > $displaySize)">
          <xsl:value-of select="number(26)"/>--><!--HTML5 Changes 6/OCT/2016-->
        <!--</xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="number(0)"/>
      ...
      <xsl:variable name="narrativeWidth">
        <xsl:choose>
        <xsl:when test="count(../@HOTKEY) > 0 and ../@HOTKEY = 'Y'">
          <xsl:value-of select="number(26)"/>--><!--HTML5 Changes 6/OCT/2016-->
        <!--</xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="number(0)"/>
    ...
    <xsl:value-of select="(((number($partWidth) div 100) * $screenWidth) - ((18 * (number($partWidth) div 100) )+ 8) - ($labelWidth  + 15)- $lovWidth - $popUpWidth - $narrativeWidth)"></xsl:value-of>
    <xsl:text>px</xsl:text>
  </xsl:attribute>-->
  </xsl:if>
</xsl:template>
...
<!--REDWOOD_35670751 ends-->
    ...
    <!--</oj-input-text>--> <!--Redwood_35358400 below condition moved to inside oj-input-text tag-->
    <xsl:if test="count(../POPUPEDIT) &gt; 0 or (number(../MAXLENGTH) &gt; $displaySize)">
      <xsl:call-template name="Popup_Handler_tmp">
            <xsl:with-param name="curr_fld" select=".."/>
        </xsl:call-template>
        <!--<BUTTON class="BTNhide" oldClassName="BTNimg" disabled="true" type="button">
                --><!-- 1203 JAWS change --><!--
                <xsl:if test="count(../READ_ONLY) = 0 or ../READ_ONLY = 0">
                <xsl:attribute name="tabindex">0</xsl:attribute>--><!-- 1203 JAWS change --><!--
            </xsl:if>
	...
	</oj-input-text><!--Redwood_35358400-->
        <!--</xsl:if>-->
        <!--HOTKEY-->
    <xsl:if test="count(../@HOTKEY) &gt; 0 and ../@HOTKEY = 'Y'">
        <xsl:attribute name="onkeydown">
...
<xsl:template name="LovHandler_tmp">
    ...
    <!--1203 JAWS change -->
        <oj-button  slot="end"  display="icons" chroming="borderless" oldClassName="BTNimg" title="{$lov}"  disabled='true' tabindex="0" type="button">
           <!-- <xsl:if test="count($curr_fld/INPUT_LOV) &gt; 0">
...
<xsl:template name="dispAutoLov">
	...
	<!--Fix for 18678458 -Exact Fetch changes-->
    <xsl:text>','</xsl:text>
    <xsl:call-template name="replaceApos">
     ...
     <!--   <xsl:if test="../LBL != ''"> -->
        ...
        <!--<xsl:if test="../LBL = ''">
            <label>
                    ...
                    <xsl:choose>
                            <xsl:when test="../../HREQ = '-1'">
                            <xsl:value-of select="'LBLChkRadSel LBLinvHF'"/> 
                        </xsl:when>
                            <xsl:otherwise>
                            <xsl:value-of select="'LBLChkRadSel LBLinv2'"/ 
                        </xsl:otherwise>
                        </xsl:choose>
                </xsl:attribute>
                    <xsl:attribute name="for">
...
<!--REDWOOD_35670751 ends-->
   ...
   <!-- <xsl:if test="count(../LOV) = 0">      
        <xsl:if test="count(../READ_ONLY) = 0 or ../READ_ONLY != -1">            
            <button class="BTNhide" oldClassName="BTNimg" title="{$calendar}" onclick="disp_cal('{../NAME}', event)" disabled="true" tabindex="0" type="button">
                   --> <!-- 1203 JAWS change --><!--
...
<!--REDWOOD_35670751 ends-->
    ...
    <!-- 1203 JAWS change --><!--
         ...
         <BUTTON class="BTNhide" oldClassName="BTNimg" title="{$narrative}" disabled="true" tabindex="0" type="button">
            <xsl:call-template name="Popup_Handler_tmp" />
...
<xsl:template name="dispLinkType_tmp">
    ...
    <a  onclick="fnLaunchLinkWindow(this,'{$paramList}');">
    ...
    <xsl:if test="../LBL != ''">
        <label class="LBLChkRadSel NewChkbox"><!--HTML5 Changes-->
                ...
                <input type="checkbox" disabled="true"><!--HTML5 Changes-->
                    ...
                    <xsl:if test="count(../CHECKED) > 0 and ../CHECKED = -1"><!--HTML5 Changes-->
                    <xsl:attribute name="CHECKED">CHECKED</xsl:attribute>
                    <xsl:attribute name="DEFAULT">yes</xsl:attribute>
                </xsl:if>
                </input>
   ...
   <xsl:template name="DisplayTemplateTabs">
        <!--<div class="DIVThreeColSectionContainer">-->
            ...
            <div class="DIVColumnTripple">
                ...
                <div id="SYS_TBL_TABS">
               <oj-conveyor-belt class="convyorBeltContainer oj-sm-padding-5x-horizontal oj-flex-item oj-sm-12 oj-sm-margin-2x-bottom" arrow-visibility="visible" data-oj-binding-provider="none">
...
<xsl:template match="BODY/TAB" mode="template">
    <li >
        ...
        <a href="#href{@ID}" class="oj-sm-margin-2x-start" tabindex = '0' onkeydown="return handleTabKeys(this,event)" objClicked="false">
    ...
    <xsl:template name="SubSysBtns_tmp">
        <div class="DIVAbutton" id="DIVSubSystem">
             <xsl:if test="count(//*[name() = 'SCREEN' and @NAME =  $screen]/*[name() ='SUBSCREEN']/FORM) > 0">
                <oj-conveyor-belt id="subSystemConveyorBelt" class="convyorBeltContainer oj-sm-padding-5x-horizontal oj-md-9 oj-sm-12 oj-flex-bar-start" arrow-visibility="visible" data-oj-binding-provider="none">
                <xsl:for-each select="//*[name() = 'SCREEN' and @NAME =  $screen]/*[name() ='SUBSCREEN']/FORM">
                    ...
                    <xsl:variable name="lbl" select="LBL"></xsl:variable>
                            <oj-button onkeydown="return fnhandleSubScrBtn(event)" class="conveyorBeltItem oj-sm-margin-1x" id="{@id}" >
