...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="MEHandler">
        ...
        <div name="dataContainer" id="dataContainer" class="oj-sm-padding-2x-vertical oj-sm-margin-2x-vertical oj-sm-padding-1x-end">
            <!--<xsl:choose>
            ...
            <div id = "multipleboxDiv" class="oj-sm-padding-4x-horizontal partitionPanel">
            
                 <div class="oj-flex-bar oj-sm-align-items-center" id="wndtitle">
                        <div class="oj-flex-bar-start">
                        <h4 id="heading"><xsl:value-of select= "$scrTitle"/></h4>
                        </div>
                        <div class="oj-flex-bar-end" id="btnDiv">
                            <oj-button display="icons" chroming="borderless" onclick="fnNavigate('PREVIOUS')" disabled="true" id="btnprev">
                    <span slot='startIcon' class="oj-ux-ico-caret-left"  title="{$previous}"></span>
                    </oj-button>
                     <oj-button display="icons" chroming="borderless" onclick="fnNavigate('NEXT')" disabled="true" id="btnnext">
                    <span slot='startIcon' class="oj-ux-ico-caret-right"  title="{$next}"></span>
                    </oj-button>
                          <oj-button display="icons" chroming="borderless" onclick="fnRefreshData()" disabled="true" id="btnrefreshd" accesskey="1">
                    <span slot='startIcon' class="oj-ux-ico-refresh" title="{$refresh}" ></span>
                     
                    </oj-button>  
                    <oj-button display="icons" chroming="borderless" onclick="fnLaunchMainSumScr()" disabled="true" id="btnmore" accesskey="1">
                    <span slot='startIcon' class="oj-ux-ico-overflow-h" title="{$more}" ></span>
                ...
                <!--<h2 class="hh4dash" id="heading"><xsl:value-of select= "$scrTitle"/></h2>-->
                    <!--<div id="btnDiv" style="float:right; padding-bottom: 3px">
                        --><!--Fix for 21609958 start--><!--
                        --><!--<BUTTON id="btnprev" class="Abut" disabled="disabled" onclick="fnNavigate('PREVIOUS')" title="{$previous}">
                            <SPAN class="WidgetonePrevious" title="{$previous}"></SPAN>
                        </BUTTON>
                        <BUTTON  id="btnnext" class="Abut" disabled="disabled" onclick="fnNavigate('NEXT')" title="{$next}">
                            <SPAN class="WidgetoneNext" title="{$next}"></SPAN>
                        </BUTTON>
                        <BUTTON class="Abut" id="btnrefreshd" onclick="fnRefreshData()" title="{$refresh}">
                            <SPAN class="WidgetoneRefresh" title="{$refresh}"></SPAN>--><!--Fix for 21609958 end--><!--
                        </BUTTON>--><!--   
                        ...
                        <A id="btnmore" class="Afoot" title="{$more}" onfocus="if(!this.disabled) this.className='Afoothover'" onmouseover="if(!this.disabled) this.className='Afoothover'" onmouseout="if(!this.disabled) this.className='Afoot'" href="#" onclick="fnLaunchMainSumScr()">more...</A>--><!--HTML5 changes 2/NOV/2016 Fix for 24904397--><!--
                    </div>-->
                    <div style="clear: both; BACKGROUND-COLOR: #f2f2f2;"><div id="tableHeaderContainer" style="overflow: hidden; clear: both">
                            <!--<xsl:choose>
                                <xsl:when test="$scrType='L'"> 
                                    <xsl:attribute name="class">
                            ...
                            <!--<table id="{$tablename}Header" style="width:100%" border="0" cellspacing="0" cellpadding="0" role="presentation" class="TBLgrid" type="ME" onkeydown="return addRowShortcut(this, event);">
                        ...
                        <!--<xsl:call-template name="MultipleHandlerHeader_tmp"/>-->
                    </div></div>
	            <div id="tableContainer" class="oj-sm-margin-2x-bottom">
	                <!--<xsl:choose>
	                    <xsl:when test="$scrType='L'"> 
	                        <xsl:attribute name="class">
	                            <xsl:value-of select="'DIVMultipleBigInner'" />
                        ...
                        <!--<table id="{$tablename}" style="width:100%" border="0" cellspacing="0" cellpadding="0" role="presentation" class="TBLgrid" type="ME" onkeydown="return addRowShortcut(this, event);">
                            <xsl:attribute name="DBT">
                                <xsl:value-of select="//SUMMARY_DATA_BLK"/>
                            </xsl:attribute>
                        <xsl:call-template name="MultipleHandler_tmp"/>
                    </table>-->
                    <oj-table ID="{$tablename}"  type="ME" display="grid" 
                            role="presentation" class="oj-sm-width-full">
    ...
    <xsl:template name="Sum_Custom_Btn">
            ...
            <div id="SUM_CUST_BTNS">
                <div class="oj-flex-bar oj-sm-align-items-center" id="CUST_BTNS">
                <xsl:if test="(count(//SUMBUTTONS) > 0)">
                    <xsl:for-each select="//SUMBUTTONS/BUTTON">
                        <div class="oj-sm-padding-5x-horizontal oj-flex-bar-start">
                            <oj-button class="oj-sm-margin-1x" name="{BUTTON_NAME}" id="{BUTTON_NAME}">
                    ...
                    <xsl:if test="$functionId = 'ITDREMNB'  or $functionId = 'ITSREMNB' ">
                        <div class="oj-sm-padding-5x-horizontal oj-flex-bar-start">
                            <oj-button class="oj-sm-margin-1x" id="divBtnDismiss" onclick="fnDismissReminders()">
    ...
    <xsl:template name="MultipleHandlerHeader_tmp">
    ...
    <script type="text/javascript" DEFER="DEFER">
     <xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>columnArray =landingKo.observableArray([]); 
     <xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>fieldObj= landingKo.observableArray([]); 
    ...
    <xsl:for-each select="$multipleEntryNode/FIELD">
        var obj = { 'headerText':  "<xsl:value-of select="LBL"/>", 'field': "<xsl:value-of select="NAME"/>" , minWidth: "10%",
                          maxWidth: "25%",'sortable':'disabled'};
             <xsl:if test="TYPE = 'HIDDEN'">
               obj = { 'headerText':  "<xsl:value-of select="LBL"/>", 'field': "<xsl:value-of select="NAME"/>" ,'headerStyle':'display:none','sortable':'disabled'};
           </xsl:if><!--
       ...
       obj['frozenEdge']='start';
          ...
          <xsl:if test="count(VALUE) > 0 and VALUE != ''">
             multipleEntryFieldList['<xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>']['<xsl:value-of select="NAME"/>'] ='<xsl:value-of select="VALUE"/>';
          </xsl:if>
    </xsl:for-each>
    multipleEntryFieldList['<xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>']['readOnly'] = true;
    meArrayForAddDelete['<xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>'] = landingKo.observableArray([]);
    
    <xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>dataprovider=landingKo.observable( new tempArrayDataProvider( meArrayForAddDelete['<xsl:value-of select="$multipleEntryNode/../SUMMARY_DATA_BLK"></xsl:value-of>']));
   ...
   <xsl:param name="curNode" />
     <xsl:for-each select="$curNode/FIELD">
                        <xsl:variable name="fldName" select="NAME"/>
   ...
   <xsl:template name="dispHiddenLabel_tmp" >
    ...
    <xsl:template name="dispLinkField_tmp">
        <a   href="#;return false" tabindex ='0'>
            <xsl:attribute name="onclick">
    ...
    <xsl:template name="dispCheckboxField_tmp">
        <oj-label>
            ...
            <oj-selector selection-mode="multiple">
                <xsl:call-template name="ATTR_Handler_tmp">
                    <xsl:with-param name="curr_fld" select=".." />
                </xsl:call-template>
            </oj-selector>
   ...
   <xsl:template name="dispTextField_tmp">
    <xsl:param name="EntityType"/>
    <oj-input-text type="text" class="TXTroDash">
...
<xsl:template name="ATTR_Handler_tmp">
    ...
    <!--<xsl:attribute name="ID">1203 oghag fix
  ...
  <xsl:template name="dispDateTimeField_tmp">
    <oj-input-text TYPE="HIDDEN" style="display:none" data_type="DATE" onpropertychange="fnFormatTimeStamp(this)">
...
<xsl:template name="dispDateField_tmp">
    <oj-input-text TYPE="HIDDEN" style="display:none" data_type="DATE" onpropertychange="displayDate(this)">
        <xsl:call-template name="ATTR_HiddenEntity_Handler_tmp">
            <xsl:with-param name="curr_fld" select=".." />
        </xsl:call-template>
    </oj-input-text>
    ...
    <oj-input-text TYPE="TEXT" class="TXTroDash" title="{../LBL}"  onblur="validateInputDate('{../NAME}', event)" readOnly="true" >
...
<xsl:template name="dispAmountField_tmp">
    ...
    <oj-input-text TYPE="HIDDEN" onpropertychange="displayAmountDB(this, '{$relatedFld}')" related_ccy = "{$relatedFld}"> <!--BUG 19457357 changes -->   
        <xsl:call-template name="ATTR_HiddenEntity_Handler_tmp">
            <xsl:with-param name="curr_fld" select=".." />
        </xsl:call-template>
    </oj-input-text>
    ...
    <INPUT TYPE="TEXT" class="TXTroDash numeric" title="{../LBL}" readOnly="true">
        ...
        <!--<xsl:attribute name = "onblur">
            <xsl:text disable-output-escaping="yes">validateInputAmountDB('</xsl:text>--><!-- BUG 19457357 changes--><!--
            <xsl:value-of select="../NAME"/>
            <xsl:text disable-output-escaping="yes">', '</xsl:text>
...
<xsl:template name="dispNumberField_tmp">
    ...
    <xsl:variable name="refresh" select="substring-before(substring-after($XslLabels, 'LBL_REFRESH~~'), '@@')" /><!--Fix for 21609958 -->
    <xsl:variable name="more" select="substring-before(substring-after($XslLabels, 'LBL_MORE~~'), '@@')" /> <!--HTML5 changes 2/NOV/2016 Fix for 24904397-->
