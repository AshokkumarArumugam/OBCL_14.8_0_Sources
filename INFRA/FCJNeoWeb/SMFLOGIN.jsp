...
Copyright © 2004-2016 by Oracle Financial Services Software Limited..
  ...
  **  Modified Reason      : Changes done to close the application window when invalid session error pops 
                             up. The Application window will be closed on clicking OK of the error message. 
  **  Retro Source         : 9NT1606_11_3_BANK_OF_SCOTLAND_PLC
  **  Search String        : 9NT1606_12_2_RETRO_11_3_23656224
...
<html class="loginHtml" lang="<%=StringEscapeUtils.escapeHTML(StrlangISO)%>">
    ...
    if(!"".equals(ieCss)) {
    %>
        <link href="Theme/Ext<%=StringEscapeUtils.escapeURL(ieCss)%>" rel="stylesheet" type="text/css"/>
        <%
    }
    %>
        <%  
    ...
    /* # BUG 14278984 fixes end */  
       ...
       if(releaseMap == null){
        ...
        if ("FGL".equalsIgnoreCase(BranchConstants.APPLICATION_NAME)) {
			...
			title = (String)itemDescMap.get("LBL_FCUBS_TITLE");
        }
        %>
        <title>
    ...
    <script type="text/javascript">
        ...
        function CheckUser() {
            ...
            if (document.getElementById("LOGINUSERID").value == "") { /* HTML5 Changes 6/OCT/2016 */
                ...
                document.getElementById("LOGINUSERID").focus(); /* HTML5 Changes 6/OCT/2016 */
                return false;
            } else if (document.getElementById("user_pwd").rawValue == "" && SSO_REQ != "Y") {
                ...
                var password = document.getElementById("user_pwd").rawValue;
...
//                }
        ...
        function CheckUserForSSLEnabled() {
            ...
            if (document.getElementById("LOGINUSERID").value == "") { /* HTML5 Changes 6/OCT/2016 */
                ...
                selCSS = document.getElementById("IDCSS");
                document.getElementById("LOGINUSERID").value = document.getElementById("LOGINUSERID").value.toUpperCase(); /* HTML5 Changes 6/OCT/2016 */
                document.getElementById("user_pwd").value = document.getElementById("user_pwd").rawValue;// Ashok -- Password is  case sensitive.
                var userId = document.getElementById("LOGINUSERID").value; /* HTML5 Changes 6/OCT/2016 */
        ...
        function checkErr() {
            ...
            if (window.name != "LoginWindow" && SSO_REQ == "Y") {
                alertXML = fnBuildAlertXML('', 'I', loginProperChannel);
                mask();
                showAlerts(alertXML, 'I');
                document.getElementById("LOGINUSERID").disabled = true; /* HTML5 Changes 6/OCT/2016 */
                document.getElementById("user_pwd").disabled = true;
                document.getElementById("IDCSS").disabled = true;
                document.getElementById("fc_sbmit").disabled = true;
                return;
            }
            fnEnableScreen();
...
//      mask();
//      document.getElementById("Div_AlertWin").style.display = "block";
//       document.getElementById("ifr_AlertWin").className += " frame-border";
//      document.getElementById("ifr_AlertWin").src="MFALOGIN.jsp";
      
      var alertWindow = document.getElementById("ifr_AlertWin");
        
        
        alertWindow.src = "MFALOGIN.jsp";
        var alertWinObj = document.getElementById("Div_AlertWin");
...
// redundunt Code commented as part of OJET migration       
...
//
    ...
    <body class="loginBody-main" onkeydown="windowCtrl(event)" oncontextmenu="return false;" onunload="doUnload()">
        ...
        <div class="loginCont-main">
            <div class="oj-sm-margin-4x-top" align="center">
            <div>
            <img src="./Images/login_color_Strip.svg" alt="background strip" class="oj-sm-width-full background-strip"/>
            </div>
           <img src="./Images/oracle.png" class="login-logo" alt="logo image">
           
            
            <form class="login-form" method="POST" action="LoginValidateServlet?msgType=NONWORKFLOW&amp;actionType=login" name="frmLogin" onsubmit="return CheckUser()" autocomplete="off">
            ...
            <input id="logintheme" type="hidden" name="logintheme" value="L"/><!--HTML5 Changes-->
             ...
             <div id="username" class="login-field oj-sm-margin-4x-top" align="center">
               
                <%if (extlogin.equalsIgnoreCase("N")){%>
                    <oj-input-text slot="value" id="LOGINUSERID" value='' name="USERID" title='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_USER_ID"))%>' label-hint='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_USER_ID"))%>'  tabindex="0" size="15" maxlength="20" />
                <%} else {%>
                    <oj-input-text  slot="value" id="LOGINUSERID" value='' name="USERID" title='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_USER_ID"))%>'  label-hint='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_USER_ID"))%>' tabindex="0" size="15" maxlength="30" />
                <%}%>
              
            </div>
            <div  class="login-field oj-sm-margin-4x-top" align="center">
                <oj-input-password slot="value" id="user_pwd" name="user_pwd" value='' onpaste="return false;" title='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_LOGIN_PWD"))%>' label-hint='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_LOGIN_PWD"))%>'  maxlength="35" size="15" tabindex="0" />
            </div>

            <div id="submit" class="loginSubmitBtn oj-sm-margin-8x-top" >
                <oj-button class="action-button-primary login-button" chroming="solid" type="submit" name="submit" value='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_SIGN_IN"))%>' id="fc_sbmit" tabindex="0" accesskey="l">
                <%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_SIGN_IN"))%></oj-button>
                <!--HTML5 Changes-->
               
                </div>
            <div id="login_cancel" class="loginCancelBtn oj-sm-margin-4x-top">
                <oj-button class="oj-button-primary login-button" chroming="full" name="cancel" onClick="handleCancel(event)" value='<%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_CANCEL"))%>' id="fc_loginCancel" >
                <%=StringEscapeUtils.escapeHTML((String) itemDescMap.get("LBL_CANCEL"))%></oj-button>
                <!--HTML5 Changes-->
        ...
        <div id="footer" class="loginFooter-main" align="center">
                    <%
           String copyRights = "Copyright © 2017 <br> Oracle and/or its affiliates. All rights reserved.";
		   //Fix for 15896771
...
//                copyRights = BranchConstants.APPLICATION_COPYRIGHTS;
        ...
        %>
                 ...
                 <div id="Div_AlertWin"   style="position:absolute;display:none" onclose="fnLoginUnloadAlert()" oncancel="fnExitAlertWin()"><!--HTML5 Changes-->
                        <iframe id="ifr_AlertWin" src="" allowtransparency="true" frameborder="0" scrolling="no"></iframe>
                    </div>
