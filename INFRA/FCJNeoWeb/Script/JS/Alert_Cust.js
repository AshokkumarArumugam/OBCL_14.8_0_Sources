...
function showAlerts(alertMsg, type, parentSqNo) {
    unmask();
    var tempScrType = "M";
    ...
    if(alertMsg != null && alertMsg != "") {
        ...
        var iFrameObj;//Fix for 21097061
        if(typeof(parentSqNo)!='undefined'){
            alertWindow = mainWin.document.getElementById(parentSqNo).children[0].contentWindow.document.getElementById("ifr_AlertWin");
        }else{
            //Fix for 21097061 start
            ...
            for(var cnt = 0; cnt < tabLinkArr.length; cnt++){
                ...
                iFrameObj = mainWin.document.getElementById("Content"+activeTab).children[0].children[0];
                alertWindow = iFrameObj.contentWindow.document.getElementById("ifr_AlertWin");
            //Fix for 21097061 end
        }
        if(alertWindow==null){
        ...
        debugs("Openning Alert_Cust.jsp with ", "MSG_TYPE="+type.toUpperCase()+"&MESSAGE=''&THEME="+strTheme+"&OVDSCRTYP="+tempScrType+"&OVDROUTINGTYP="+ovdRoutingType);
        alertWindow.src = encodeURI("Alert_Cust.jsp?MSG_TYPE="+type.toUpperCase()+"&MESSAGE="+msgChar+"&THEME="+strTheme+"&OVDSCRTYP="+tempScrType+"&OVDROUTINGTYP="+ovdRoutingType+"&ENTITY="+mainWin.entity+"&PARENTSQNO="+seqNo);/* security fixes for WF */
        var alertWinObj;
        if(typeof(parentSqNo)!='undefined'){
            alertWinObj = mainWin.document.getElementById(parentSqNo).children[0].contentWindow.document.getElementById("Div_AlertWin");
        }else{
            alertWinObj = iFrameObj.contentWindow.document.getElementById("Div_AlertWin");//Fix for 21097061
        }
        if(alertWinObj == null) {
...
function resize_iframe() {
    ...
    iframeObj.style.width = document.getElementById("wndtitle").offsetWidth+"px" ;//21845944
...
function closeAlerts(event, parentSqNo) {
    if(alertType == "O" && document.getElementById("REMARKS"))
        parent.override_remarks = document.getElementById("REMARKS").value;
    window.frameElement.src = "";
    window.frameElement.height = 0;
    ...
    //eval("parent."+parent.document.getElementById("Div_AlertWin").getAttribute("onclose"));
    //var fnEval = new Function("event","parent."+parent.document.getElementById("Div_AlertWin").getAttribute("onclose"));
    if (parentSqNo && parentSqNo != "") {
        for (var i = 0; i < mainWin.arrCustWindows.length; i++) {
            if (mainWin.arrCustWindows[i].id == parentSqNo) {
                iframeObj = mainWin.arrCustWindows[i].children[0].contentWindow;
                break;
            }
        }
    }
    if(typeof(iframeObj)=='undefined') {
       iframeObj = parent.parent.document.getElementById(parentSqNo).children[0].contentWindow;
    }
    var funcName= iframeObj.document.getElementById("Div_AlertWin").getAttribute("onclose");
    var fnEval = new Function("event", "iframeObj."+funcName);  
...
function showErrorAlerts(errcode,alertType,replaceString, parentSqNo){
    var alertXML = fnBuildAlertXML(errcode,'E','',replaceString);
    //mask();
    if(typeof(alertType) != 'undefined')
        showAlerts(alertXML, alertType, parentSqNo);
    else
        showAlerts(alertXML, 'I', parentSqNo);
	
	alertAction = "UNMASK";
}

function fnBuildAlertHTML(alertMsgTitle, imageClass, parentSqNo) {
    ...
    if(typeof(parentWin)!='undefined') {
        messageArr = parentWin.gAlertMessage.split("__");
    } else {
        messageArr = parent.parent.document.getElementById(parentSqNo).children[0].contentWindow.gAlertMessage.split("__");
    }
...
function setParentWinActive(evnt) {
    if (parent.seqNo && mainWin.document.getElementById(parent.seqNo)) {
        mainWin.document.getElementById(parent.seqNo).style.zIndex = 11;
    } else if (parent.parent.seqNo &&  mainWin.document.getElementById(parent.parent.seqNo)) {
