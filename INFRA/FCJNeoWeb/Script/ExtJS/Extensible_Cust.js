...
function fnSetPkvals(v_pkVals) {
  ...
  for (var i = 0;i < l_pkArray.length;i++) {
   ...
   // if (element.type.toUpperCase() == 'RADIO') {
    if (element.tagName.toUpperCase() == 'OJ-RADIO-SET') { //OJET Migration
      ...
      for (var lpIdx = 0;lpIdx < radioEle.length;lpIdx++) {
		...
		element.setAttribute("value", l_pkArray[i]) ;//OJET Migration
...
function fnLoad(xmlFileName, xslFileName) {//OJET Migration
  var t = getDateObject();
  var startjsTime = (t.getHours() * (3600 * 1000)) + (t.getMinutes() * (60 * 1000)) + (t.getSeconds() * 1000) + t.getMilliseconds();

  var tmpFnid = functionId;
    seqNo = getFrameForDocument().parentNode.id;
    prevSeqNo = parent.document.getElementById(seqNo).children[0].getAttribute("prevSeqNo");
...
//  fnBuildMultipleEntryArray(strCurrentTabId);OJET Migration
  ...
  debugs("loadCustWindow for seqno", seqNo);
  mainWin.loadCustWindow(parent.document.getElementById(seqNo), window);
...
responseDOM_Modify = loadXMLDoc(getXMLString(fcjResponseDOM)); //Fix for 19438327
  ...
  if (document.getElementsByName("ONCEAUTH")[0]) {
    if (document.getElementsByName("ONCEAUTH")[0].value == 'Y') {
     // fnChangeLabelToText("TEXTAREA");OJET Migration
...
function fnAuthorize() {
  ...
  //if(screenType == 'O'||screenType == 'PMO')
   if ((screenType.indexOf('O'))!= -1|| screenType == 'PMM') //31670971 added PMM TypeString //to support PM transaction screens
  {
      ...
      if (screenArgSource[i] == "")
        ...
        if (document.getElementById(block) && document.getElementById(block).getAttribute("TYPE") && document.getElementById(block).getAttribute("TYPE") == "ME" && document.getElementById(block).getAttribute("VIEW") != "SE") {
          ...
          if (typeof (document.getElementsByName(blockFld)[blkRowIndx]) != "undefined") {
            screenArgs[screenArgName[i]] = document.getElementsByName(blockFld)[blkRowIndx].value;
          }
          /* Fix for 17997956 Ends*/
        }
        else {
    ...
    for (var i = 0;i < udfFldSet.length;i++) {
        if (udfFldSet[i].getAttribute("VIEW") != "ME") {
            for (var k = 0;k < tagNameList.length;k++) {
                udfFields = udfFldSet[i].getElementsByTagName(tagNameList[k]);
...
function fnUpdateUDFDBField(currFld) {
  ...
  if (indexDate > 0 || indexAmount > 0 || indexNumber > 0) {
    if (document.getElementById("BLK_UDF_DETAILS"))
      document.getElementById("BLK_UDF_DETAILS").tBodies[0].rows[udfFldNo].cells[2].getElementsByTagName("OJ-INPUT-TEXT")[0].value = getPreviousSibling(getPreviousSibling(currFld)).value;
    if (document.getElementById("BLK_TXN_UDF_DETAILS"))
      document.getElementById("BLK_TXN_UDF_DETAILS").tBodies[0].rows[udfFldNo].cells[2].getElementsByTagName("OJ-INPUT-TEXT")[0].value = getPreviousSibling(getPreviousSibling(currFld)).value;
  }
  else {
    if (document.getElementById("BLK_UDF_DETAILS"))
      getTableObjForBlock("BLK_UDF_DETAILS").tBodies[0].rows[udfFldNo].cells[2].getElementsByTagName("OJ-INPUT-TEXT")[0].value = currFld.value;
    if (document.getElementById("BLK_TXN_UDF_DETAILS"))
      getTableObjForBlock("BLK_TXN_UDF_DETAILS").tBodies[0].rows[udfFldNo].cells[2].getElementsByTagName("OJ-INPUT-TEXT")[0].value = currFld.value;
  }
  //12.0.2 udf changes for CSCTRUDF end
...
//    }//HTML5 Changes End
    if(srcType && srcType == 'SUBSYSTEM') {
        parent.document.getElementById(seqNo).style.width = "100%";
        parent.document.getElementById(seqNo).children[0].style.width = "100%";
...
//  containerDIVObj.children[0].style.height = scrHeight + "px";
  ...
  //document.getElementById("mainTabContainer").style.width = "100vw";//static header change
...
//  var l_DivTmpHgt = 0;
//  if (document.getElementById("toolbar")) {
//    l_DivTmpHgt = parseInt(scrHeight) - $("#toolbar").outerHeight(true) - mainWin.document.getElementById("hTab_DBoardCustomer").offsetHeight;
//  }
//  else {
//    l_DivTmpHgt = parseInt(scrHeight) -  mainWin.document.getElementById("hTab_DBoardCustomer").offsetHeight;
//  }
//  document.getElementById("DIVMainTmp").style.height = parseInt(l_DivTmpHgt) + 'px';
  var divMainTmpChildren = document.getElementById("DIVMainTmp").children;
  var tempContainerHgt = 0;
...
function fnBuildTabHTML(flag) {//OJET Migration
  if (tab_arr.length == 0 || tab_ids.length == 0) {
    fnTabDetails();
  }
  for (var i = 0;i < tab_arr.length;i++) {
    if (tab_arr[i] && document.getElementById("TBLPage" + tab_arr[i].id)) {
...
//12.0.3 Summary to detail changes ends
//static header change
function fnSetScreenSize() {//OJET Migration
  if (g_scrType != 'L') {
...
function fnLoadCustomer(xmlFileName, xslFileName) {
  var t = getDateObject();
  var startjsTime = (t.getHours() * (3600 * 1000)) + (t.getMinutes() * (60 * 1000)) + (t.getSeconds() * 1000) + t.getMilliseconds();
  ...
  try {
    ...
    if (typeof (mainWin.screenHtmlCache[tmpFnidScrName]) == "undefined") {
      ...
      mainWin.screenHtmlCache[tmpFnidScrName].cachedHTML = document.getElementById("ResTree").innerHTML;
      mainWin.screenHtmlCache[tmpFnidScrName].cachedScrName = strScreenName;
      mainWin.screenHtmlCache[tmpFnidScrName].cachedScrType = g_scrType;
      mainWin.screenHtmlCache[tmpFnidScrName].cachedSubScrHeaderTabId = subScrHeaderTabId;
      mainWin.screenHtmlCache[tmpFnidScrName].cachedSubScrBodyTabId = subScrBodyTabId;
...
function fnCalcHgtCustomer(){
/*HTML5 Changes Start*/
  var dBoardContainerHeight =  mainWin.document.getElementById('vTabDB_DASHBOARD').offsetHeight - (mainWin.document.getElementById('hTab_DBoardCustomer').offsetHeight);
  var scrHeight = parseInt(mainWin.document.getElementById("mainContent").offsetHeight);
  //var dBoardContainerWidth =  mainWin.document.getElementById('dashboard').offsetWidth;
/*HTML5 Changes End*/
  mainWin.document.getElementById("hTabCN_Customer").style.height = scrHeight - mainWin.document.getElementById("tabListCust").offsetHeight +'px';
  //mainWin.document.getElementById("hTabCN_Customer").style.width = dBoardContainerWidth+'px';
}



function fnLoad1(xmlFileName, xslFileName) {
  var t = getDateObject();
  var startjsTime = (t.getHours() * (3600 * 1000)) + (t.getMinutes() * (60 * 1000)) + (t.getSeconds() * 1000) + t.getMilliseconds();
  ...
  if(parent.document.getElementById(seqNo).children[0].getAttribute("TYPE") == 'TABS'){
    var frameElem = parent.document.getElementById(seqNo).children[0];
    for(j=0;j<frameElem.contentWindow.document.getElementsByTagName('IFRAME').length;j++){        
            frameElem.contentWindow.document.getElementsByTagName('IFRAME')[j].setAttribute("prevSeqNo", prevSeqNo);
        }
  ...
  //static header change start
  //fnSetScreenSize();
  if( srcType =='TABS' )//Customer Accessibility changes
    highlighttab(document.getElementById(strCurrentTabId));
  /*if (ShowSummary != "TRUE")
    expandContentLoad(strCurrentTabId);//12.0.3 changes*/
  //fnBuildMultipleEntryArray(strCurrentTabId);
  //Static Header change end
...
function fnCalcScreenSize() {
    var Width = parent.document.getElementById(seqNo).children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].children[0].scrollWidth;
    var Height = parent.document.getElementById(seqNo).children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].children[0].scrollHeight;

//    parent.document.getElementById(seqNo).children[0].style.width = Width +  "px";
    parent.document.getElementById(seqNo).children[0].style.width = "100%";
    parent.document.getElementById(seqNo).children[0].style.height = Height + "px";
    var parentWidth = parent.parent.document.getElementById(parent.seqNo).children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].children[0].scrollWidth;
    var parentHeight = parent.parent.document.getElementById(parent.seqNo).children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].children[0].scrollHeight;
//    parent.parent.document.getElementById(parent.seqNo).children[0].style.width = parentWidth + "px";
    parent.parent.document.getElementById(parent.seqNo).children[0].style.width = "100%";
    parent.parent.document.getElementById(parent.seqNo).children[0].style.height = parentHeight + "px";
    if (typeof (parent.parent.seqNo) != 'undefined') {
        var superParentWidth = parent.parent.parent.document.getElementById(parent.parent.seqNo).children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].children[0].scrollWidth;
        var superParentHeight = parent.parent.parent.document.getElementById(parent.parent.seqNo).children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].children[0].scrollHeight;
        //parent.parent.parent.document.getElementById(parent.parent.seqNo).children[0].style.width = superParentWidth + "px";
        parent.parent.parent.document.getElementById(parent.parent.seqNo).children[0].style.width = "100%";
        parent.parent.parent.document.getElementById(parent.parent.seqNo).children[0].style.height = superParentHeight + "px";
    }
}

function getFrameForDocument() {
