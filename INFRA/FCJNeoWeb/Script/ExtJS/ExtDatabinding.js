...
**  Change Description   : Redwood fixes 
**  Search String        : REDWOOD_35231803
                           REDWOOD_35200441
 ...
 **  Modified By          : MANOJ S/Girish
  ...
  **  Modified Reason      : Redwood Fixes
  **  Search String        : REDWOOD_35243127
  **
  **  Modified By          : Shayam Sundar Ragunathan
  **  Modified On          : 02-May-2023
  **  Reason               : Changes done to format OJ-INPUT-DATE-TIME tag values
  **  Search String        : REDWOOD_35277235
  **
  **  Modified By          : Manoj S
  **  Modified On          : 04-May-2023
  **  Reason               : Changes made to add the first option value from the drop-down menu which has no default value in the Multigird.
  **  Search String        : REDWOOD_35349308
  **
  **  Modified By          : Manoj S
  ...
  **  Reason               : Changes made to add new row with Editable of Non Amendable Fiedls 
  **  Search String        : REDWOOD_35303668  
  **
  **  Modified By          : Girish M
  **  Modified On          : 14-May-2023
  **  Reason               : Date Time support changes.
  **  Search String        : REDWOOD_35388757
  **
  **  Modified By          : Girish M
  **  Modified On          : 10-June-2023
  **  Reason               : OJ-TABLE refresh handling
                             Null values for drop down hanlding
  **  Search String        :REDWOOD_35472205
                            REDWOOD_35434969 
  **
  **  Modified By          : Girish M
  **  Modified On          : 31-July-2023
  **  Reason               : OJ-TABLE refresh handling for default and subsys pickup operations 
  **  Search String        : REDWOOD_35615856                    
  **
  **  Modified By          : Selvam Manickam
  **  Modified On          : 09-Aug-2023
  **  Reason               : Default value not assign to OJ-SINGLE-SELECT even though DEFAULTSEL attribute have values.
  **  Search String        : REDWOOD_35488341  
  ...
  **  Modified Reason      : Redwood Fixes to keep disalbed NON-AMENDABLE FIELD in grid 
  **  Search String        : redwood_35691800
 
  **  Modified By          : Selvam Manickam
  **  Modified On          : 13-Sep-2023
  **  Modified Reason      : OJ-SWITCH fields should be disable in ME when Action code as EXECUTEQUERY
  **  Search String        : REDWOOD_35780922  
   
  **  Modified By          : Girish M
  **  Modified On          : 28-Sep-2023
  **  Modified Reason      : Redwood Fixes to handle null amount.
  **  Search String        : REDWOOD_35840493 
  
  **  Modified By          : Manoj S
  **  Modified On          : 12-Dec-2023
  **  Modified Reason      : Redwood Fixes to handle non amendable button on Modify.
  **  Search String        : redwood_36021606
  
  **  Modified By          : Manoj S
  **  Modified On          : 24-Jan-2024
  **  Modified Reason      : increased the time to defaulting drop-down value in multiEntry
  **  Search String        : redwood_36213474
  
   
  **  Modified By          : Manoj/Girish
  **  Modified On          : 16-Apr-2024
  **  Modified Reason      : Local ccy precision was taking precedence even if settlement ccy is provided.
  **  Search String        : REDWOOD_36419564  

  **  Modified By          : Girish
  **  Modified On          : 05-May-2024
  **  Modified Reason      : Date field with Lov button handling.
  **  Search String        : REDWOOD_36688469 
   
  **  Modified By          : Girish
  **  Modified On          : 24-July-2024
  **  Modified Reason      : Multi Entry new row addition not enabling for modify operation.
  **  Search String        : REDWOOD_36844383  
  
  
  **  Modified By          : Sudharshini Balaji
  **  Modified On          : 15-Oct-2024
  **  Change Description   : Fix provided to initialize the childValueXML outside the loop on loading data in the Child blocks on pagination.(OBCL BUG)
  **  Search String        : Bug_37231271
   
  **  Modified By          : Manoj S
  **  Modified On          : 10-Jan-2025
  **  Modified Reason      : In Multi Entry, additional conditions are used to validate Switch or checkbox values. .
  **  Search String        : REDWOOD_37370284
----------------------------------------------------------------------------------------------------
*/
...
if ( typeof(extBpelJs) !=  'undefined' && extBpelJs != '' ) {
    coreFnPost = fnPost;
    coreAppendTabData = appendTabData;
}		  
//REDWOOD_CHANGES
//Performance Changes
    ...
    if (!isException) {
        if (objHTTP.status != 200) {
            ...
            mainWin.inactiveTime = 0;
                ...
                //E2E Changes
                if (typeof (reqEncReqd) != "undefined" && reqEncReqd == "Y") {
                    responseDOM = loadXMLDoc(AESDecrypt(objHTTP.responseText)); 
                }else{
                    responseDOM = objHTTP.responseXML
                }
            if (responseDOM != null) { //Bug_30849927
            var csrfNode = selectSingleNode(responseDOM, "//CSRF");
            ...
            } else if (selectSingleNode(responseDOM, "//ERROR") != null && ((getNodeText(selectSingleNode(responseDOM, "//ERROR")).indexOf("OF-4560") !=  - 1) || (getNodeText(selectSingleNode(responseDOM, "//ERROR")).indexOf("SM-00420") !=  - 1))) {
                //12.1 session expiry fix 2
                alert(getNodeText(selectSingleNode(responseDOM, "//ERROR")));
                /*Security bug 9843512 fixes ends*/
            } else if (selectSingleNode(responseDOM, "//SESSION") != null && getNodeText(selectSingleNode(responseDOM, "//SESSION")) == "EXPIRED") {
            ...
            } else if(selectSingleNode(responseDOM, "//SAFARIREQ") != null && getNodeText(selectSingleNode(responseDOM, "//SAFARIREQ")) == "TRUE") {
                fnSetDelay(2000);
...
function appendTabData(tabObject) {//OJET Migration	//REDWOOD_CHANGES
if(!tabObject) return;	 //REDWOOD_CHANGES
    var operation = gAction;
    if (operation != '' && operation != 'ENTERQUERY' && operation != 'EXECUTEQUERY') {
        if (tabObject.innerHTML == "") {
            var id = tabObject.id;
            if (id.indexOf("TBLPage") !=  - 1) { //REDWOOD_CHANGES
  ...
  //          fnBuildMultipleEntryArray(id);  //REDWOOD_CHANGES
        ...
        for (var fldSetCnt = 0;fldSetCnt < fldSetList.length;fldSetCnt++) {
            ...
            if (l_fieldSetType == 'ME' && l_fieldSetView != 'SE') {
               ...
               // if(!l_tableobj) return; //REDWOOD_CHANGES
			    if(l_tableobj){ //REDWOOD_CHANGES
...
function appendTableValue(tableObject, id, DBT) {//OJET Migration //REDWOOD_CHANGES
    var rowList = tableObject.tBodies[0].rows;
    var tblId = tableObject.parentNode.parentNode.id;//OJET-Arun  //REDWOOD_CHANGES
    
    var cPage = Number(document.getElementById('paging_'+tblId+'_nav_input').value) - 1;//OJET-Arun //REDWOOD_CHANGES
    if (cPage ==  - 1) {
        cPage = 0;
    }
    var rowIndx = 0;
...
function showAllData() {
    ...
    if (document.getElementById("tablist")) {
        tabs = tabList.getElementsByTagName("li");
        for (var tabCnt = 0;tabCnt < tabs.length;tabCnt++) {
           if(! tabs[tabCnt].classList.contains( "oj-navigationlist-divider") && tempCurrentTabId != tabs[tabCnt].childNodes[0].id ){ //OJET Migration //REDWOOD_CHANGES
...
//REDWOOD_CHANGES
            if(html!= '' && html!=null){
                 screenKo.cleanNode(tabObject);
                html = html.replace(new RegExp("readonly_temp", 'g'), "readonly").replace(new RegExp("oj_switch_readonly", 'g'), "disabled");
//REDWOOD_CHANGES 
           tabObject.innerHTML = html;
 //REDWOOD_CHANGES               
                ...
                fnBindScreenElements(tabObject);//OJET Migration
                //screenKo.applyBindings( {},tabObject); //REDWOOD_35243127 -Commented
...
//REDWOOD_CHANGES           
      ...
      //  fnBuildMultipleEntryArray(objName); //REDWOOD_CHANGES
    }
    var fldSetList = tabObject.getElementsByTagName("fieldset");
    showFldsetData(fldSetList);
}

function showFldsetData(fldSetList) {//OJET Migration	  //REDWOOD_CHANGES
    var recLength = 0;/*12.0.4 UI performance changes starts*/
    for (var fldSetCnt = 0;fldSetCnt < fldSetList.length;fldSetCnt++) {
        var l_curBlock = fldSetList[fldSetCnt].getAttribute("block");
        var l_fieldSetType = fldSetList[fldSetCnt].getAttribute("type");
        var l_fieldSetView = fldSetList[fldSetCnt].getAttribute("view");
        if (l_fieldSetType == 'ME' && l_fieldSetView != 'SE') {//debugger; //REDWOOD_CHANGES
            var l_tableobj = document.getElementById(l_curBlock);// getTableObjForBlock(l_curBlock);  //REDWOOD_CHANGES
            if (l_tableobj) {
                nodeName = l_curBlock;//l_tableobj.id  //REDWOOD_CHANGES
...
//REDWOOD_CHANGES
                    ...
                    //REDWOOD_35615856 Starts
                      if(gAction!='NEW' && gAction!='MODIFY' && gAction!='COPY' && gAction!=""){  
                        meArrayForAddDelete[nodeName]([]);
                      } 
                      //REDWOOD_35615856 Ends
...
function setData(currObject, value) {//OJET Migration  //REDWOOD_CHANGES
...
//REDWOOD_CHANGES
        {
            ...
            if (value == "") {
				...
				//REDWOOD_35231803
				  if (currObject.getAttribute("DEFAULT") == "yes" && currObject.tagName =="OJ-RADIOSET") {
                     currObject.value = currObject.getAttribute('value');
                     }
				//REDWOOD_35231803	 
            }
     ...
     //REDWOOD_CHANGES   
        case "OJ-SELECT-SINGLE": {
            ...
            if (DBC) {
                if (value != "") {
                    currObject.value = value;
                }else if (currObject.getAttribute("DEFAULTSEL") != "") { //REDWOOD_35488341
                    currObject.value = currObject.getAttribute("DEFAULTSEL"); //REDWOOD_35488341 
					
				}else if (value == "") { //REDWOOD_35434969 
                    currObject.value = value; //REDWOOD_35434969 
               }
            }
            break;
        }
            ...
            for (k = 0;k < ojText.length;k++) {
                if (ojText[k].getAttribute("converter") != null) {
                    ...
                    if (converterName.includes("amountConverter")) {
                       if  (ojText[k].value !="" && ojText[k].value !=null ){ //REDWOOD_35200441 //REDWOOD_35840493 added null.
                        var ccy = getCurrencyValue(ojText[k],  ojText[k].getAttribute("related_field"), false, true);
						if(getNumDigitsAfterDecimal(ccy)>0){ //REDWOOD_36419564
...
function showPageWise(htmlTableObj, parentNodeName, nodeList, isNew) {
    //debugger;
    //meArrayForAddDelete[parentNodeName]([]); //REDWOOD_35472205
   // document.getElementById(parentNodeName).refresh();
...
//            updateAmountOrNumberConverter(parentNodeName);
        ...
        0);
       if(viewMnt){//REDWOOD_35670751 starts
         setTimeout(function () {
           updateColorOnRow(parentNodeName);  },
        500);
...
function updateColorOnRow(parentNodeName){//REDWOOD_35670751 starts
      ...
      for(var rowLength=0;rowLength<mainTableObj.tBodies[0].rows.length;rowLength++) {
            var row = mainTableObj.tBodies[0].rows[rowLength];
            for (var k = 0; k < row.cells.length; k++) {
              var cell = row.cells[k];
              if(cell.children[0] && cell.children[0].children[0]&& cell.children[0].children[0].getAttribute('viewchanges')){
                 cell.children[0].children[0].setAttribute('style','color:'+cell.children[0].children[0].getAttribute('viewchanges'))
              }
            }
        }
}//REDWOOD_35670751 ends
function addRowsFromDOM(htmlTableObj, parentNodeName, nodeList, isNew) {
	...
	var disableButton=true;//redwood_36021606
    if (ojTableObj) {
    ...
    for(var i = 0;i<nodeList.length;i++){
		...
		//redwood_35691800 starts
		if (gAction == 'MODIFY' && (onceAuthObj!= null && onceAuthObj!= "undefined"))
		{
			if(onceAuthObj.value!='Y')
			{
				singleRecObj['readOnly'] = false;
				disableButton=true;//redwood_36021606
			}
			else 
			{
				 singleRecObj['readOnly'] = true;
				 disableButton= false;//redwood_36021606
			}
		}//redwood_35691800 ends
        else if (gAction != '' && gAction != 'EXECUTEQUERY') {
            singleRecObj['readOnly'] = false;
        }else{
             singleRecObj['readOnly'] = true;
        }
        ...
        for(var k=0; k<singleRec.childNodes.length;k++){
            ...
            for(var tdObjCnt = 0; tdObjCnt<tdObj.length ; tdObjCnt++) {
                ...
                if(tdObjElem.getAttribute("name") == nodeName) {
                  ...
                  if ((gAction != '' && gAction != 'EXECUTEQUERY' && gAction != 'REVERSE' && gAction != 'REMOTEAUTH' && gAction != 'VIEW' && gAction != 'AUTH')) {//redwood_36021606
                    ...
                    if(tdObjElem.tagName.toUpperCase() == "OJ-INPUT-NUMBER") {
                        ...
                        if(getNodeText(singleRec.childNodes[k])!=null && getNodeText(singleRec.childNodes[k])!=''){
                            if(tdObjElem.getAttribute("ON") == getNodeText(singleRec.childNodes[k]) || CHECK_B0X_SELECTED_VALUE==getNodeText(singleRec.childNodes[k])) { //Redwood_37370284
                                singleRecObj[nodeName] = true;
                            } else {
                                singleRecObj[nodeName] = false;
                            }
                        }
						//REDWOOD_35780922 START below code cause issue when ME have oj-switch 
                       /*  //REDWOOD_35384868 Starts
						if(tdObjElem.getAttribute("readonly") == "true" || tdObjElem.getAttribute("readonly1") == "true"){
							tdObjElem.setAttribute("disabled",true);
						}else{
							tdObjElem.setAttribute("disabled",false);
              }
                        //REDWOOD_35384868 Ends */
				  //REDWOOD_35780922 End
                        ...
                        for(var selectLength=0;selectLength<selectControl[tdObjElem.id].length;selectLength++) {
							...
							if (typeof (action) != "undefined") {
									if (action == 'M') {
											...
											var sp = getNodeText(singleRec.childNodes[k]).lastIndexOf("_OB_");
											var sp1 = getNodeText(singleRec.childNodes[k]).lastIndexOf("_CB_");
											if (sp>=0)
											{	
												if (selectControl[tdObjElem.id][selectLength].value == getNodeText(singleRec.childNodes[k]).substr(0,sp)){
													
													singleRecObj[nodeName] = getNodeText(singleRec.childNodes[k]).substr(0,sp);																						
															singleRecObj[nodeName+'color'] =color;
                    ...
                    //REDWOOD_35277235 Starts
                    else if(tdObjElem.tagName.toUpperCase() == "OJ-INPUT-DATE-TIME") {
                        if (getNodeText(singleRec.childNodes[k]) != '')
                        {					
                            var timeStamp = new Date(getNodeText(singleRec.childNodes[k]));
                            singleRecObj[nodeName] = timeStamp.toISOString();                    
                        }
                        else
                            singleRecObj[nodeName] = getNodeText(singleRec.childNodes[k]);
                    //REDWOOD_35277235 Ends
                    } else {
                    //REDWOOD_35670751 starts
					singleRecObj[nodeName] = getNodeText(singleRec.childNodes[k]);
						if(viewMnt){
							if (typeof (action) != "undefined") {
								var fieldValue =  getNodeText(singleRec.childNodes[k]);
								if (action == 'M') {                           
								var s1 = fieldValue.indexOf("_OB_");
								var s2 = fieldValue.indexOf("_CB_");
								if (s1 >= 0) {
									var curval = fieldValue.substring(0, s1);
									var preval = fieldValue.substring(s1 + 2, s2);
									singleRecObj[nodeName] = curval;                               
									singleRecObj[nodeName+'color'] =color;
								}
								
								
							}else if (action == "N") {
								singleRecObj[nodeName] =fieldValue;
								singleRecObj[nodeName+'color'] =color;
							}else if (action == "D"){ 
								singleRecObj[nodeName] =fieldValue;
							singleRecObj[nodeName+'color'] =color;
		...
		setTimeout(function(){
			...
			for(var rowLength=0;rowLength<mainTableObj.tBodies[0].rows.length;rowLength++) {
				...
				if(mainTableObj.tBodies[0].rows[0].cells[0].children[0].tagName.toUpperCase()=='OJ-SELECTOR') {
							fnEventsHandler('fnPostAddNewRow_' + parentNodeName, mainTableObj.tBodies[0].rows[rowLength]);
...
//     redwood_36213474 Moved here
			...
			setTimeout(function(){ 
				...
				//REDWOOD_35349308 Ends
		
//redwood_36213474 starts
				if (gAction != '' && gAction != 'EXECUTEQUERY' && gAction != 'REVERSE' && gAction != 'REMOTEAUTH' && gAction != 'VIEW' && gAction != 'AUTH')//REDWOOD_35303668
				{
					//setTimeout(function(){			
					enableRowElements(getTableObjForBlock(tableName).tBodies[0].rows[rowIndex-1]);
					//},timer);
				}
 //REDWOOD_36844383 Moved the below code here.
				var dateField = getTableObjForBlock(tableName).tBodies[0].rows[rowIndex-1].getElementsByTagName('OJ-INPUT-DATE');
				for(var i = 0;i<dateField.length;i++){
					if( dateField[i]){
						 fnFireCustomDateEvent(dateField[i]);
						 //btns[i].refresh();
					}
				}				
			},20);
//      redwood_36213474 end
		//},dtime);
	//REDWOOD_35327971 starts //redwood_36213474 comented
/*	var timer=0;
...
*/ //redwood_36213474				
    ...
    },dtime);//redwood_36213474 increased the time 0 to 2  //redwood_36213474 dtime added instead of numeric
...
function setDataViewChg(currObject, value) {
    ...
    setTimeout(function(){	//REDWOOD_35277235
...
function getFieldData(currObject) {
    ...
    switch (tagName.toUpperCase()) {
        ...
        //REDWOOD_36688469 Starts
       if(currObject.getAttribute('DateValue') && fieldValue!=null && fieldValue!= "" && currObject.getAttribute('dtype')=='DATE' && currObject.getAttribute('DateValue') !=null){
                  fieldValue = currObject.getAttribute('DateValue');
                }
...
function fnProcessPartialFCJRecNode(pobjFCJXMLRecNode, relationshipArray, fieldNameArray, id, recType, startId, nodeName, isLast, tmpstartid) {
    ...
    returnXMLValue += fieldXML;
		childValueXML = ""; //Bug_37231271 CHANGES
    if (relationshipArray[blkName]) {
        var childBlkNames = relationshipArray[blkName].split("~");
        for (var i = 0;i < childBlkNames.length;i++) {
            var childRecLen = 0;
		...
		//	childValueXML =""; //9NT1606_12_4_RETRO_12_1_26780563 COMMENTED FOR Bug_37231271 CHANGES
...
function appendTextFieldValue(textObject, id, DBT) {
    ...
    //value = NVL(textObject.rawValue) ? NVL(textObject.rawValue) : NVL(textObject.value);
    //var value = NVL(textObject.value);    var txtObjHTML = getOuterHTML(textObject);
...
//        value = value.replace(".", decimalSymbol);
    ...
    if (DBC != "" && DBC != null) {
        ...
        if(value!=null && textObject.tagName.toUpperCase() == 'OJ-INPUT-DATE-TIME'){
          ...
          value = value.substring(0, 10); //REDWOOD_35388757
        }
 //REDWOOD_36688469 Starts
		if (value!=null && value!="" && textObject.tagName.toUpperCase() =='OJ-INPUT-TEXT' && textObject.getAttribute('dtype')=='DATE'  && textObject.getAttribute('DateValue') !=null)
              {
                value =textObject.getAttribute('DateValue');
		 }
        //REDWOOD_36688469 Ends
        if (currNode) {
...
function appendSelectFieldValue(selectObject, id, DBT) {
    ...
    if (DBC != null && DBC != "") {
        DBC = DBC.toUpperCase();
        if (selectObject.value !="") { //REDWOOD_35434969 
            value = NVL(selectObject.value,value); //REDWOOD_35434969 
            value = gEncodeData(value);
        }
