...
function resetElements(obj) {//OJET Migration
...
function fnSetExitButton() { //OJET Migration
	...
	try { 
            if (gAction!="ENTERQUERY" && gAction!="NEW" && gAction!="COPY") {
		setTimeout(function(){document.getElementById("BTN_EXIT_IMG").focus();},0);
            }
	} catch(ex){}
	//Fix for 25400971  ends
    if (gAction == "EXECUTEQUERY" || gAction == "") {
        document.getElementById("BTN_EXIT_IMG").label = mainWin.getItemDesc("LBL_EXIT");
    } else {
        document.getElementById("BTN_EXIT_IMG").label = mainWin.getItemDesc("LBL_CANCEL");
    }
}
...
function fnClearPKFields() {
    for (var fieldIndex = 0; fieldIndex < pkFields.length; fieldIndex++) {
        if (document.getElementById(pkFields[fieldIndex])) {
            if (document.getElementById(pkFields[fieldIndex]).tagName.toUpperCase() == 'SELECT') {
                ...
                for (var optnCnt = 0; optnCnt < selOptions.length; optnCnt++) {
                    if (selOptions[optnCnt].getAttribute("DEFAULT") != null) {
                        anySelected = true;
            ...
            }else if (document.getElementById(pkFields[fieldIndex]).getAttribute("type") && document.getElementById(pkFields[fieldIndex]).getAttribute("type").toUpperCase() == 'RADIO') {
            ...
            } else if (document.getElementById(pkFields[fieldIndex]).getAttribute("type")&& document.getElementById(pkFields[fieldIndex]).getAttribute("type").toUpperCase() == 'CHECKBOX') {
                var tmpElem = document.getElementById(pkFields[fieldIndex]);
...
function fnEnableAmendFields(v_action,amendFields) {/*Amend Array Changes*/
    var l_amendArrayName = window[v_action + "AmendArr"];
    for (var j = 0; j < multipleEntryIDs.length; j++) { //OJET Migration
       // if (document.getElementById(multipleEntryIDs[j] + "Header_CHK_ME")) { 
       if(document.getElementsByName("cmdAddRow_" + multipleEntryIDs[j])[0]){
               ...
               if(amendableFld[j].charAt(amendableFld[j].length - 1) == 'I') {
                ...
                var amndFld = amendableBlk+"__"+amendableFld[j];
                    ...
                    if (document.getElementById(amndFld)) {
                        if (document.getElementById(amndFld).tagName.toUpperCase() == 'OJ-RADIO') {
                            var amendFlds = document.getElementsByName(amendableFld[j]);
                            for (var k = 0; k < amendFlds.length; k++) {
                                fnEnableElement(amendFlds[k]);
                            }
...
/*
function fnEnableAmendFields(v_action) {//debugger;
    //var l_amendArrayLen = eval(v_action + "AmendArr").length;
    var l_amendArrayLen = window[v_action + "AmendArr"].length;
    //var l_amendArrayName = eval(v_action + "AmendArr");
    var l_amendArrayName = window[v_action + "AmendArr"];
        ...
        for (var i in eleArray) {
            ...
            for (var elemIndex = 0; elemIndex < elements.length; elemIndex++) {
                ...
                if (elements[elemIndex].getAttribute("REQUIRED") == "true") {
                    ...
                    if (tempVal == null || isNull(tempVal)) {
...
function fnCloseAlertWin_SWITCH_TAB(evnt) {
	mainWin.alertFlag = false;
	mainWin.showDBoardTabs(tabid, evnt);//HTML5 Changes
        return true;
}

function fnCloseAlertWin_SWITCH_ACC(evnt) {
	mainWin.alertFlag = false;
	mainWin.fnToggleDisplay(divId);
        return true;
}