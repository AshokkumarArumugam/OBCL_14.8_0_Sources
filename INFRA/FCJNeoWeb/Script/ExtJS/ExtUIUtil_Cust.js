...
function resetAllElements(type, obj) { //OJET Migration
    var elements;
...
function fnSetFocusOnMasterPKField() {
    if (pkFields && pkFields.length > 0) {
        ...
        for (var l_PkCount = 0; l_PkCount < l_SortedPkFlds.length; l_PkCount++) {
            ...
            if (l_PkElement_Misc != null) l_PkElement = l_PkElement_Misc;
            try {
                if (l_PkElement) {
...
function enableForm(obj) {//OJET Migration
...
function disableForm(obj) {//Fix for 2125450, OJET Migration
	...
	disableAllElements("OJ-INPUT-NUMBER", obj); //OJET Migration
        ...
        disableAllElements("OJ-SWITCH",obj); //OJET Migration
...
//                        var fieldSize;
						//Fix for 20856727 start
//                        if(object.getAttribute("ondblclick")){
//                            var fieldOndblclick = object.getAttribute("ondblclick");
//                        }
                        //Fix for 20856727 end
...
//									textareaNode.setAttribute("COLS", fieldSize);
									//Fix for 20856727 start
//                                    if(typeof(fieldOndblclick) != 'undefined'){
//                                        textareaNode.setAttribute("ondblclick", fieldOndblclick);
//                                    }
                                    //Fix for 20856727 end
...
function fnEnableElement(object) {//OJET Migration
    if (object == null)
        return;
    ...
    if (object) {
        if (type == 'HIDDEN') {
                            ...
                            if (nextSibling.name == object.name + "I") {
                                if (nextSibling.getAttribute("READONLY1")) {
                                   ...
                                   // nextSibling.className = "TXTro numeric"
                                    nextSibling.setAttribute("readOnly",true);//OJET Migration
                                } else {
                                    nextSibling.disabled = false;
                                    nextSibling.setAttribute("readOnly",false);//OJET Migration
            ...
            if ( tagName == 'OJ-INPUT-TEXT' || tagName == "OJ-INPUT-NUMBER" || tagName == "OJ-INPUT-DATE" || tagName == "OJ-INPUT-DATE-TIME") { //OJET Migration
                if (type == 'TEXT') {
                    if (object.getAttribute("READONLY1") && gAction != "ENTERQUERY") {
                        //object.readOnly = true;        //OJET Migration
                        object.setAttribute("readOnly",true);//OJET Migration
                        ...
                        if (getOuterHTML(object).indexOf("validateInputAmount") > 0 || getOuterHTML(object).indexOf("validateInputNumber") > 0 || getOuterHTML(object).indexOf("processAmount") > 0 || getOuterHTML(object).indexOf("processNumber") > 0 || getOuterHTML(object).indexOf("displayRate") > 0) {//added for Amount/Exchange Rate patterns //Changes_prefix_rateType_Fields
                            ...
                            object.setAttribute("readOnly",false); //OJET Migration
            ...
            } else if (tagName == 'OJ-TEXT-AREA' || tagName == 'OJ-INPUT-PASSWORD'  ) {
                if (object.getAttribute("READONLY1") && gAction != "ENTERQUERY") {
                  object.setAttribute("readOnly", true);
                   // object.readOnly = true;
                    //object.className = "TXAro";
                ...
                } else if (tagName == 'SELECT' || tagName == 'OJ-SELECT-SINGLE') { //OJET Migration
...
function ShowXML(xmlFile, xslName, scrnName) {
  ...
  /*if (typeof(screenType) != "undefined" && screenType != "WB") { caching change start
    ...
    if (html == null || mainWin.cacheContent == 'D') {
        ...
        params["largeScreenWidth"] = mainWin.scrWidth;
        ...
        if(typeof(prevSeqNo) != 'undefined' && prevSeqNo!= '' && prevSeqNo!= null) {
            params["prevSeqNo"] = prevSeqNo;
        } else {
            params["prevSeqNo"] = '';
        }
        try {
...
function fnBuildXslLabels() {
    ...
    labels += "@@LBL_SEARCH_RESULT~~"+ mainWin.getItemDesc("LBL_SEARCH_RESULT");  
    ...
    labels += "@@LBL_SIMPLE_SEARCH~~" + mainWin.getItemDesc("LBL_SIMPLE_SEARCH");
    labels += "@@LBL_SEARCH_RESULTS_FOR~~" + mainWin.getItemDesc("LBL_SEARCH_RESULTS_FOR");
    labels += "@@LBL_NAVIGATION_SECTION~~" + mainWin.getItemDesc("LBL_NAVIGATION_SECTION");//Fix for 20817661
    labels += "@@LBL_END_OF_TABLE~~" + mainWin.getItemDesc("LBL_END_OF_TABLE");//Fix for 20817661
	labels += "@@";
    return labels;
}
...
//   customWin.style.zIndex= 2000;
    var customWinData = '<iframe id="ifrSubScreen" title="" src="' + src + '" allowtransparency="true" frameborder="0" scrolling="no"></iframe>';
...
function fnLoadSubScreen(xmlFileName, xslFileName, screenName) {//OJET Migration
...
//12.1 summary performance changes new end
   ...
   // fnBuildMultipleEntryArray();
    ...
    try {
		...
		setTimeout(function(){fnCalcHgtSubScreen();},0); //21232462
   ...
   /* if (funcId  == "EXTAUTHORIZE") {
    ...
    fnEnableElement(document.getElementById('BTN_EXIT_IMG'));
   // setTimeout(function(){ appendData();document.getElementById("BTN_EXIT_IMG").focus();},0);
    //document.getElementById('BTN_EXIT_IMG').focus();
   // parent.mask();
}
...
function handleChkBoxBindVar(bindString, bindObj) {
    if (bindObj.getAttribute("ON")) {
        if (bindObj.value) bindString = bindString + bindObj.getAttribute("ON") + "~";
        else bindString = bindString + bindObj.getAttribute("OFF") + "~";
    } else {
        if (bindObj.value) bindString = bindString + "Y" + "~";
...
function getUIFldBindVal(bindStr, bindFldBlkName, bindFldName, bindFldsStr, singleView) {
    if (getElementsByOjName(bindFldName).length > 1) { //OJET CHANGES
        var bndRowNo = dbIndexArray[bindFldBlkName] - 1;
        if (getElementsByOjName(bindFldName,null,bindFldBlkName,bndRowNo)[bndRowNo].type.toUpperCase() == "OJ-SWITCH") bindFldsStr = handleChkBoxBindVar(bindFldsStr, getElementsByOjName(bindFldName,null,bindFldBlkName,bndRowNo)[bndRowNo]); //OJET CHANGES
		...
		else if (getOuterHTML(document.getElementsByName(bindFldName)[bndRowNo]).indexOf("displayDate") > 0) bindFldsStr = bindFldsStr + getElementsByOjName(bindFldName,null,bindFldBlkName,bndRowNo)[bndRowNo].value + "###DATE"+"|"; //Bug_31534120 //OJET CHANGES
        else bindFldsStr = bindFldsStr + document.getElementsByName(bindFldName)[bndRowNo].value + "|";/* security fixes for WF */
    } else {
        if (typeof (document.getElementsByName(bindFldName)[0]) != "undefined") {
            if (document.getElementsByName(bindFldName)[0].type.toUpperCase() == "OJ-SWITCH") bindFldsStr = handleChkBoxBindVar(bindFldsStr, document.getElementsByName(bindFldName)[0]);
			...
			else if (getOuterHTML(document.getElementsByName(bindFldName)[0]).indexOf("displayDate") > 0) bindFldsStr = bindFldsStr + document.getElementsByName(bindFldName)[0].value + "###DATE"+"|"; //Bug_31534120
            else bindFldsStr = bindFldsStr + document.getElementsByName(bindFldName)[0].value + "|";/* security fixes for WF */
        } else {
            if (getElementsByOjName(bindFldName, parent ).length > 1) { //OJET CHANGES
                var rowNo = dbIndexArray[bindFldBlkName] - 1;
                if (parent.document.getElementsByName(bindFldName)[rowNo].type.toUpperCase() == "OJ-SWITCH") bindFldsStr = handleChkBoxBindVar(bindFldsStr, getElementsByOjName(bindFldName,parent,bindFldBlkName,rowNo)[rowNo]); //OJET CHANGES
				...
				else if (getOuterHTML(parent.document.getElementsByName(bindFldName)[rowNo]).indexOf("displayDate") > 0) bindFldsStr = bindFldsStr + getElementsByOjName(bindFldName,parent,bindFldBlkName,rowNo)[rowNo].value + "###DATE"+"|"; //Bug_31534120 //OJET CHANGES
                else bindFldsStr = bindFldsStr + parent.document.getElementsByName(bindFldName)[rowNo].value + "|";/* security fixes for WF */
            } else {
                if (singleView == "true") {
                    if (parent.document.getElementsByName(bindFldName)[0].type.toUpperCase() == "OJ-SWITCH") bindFldsStr = handleChkBoxBindVar(bindFldsStr, parent.document.getElementsByName(bindFldName)[0]);
					...
					else if (getOuterHTML(parent.document.getElementsByName(bindFldName)[0]).indexOf("displayDate") > 0)  bindFldsStr = bindFldsStr + parent.document.getElementsByName(bindFldName)[0].value + "###DATE"+"|";//Bug_31534120
                    else bindFldsStr = bindFldsStr + parent.document.getElementsByName(bindFldName)[0].value + "|";/* security fixes for WF */
                } else {
                    if (parent.document.getElementsByName(bindFldName)[0].type.toUpperCase() == "OJ-SWITCH") bindFldsStr = handleChkBoxBindVar(bindFldsStr, parent.document.getElementsByName(bindFldName)[0]);
                    //else if (parent.document.getElementsByName(bindFldName)[0].type.toUpperCase() == "OJ-RADIOSET") bindFldsStr = handleRadioBindVar(bindFldsStr, parent.document.getElementsByName(bindFldName));
					else if (getOuterHTML(parent.document.getElementsByName(bindFldName)[0]).indexOf("displayDate") > 0)  bindFldsStr = bindFldsStr + parent.document.getElementsByName(bindFldName)[0].value + "###DATE"+"|";//Bug_31534120
...
isME = isMultipleEntry(blockId); /*12.0.4 UI performance changes starts */
    ...
    */
    ...
    //mask();
    loadSubScreenDIV("ChildWin", "ExtLovDef_Cust.jsp?" + l_Params);
    ...
    for (var loopIndex = 0; loopIndex < elements.length; loopIndex++) {
        ...
        if (tmpElem.type.toUpperCase() != 'HIDDEN' && getCurrentStyle(tmpElem, "visibility").toUpperCase() != "HIDDEN" && tmpElem.className.toUpperCase() != "HIDDEN") {
            ...
            if (gAction == 'EXECUTEQUERY' || gAction == '' || ShowSummary == 'TRUE') {
                ...
                if (nextSibling) {
                    if (nextSibling.tagName) {
                        if (nextSibling.tagName == 'OJ-BUTTON' && nextSibling.innerHTML) {
        ...
        if (type.toUpperCase() == "OJ-BUTTON") {
            if (gAction == 'EXECUTEQUERY' || gAction == '' || ShowSummary == 'TRUE') {
                if (tmpElem.getAttribute('name')!=null &&( tmpElem.getAttribute('name').indexOf("BTN_PREV_") != -1 || tmpElem.getAttribute('name').indexOf("BTN_NEXT_") != -1)) {
                    fnEnableElement(tmpElem);
...
function fnShowSingleViewForME(blockId) {
   ...
   // if (!mainWin.isSessionActive()) return; session expiry change 
    //var tableObject = document.getElementById(blockId);
     var tableObject =getTableObjForBlock(blockId); //OJET Migration
...
function show_editor(elemId, maxLength, title, e) {
    ...
    if (scrElem.tagName.toUpperCase() == 'OJ-BUTTON') {//OJET Migration
        editorSrcElem = scrElem;
    } else if (scrElem.parentNode && scrElem.parentNode.tagName.toUpperCase() == 'OJ-BUTTON') {//Fix for 21490468
        editorSrcElem = scrElem.parentNode;
    } else if (getNextSibling(scrElem) && getNextSibling(scrElem).tagName.toUpperCase() == 'OJ-BUTTON') { //12902575 - F4 not opening Editor in Non-IE browsers
    ...
    var inputBoxName;
    if (scrElem.parentNode.tagName.toUpperCase() == 'OJ-BUTTON') { //Safari Change
        if (getPreviousSibling(scrElem.parentNode).tagName.toUpperCase() == 'OJ-BUTTON') {
        ...
        if (getPreviousSibling(scrElem) && getPreviousSibling(scrElem).tagName.toUpperCase() == 'OJ-BUTTON') {
            inputBoxName = getPreviousSibling(getPreviousSibling(scrElem)).name;
        } else {
            inputBoxName = getPreviousSibling(scrElem) && getPreviousSibling(scrElem).name;
        }
    }
	...
	//SFR#17255467 : Fix for 16474495 OJET Migration
...
//OJET Migration
var readOnlyAttr = document.getElementById(elemId).readonly || document.getElementById(elemId).disabled;
    ...
    if (document.getElementById("tablist")) {
      //  highlighttab(aobject);
       // detectSourceindex(aobject);
        ...
        document.getElementById(cid).parentNode.classList.add("oj-selected");
...
function expandcontent(cid) {
    ...
    if (document.getElementById("tablist")) {
        ...
        if (previoustab != "") {
            ...
            document.getElementById(previoustab).parentNode.classList.remove("oj-selected");
        }
        document.getElementById("TBLPage" + cid).style.display = "block";
        previoustab = cid;
        for (var i = 0; i < tab_arr.length; i++) {
    ...
    if (document.getElementById("TBLPage" + strCurrentTabId).innerHTML == "") {
       ...
       //12.1 Caching Tab load end
        //fnBuildMultipleEntryArray(strCurrentTabId);//OJET Migration
        if (gAction == "NEW") {
...
//    
	...
	//fnChangeLabelToText("TEXTAREA"); //9NT1606_12_2_RETRO_12_0_1_23652286 changes
...
function highlighttab(aobject) {
   ...
   // fnSetTabAttributes(aobject, tabobjlinks); //OJET Migration
}

function fnSetTabAttributes(aobject, tabobjlinks) { //OJET Migration
...
function handleTabKeys(tabObj, e) {
    ...
    if (e.shiftKey && e.keyCode == 9) {
       ...
       if(tabObj.parentNode.parentNode.id == "dboardtablist"){
      ...
      }else if(tabObj.className && tabObj.className == "Htabsel" && document.getElementById("tabCN_"+tabObj.id)){//Customer Accessibility
            var firstElementToFocus = getFirstElementToFocus(document.getElementById("tabCN_"+tabObj.id).getElementsByTagName("IFRAME")[0].contentWindow.document.documentElement);
      }
       else{
        ...
        if (getNextSibling(tabObj.parentNode) != null) {
            ...
            if (tabObj.parentNode.parentNode.id == "dboardtablist" || tabObj.parentNode.parentNode.parentNode.id == "hTab_" + mainWin.currentTab) {
                ...
                highlightSelTab(newObj.id);//Customer accessibility
            }
            preventpropagate(e);
            return false;
        }
...
function fnCheckProductPickup(elem) {//OJET Migration
    try{// for common lov like modulecode , element will be null;so added the try and catch
...
function fnCheckLOVReferredPickup(elem) {//OJET Migration
    try{// for common lov like modulecode , element will be null;so added the try and catch
...
function disp_auto_lov(containerId, blockId, fieldName, FieldLabel, lovId, title, columnHeaders, rednFldInfo, exactFetch, dispField, e) {//Exact Fetch changes
 if((typeof(callformTabArray) != "undefined") && typeof(callformTabArray[strScreenName + "__" +strCurrentTabId]) != "undefined") containerId = callformTabArray[strScreenName + "__" +strCurrentTabId];//12.2 callform change
    ...
    var event = window.event || e;
    //var elem = getEventSourceElement(event);
    var elem = event.target;//OJET Migration
    var customLov = "N";
    ...
    lovSrcElem = event.target;//OJET Migration
    //lov change starts
            ...
            for (var i = 0; i < field_namesArr.length - 1; i++) {
                ...
                if (isME == 'true') {
                    ...
                    if (field_namesArr[i] != "" && document.getElementById(field_namesArr[i])) {
                        ...
                        document.getElementById(field_namesArr[i]).value = "";
                        document.getElementById(field_namesArr[i]).refresh();
                         }
...
//	if (typeof (lovFldObj) == "undefined" || lovFldObj.type == undefined) lovFldObj = lovBlkObj[j].children[0]; //Fix for 20073044
                    ...
                    if (lovFldObj.name == fldName) {
                        lovFldObj.value = fieldValuesArr[i];
                         fireHTMLEvent(lovFldObj, "onpropertychange");//19224295
                        if (lovFldObj.type && lovFldObj.type.toUpperCase() == 'CHECKBOX') {
                        ...
                        } else if (lovFldObj.type && lovFldObj.type.toUpperCase() == 'RADIO') {
...
//    var dtFmt = document.getElementById("dtFmt").value;
    var dtFmt = mainWin.systemDateFormat;
    ...
    //debugger;
    var numberFrmtMask = document.getElementById("NUM_FORMAT_MASK").value;
    var strFormData = 'numberFrmtMask=' +numberFrmtMask+'&amtFmt=' + amtFmt + '&dtFmt=' + dtFmt + '&theme=L'+'&dboardReqd=' + dboardReqd + '&alertHome=' + alertHome +'&scrSaverTime=' + scrSaverTime + '&extalertReqd=' + extalertReqd + '&font=M' ;//external alert changes
    ...
    if (respNode == null || getNodeText(selectSingleNode(resp, "FCUBS_RES_ENV/FCUBS_HEADER/MSGSTAT")) == "FAILURE") {
        ...
        if (RecnodeList.length == 1 || RecnodeList.length == 8) {/*HTML5 Changes Start*/
            if (typeof(Storage) !== "undefined") {
                localStorage.logintheme = 'L';
                localStorage.loginfont = 'M';
            }/*HTML5 Changes End*/
...
function showToolbar(funcid, txnstat, authstat, showExecute) {
    ...
    */
    ...
    if (funcid && funcid != "") {
        ...
        for (i = 0; i < finalarr.length; i++) {
            ...
            if (finalarr[i]) {
                ...
                document.getElementById(finalarr[i]).style.display = "flex"; //OJET Migration
    ...
    if (gNumChildWindows != 0 && gActiveWindow && (gActiveWindow.gAction == "NEW" || gActiveWindow.gAction == "MODIFY")) {
        ...
        document.getElementById("Save").style.display = "flex";//OJET Migration
    ...
    for (var l_Cnt = 0; l_Cnt < finalarr.length; l_Cnt++) {
        if (finalarr[l_Cnt].toUpperCase() == "HOLD") {
            ...
            document.getElementById("Hold").style.display = "flex";//OJET Migration
    ...
    if (applicationName == "FCIS") {
        if (gNumChildWindows != 0 && gActiveWindow && gActiveWindow.gAction == "DELEGATE") {
            if (finalarr) {
                for (var l_Itr = 0; l_Itr < finalarr.length; l_Itr++) {
                    document.getElementById(finalarr[l_Itr]).disabled = true;
                    document.getElementById(finalarr[l_Itr]).style.display = "flex";//OJET Migration
                    //document.getElementById(finalarr[l_Itr]).className = 'BTNiconD';
                }
            }
            enableSave();
...
function fnSetImgSrc(actions_arr) {
    for (var l_idx = 0; l_idx < actions_arr.length; l_idx++) {
        ...
        if (document.getElementById(actions_arr[l_idx]).disabled) {
            ...
            document.getElementById(actions_arr[l_idx]).style.display = "flex";//OJET Migration
            //12012012
            ...
            for (var l_Cnt = 0; l_Cnt < finalarr.length; l_Cnt++) {
                if (finalarr[l_Cnt].toUpperCase() == "NEW") {
					if (mainWin.BranchEoi == 'N' || mainWin.eodFunctions.indexOf(functionId) != -1 || mainWin.branchAvlbltyStatus == 'Y') { //Fix for 20553520//24X7 toolbar
                        ...
                        document.getElementById(finalarr[l_Cnt]).style.display = "flex";//OJET Migration
                        //document.getElementById(finalarr[l_Cnt]).className = 'BTNicon';
                        document.getElementById("EnterQuery").disabled = false;
                        document.getElementById("EnterQuery").style.display = "flex";//OJET Migration
...
function ExtshowToolbar(funcid, txnstat, authstat, showExecute) {
    ...
    if (typeof (funcid) == 'undefined' || funcid == '') {
        ...
        if (gAction == "ENTERQUERY") {
            document.getElementById("ExecuteQuery").disabled = false;
            document.getElementById("ExecuteQuery").style.display = "flex";//OJET Migration
...
function enableDeleteForIPR() {
   ...
   //     if (parent.gTxn == 'IPR' && parent.gStage == 'IPR') {    //12.0.2_single_step_process
	       if (parent.gTxn == 'IPR' && parent.gStage == 'IPR' && mainWin.UserId == dataObj.makerId) {    //12.0.2_single_step_process
            ...
            document.getElementById("Delete").style.display = "flex";//OJET Migration
        }
    }
	    //12.0.2_single_step_process
...
function fnEnableAuth() {
    ...
    document.getElementById("Authorize").style.display = "flex";//OJET Migration
}
...
function fnDoReverse() {
    ...
    document.getElementById("Reverse").style.display = "flex";//OJET Migration
...
function fnEnableGenerateButton() {
    ...
    document.getElementById("Generate").style.display = "flex";//OJET Migration
}
...
function fnEnableConfirmButton() {
    ...
    document.getElementById("Confirm").style.display = "flex";//OJET Migration
}
...
//    if (ccy == "")
//        ccy = mainWin.Lcy;
if ((ccy == "" ||  ccy==null) && dataBoundElem.value != "" ) {
        //dataBoundElem.value="";
...
function ShowCustomerXML(xmlFile, xslName, scrnName) {
    var imagePath = 'Images/Ext' + strTheme.substring(0, strTheme.indexOf('.css'));
    var html;
    strScreenName = scrnName;
   // var xmlDoc = loadXMLFile(xmlFile);
  ...
  /*if (typeof(screenType) != "undefined" && screenType != "WB") { caching change start
    ...
    if (html == null || mainWin.cacheContent == 'D') {
        ...
        params["mediumScreenWidth"] = mainWin.dashBoardWidth;
        if(isSubScreen  == true){
            params["largeScreenWidth"] = mainWin.x -5;
        }
        params["screenHeight"] = mainWin.y;
        if (thirdChar == "S") params["functionId"] = parentFunc;
        else params["functionId"] = functionId;
...
function fnLoadMain(xmlFileName, xslFileName){
  if(srcType == "null"){
      fnLoadCustomer(xmlFileName, xslFileName);
      if(typeof(prevSeqNo)!='undefined' && prevSeqNo != '' && prevSeqNo != null){
		//Fix for 20836111 start
      	accTab = parent.document.getElementById(seqNo).parentNode.id;
      	accTab = accTab.substring(accTab.indexOf("Customer"),accTab.length);
		if(screenTitle != "") //20872284 
      	mainWin.document.getElementById(accTab).children[0].innerHTML = mainWin.document.getElementById(accTab).children[0].innerHTML.replace(mainWin.getItemDesc("LBL_CUSTTAB_ACC_NO"),screenTitle);
		//Fix for 20836111 end
        fnLoadAllSections();
      }
  }else{
      if(srcType == "SUMMARY" || srcType == "DETAIL"){
        fnLoad(xmlFileName, xslFileName);
      }else if(srcType == "DASHBOARD"){
        fnLoadDashboard(xmlFileName,xslFileName)
      }else{
        fnLoad1(xmlFileName, xslFileName);
      }
      setTimeout(function(){fnCalcScreenSize();},0);
  }
}

function highlightSelTab(tabName){
  prevTab = strCurrentTabId;
  strCurrentTabId = tabName;
   highlighttab(document.getElementById(tabName));
  document.getElementById("tabCN_"+prevTab).children[0].style.display = 'none';
  //document.getElementById(prevTab).className = 'Htaball';
 // document.getElementById("tabCN_"+prevTab).children[0].children[0].setAttribute("src", "");
  //document.getElementById("tabCN_"+prevTab).children[0].children[0].removeAttribute("src");
  document.getElementById("tabCN_"+strCurrentTabId).children[0].style.display = 'block';
  //document.getElementById(strCurrentTabId).className = 'Htabsel';
  if(document.getElementById("tabCN_"+strCurrentTabId).children[0].children[0].contentWindow.document.documentElement.getElementsByTagName("BODY")[0].innerHTML == ""){
      document.getElementById("tabCN_"+strCurrentTabId).children[0].children[0].setAttribute("src",document.getElementById("tabCN_"+strCurrentTabId).children[0].children[0].getAttribute("tempsrc"));  
  }else{
    setTimeout(function(){fnCalcScreenSize();},0);
  }
}

function displaySections(sectionIdex,keys,values, event){
srcElem = getEventSourceElement(event);

while(typeof(srcElem.tagName) != "undefined" && srcElem.tagName.toUpperCase() != "TR") {
    srcElem = srcElem.parentNode;
}

  fnBuildParams(keys, values);
  var arrIndex = sectionIdex.split("~");
  if(parent.document.getElementById("ifr_LaunchWin_"+arrIndex[0]).getAttribute("src")=='' || parent.document.getElementById("ifr_LaunchWin_"+arrIndex[0]).getAttribute("src")==null){
    for(i=0;i<arrIndex.length;i++){
        frameElem = parent.document.getElementById("ifr_LaunchWin_"+arrIndex[i]);           
        frameElem.setAttribute("prevSeqNo",seqNo);                  
        frameElem.src = frameElem.getAttribute('tempsrc');
      }
      }
...
function fnBuildParams(keys, values){
  winParam=new Object();
  winParam.keys = new Array();
  winParam.values = new Array();
  winParam.keys = keys.split('~');
  winParam.values = values.split('~');
  /*for(i=1;i<rowElem.cells.length;i++){
    winParam.keys[seqNo]+= rowElem.cells[i].getAttribute("name")+'~';
    winParam.values[seqNo]+=rowElem.cells[i].children[0].children[0].innerText+'~';
  }*/
  
}

function fnGetPrevWinParam(){
    /*parentVal = parent.document.getElementById(prevSeqNo);
    test = "parent.document.getElementById(prevSeqNo)";
    while (parentVal == null) {
        test = "parent."+test
        var fnEval = new Function("return " + test);
        parentVal = fnEval();
    }*/
    var winParam = "";
    if(typeof(prevWin) == "undefined"){
        prevWin = new Object();
    }
    if (prevSeqNo && prevSeqNo != "") {
        for (var i = 0; i < mainWin.arrCustWindows.length; i++) {
            if (mainWin.arrCustWindows[i].id == prevSeqNo) {
                prevWin = mainWin.arrCustWindows[i].children[0].contentWindow;
                break;
            }
        }
    }
    winParam = prevWin.winParam;
    return winParam;
}
...
function hideRows(event){
    document.getElementById('DIVScrContainer').style.height = 'auto';
    parent.document.getElementById(seqNo).style.height = 'auto';
    srcElem = getEventSourceElement(event);    
    while(typeof(srcElem.tagName) != "undefined" && srcElem.tagName.toUpperCase() != "TR") {
        srcElem = srcElem.parentNode;
    }
    selectedRow = srcElem.rowIndex - 1;
    tBodyElem = srcElem.parentNode; 
    for(i=0;i<tBodyElem.rows.length;i++){
        if(i != selectedRow){
            tBodyElem.rows[i].style.display = 'none';
        }
    }
    //tBodyElem.parentNode.parentNode.setAttribute("tempHgt",tBodyElem.parentNode.parentNode.style.height);
    //tBodyElem.parentNode.parentNode.style.height = 'auto';
    //tBodyElem.parentNode.parentNode.style.minHeight = '0px';
    //document.getElementsByName("nFirst__"+tBodyElem.parentNode.id)[0].parentNode.parentNode.style.visibility="hidden";
    document.getElementById("paging_"+tBodyElem.parentNode.parentNode.parentNode.id).style.visibility="hidden";
    //setTimeout(function(){fnCalcScreenSize();},0);
    fnCalcScreenSize();
}


function showCustAccTab(accNo, funcId, sections){
	if(mainWin.gAccArr && typeof(mainWin.gAccArr[accNo])!="undefined" ) { ////20856982 starts
      alert( mainWin.getItemDesc("LBL_ALREADY_RECORD_OPEN"));
      return;
    }////20856982 ends
	if(mainWin.document.getElementById('hTab_DBoardCustomer').getElementsByTagName("A").length > 1){
      mainWin.curPage++;
    }
    /*if(mainWin.document.getElementById('CustomerAccTab')){
         fireHTMLEvent(mainWin.document.getElementById('CustomerAccTab').children[0].children[0], "onclick");
    }*/
    ...
    mainWin.gAccArr[accNo]= mainWin.gCustInfo["PFunctionId"]+'~'+mainWin.gCustInfo["FunctionId"]+'~'+mainWin.gCustInfo["Branch"]+'~'+mainWin.gCustInfo["CustNo"]+'~'+mainWin.gCustInfo["AccntNo"]+'~'+mainWin.gCustInfo["LiabNo"]+'~'+mainWin.gCustInfo["AccDesc"]+'~'+mainWin.gCustInfo["CustSname"]+'~'+mainWin.gCustInfo["CustName"]+'~'+mainWin.gCustInfo["Ccy"]+'~'+mainWin.gCustInfo["Amount"];//Fix for 20805413
    mainWin.gAccArr[accNo] = replaceAll(mainWin.gAccArr[accNo],'undefined','');
    var liElem =  document.createElement("LI");
    var anchorElem = document.createElement("a");
    anchorElem.id = "CustomerAccTab" + mainWin.curPage;
    addEvent(anchorElem, "onclick", "fnToggleDisplay('CustomerAccTab"+ mainWin.curPage +"')");
    anchorElem.tabIndex = '0';
    addEvent(anchorElem, "onkeydown", "handleTabKeysCust(this,event);return handleTabKeys(this,event)");//Customer Accessibility
    addEvent(anchorElem, "objclicked", "true");
    var spanElem = document.createElement("span");
    //spanElem.className  = "DBoardHeadDivSpanSel";
    spanElem.setAttribute("selectedElm",'Y');
    spanElem.id =  accNo;   
    spanElem.innerHTML = mainWin.getItemDesc("LBL_CUSTTAB_ACC_NO")+ "&nbsp;" + accNo +  "<span  slot=\"startIcon\" class=\"oj-ux-ico-close\" onclick=\"closeCurrentTab(this.parentNode.parentNode,event)\"><span class=\"tabClosedGIF\"></span></span>";
    anchorElem.appendChild(spanElem);
    liElem.appendChild(anchorElem);
//    custHeaderTabArray.push({'id': "CustomerAccTab" + mainWin.curPage,'label':mainWin.getItemDesc("LBL_CUSTTAB_ACC_NO")+ " " + accNo ,'methodName':"closeCurrentTab(this.parentNode.parentNode,event)"});
   // mainWin.document.getElementById('CustomerSearch').getElementsByTagName("SPAN")[0].className = 'DBoardHeadDivSpanDeSel';
    mainWin.document.getElementById('hTab_DBoardCustomer').getElementsByTagName("UL")[0].appendChild(liElem);
    anchorElem.focus();
    var dboardDiv = document.createElement("DIV");
    dboardDiv.id = "testwin_cust";
    var dboardDiv1 = document.createElement("DIV");
    dboardDiv1.id = "ContentCustomerAccTab" + mainWin.curPage;
    dboardDiv1.appendChild(dboardDiv);
    mainWin.document.getElementById('hTabCN_Customer').appendChild(dboardDiv1);
    mainWin.document.getElementById('ContentCustomerSearch').style.display = 'none';
    src = "TempForward.jsp?action=SMSStartLogServlet&funcid=" + funcId + "&uiName=&msgType=&actionType=initiate&timestamp=&numeric=&parentArgs=&dashboardArgs=&inTime=&txnBranch=&debugFlag=";
    var accWinData = '<iframe id="ifr_AccountWin" prevSeqNo='+seqNo+' class="frames" src=' + src + ' allowtransparency="true" frameborder="0" scrolling="no" title="" style="z-index:10"></iframe>';
    mainWin.document.getElementById('ContentCustomerAccTab'+ mainWin.curPage).children[0].insertAdjacentHTML("beforeEnd", accWinData);
    if(mainWin.document.getElementById("custLandingTabs")){
        mainWin.document.getElementById("custLandingTabs").refresh();
    }
     
}

function fnLoadAllSections(){
    secArr = mainWin.document.getElementById(seqNo).children[0].contentWindow.document.getElementById("ResTree").getElementsByTagName("IFRAME");
   /* var I = 0;
    ...
    if (I < secArr.length) {
      if (bPrevIframeLoaded) {
        frameElem = secArr[I];
        frameElem.setAttribute("prevSeqNo",prevSeqNo);
         bPrevIframeLoaded = false;
          if(frameElem.attachEvent)
            frameElem.attachEvent('onload', function () { bPrevIframeLoaded = true});
         else
            frameElem.onload = function () { bPrevIframeLoaded = true};
         frameElem.src = frameElem.getAttribute('tempsrc');
         //}
         I++;
      }
    ...
    contentId = mainWin.document.getElementById(seqNo).parentNode.id;
    anchorId = contentId.substring(7,contentId.length);
    mainWin.document.getElementById(anchorId).focus();*/
    //Customer Accessibilty end
}
function resetSections(event){
    var secArr = mainWin.document.getElementById("ifr_CustomerWin").contentWindow.document.getElementById("ResTree").getElementsByTagName("IFRAME");    
     for(i=0;i<secArr.length;i++){
         frameElem = secArr[i];
         if(frameElem.getAttribute("prevSeqNo")!=null && frameElem.getAttribute("prevSeqNo")!='null' && frameElem.getAttribute("prevSeqNo")!=''){
                 frameElem.setAttribute("tempSrc", frameElem.getAttribute("src"));
                 frameElem.setAttribute("src","");
                 frameElem.removeAttribute("src");
                 frameElem.style.height = '0px';
                 sectionName = frameElem.id.replace('ifr_LaunchWin_','');//Fix for 20805413
                 frameElem.parentNode.id = "testwin"+sectionName;
                 frameElem.setAttribute("prevSeqNo", '');
                 if(frameElem.getAttribute("TYPE")=='TABS'){
                    for(j=0;j<frameElem.contentWindow.document.getElementsByTagName('IFRAME').length;j++){
                        if(frameElem.contentWindow.document.getElementsByTagName('IFRAME')[j].parentNode.id !='testwin'){
				frameElem.contentWindow.document.getElementsByTagName('IFRAME')[j].setAttribute("prevSeqNo", '');
                            mainWin.fnExit_Cust(frameElem.contentWindow.document.getElementsByTagName('IFRAME')[j].parentNode);
                        }
                    }
                 }
                 mainWin.fnExit_Cust(frameElem.parentNode);
                 
        }
     }
     fnResetMETableHeight();
     if(mainWin.document.getElementById('CustomerAccTab')){
        //fireHTMLEvent(mainWin.document.getElementById('CustomerAccTab').children[0].children[0], "onclick", event);
         mainWin.closeCurrentTab(mainWin.document.getElementById('CustomerAccTab'), event);
     }
...
function fnResetMETableHeight(){
    tableObjs = multipleEntryIDs;
    for (var i = 0;i < tableObjs.length;i++) {
        var objMultipleEntryTable = getTableObjForBlock(tableObjs[i]);
        //document.getElementsByName("nFirst__"+objMultipleEntryTable.id)[0].parentNode.parentNode.style.visibility="visible";//Fix for 20856882
	 if(objMultipleEntryTable.parentNode.getAttribute("tempHgt")!= null && objMultipleEntryTable.parentNode.getAttribute("tempHgt")!=''){
        	objMultipleEntryTable.parentNode.style.minHeight = '11em';
        	objMultipleEntryTable.parentNode.style.height = objMultipleEntryTable.parentNode.getAttribute("tempHgt");
        	objMultipleEntryTable.parentNode.setAttribute("tempHgt",'');
        	objMultipleEntryTable.parentNode.removeAttribute("tempHgt");
	}
    }
}

function fnSetGlobalParams(brn, custNo, accntNo, liabNo, accDesc, sname, custname, ccy, amount) {
    
    var obj = mainWin.document.getElementById("hTabCN_Customer");
    for (var i = 0;i < obj.children.length;i++) {
        if (obj.children[i].style.display != 'hidden') {
            funcId = obj.children[i].children[0].children[0].contentWindow.functionId;
            break;
        }
    }
    if (typeof (funcId) != 'undefined' && funcId != '') {
        mainWin.gCustInfo["PFunctionId"] = funcId;
    }
    if (typeof (functionId) != 'undefined' && functionId != '') {
        mainWin.gCustInfo["FunctionId"] = functionId;
    }
    if (typeof (brn) != 'undefined' && brn != '') {
        mainWin.gCustInfo["Branch"] = brn;
    }
    if (typeof (custNo) != 'undefined' && custNo != '') {
        mainWin.gCustInfo["CustNo"] = custNo;
    }
    if (typeof (accntNo) != 'undefined' && accntNo != '') {
        mainWin.gCustInfo["AccntNo"] = accntNo;
    }
    if (typeof (liabNo) != 'undefined' && liabNo != '') {
        mainWin.gCustInfo["LiabNo"] = liabNo;
    }
    if (typeof (accDesc) != 'undefined' && accDesc != '') {
...
function getActiveTabWindow(){
    var activeTabWindow = '';   
    for(i=0;i<document.getElementById("tabContent").children.length;i++){
        if(document.getElementById("tabContent").children[i].children[0].style.display=='block'){
            activeTabWindow = document.getElementById("tabContent").children[i].children[0].children[0].contentWindow;
	     break;
        }
    }
...
//fix for 20804978 start
function fnDisableTAB(){
    tabId = parent.document.getElementById(seqNo).parentNode.id.replace("tabCN_","");
    parent.document.getElementById(tabId).children[0].style.backgroundColor = "#f5f5f0";//Fix for 21422988
    parent.document.getElementById(tabId).children[0].style.color = "#000000";//Fix for 21422988
}

function fnEnableTAB(){
    tabId = parent.document.getElementById(seqNo).parentNode.id.replace("tabCN_","");
    highlighttab(parent.document.getElementById(tabId));
	//parent.document.getElementById(tabId).children[0].style.backgroundColor = "#c8ddf8";
    //parent.document.getElementById(tabId).children[0].style.color = "#000";
}
//fix for 20804978 end

//Fix for 20805413 start
function setLang(iFrameElem){
 addEvent(iFrameElem.contentWindow.document.documentElement, "lang", langISOMap);
}
//Fix for 20805413 end

function ojExpandHandler(event) {
    setTimeout(function() {
        fnCalcScreenSize();
        if (!fnEventsHandler('fnPostSection_Expand', event.target.parentNode)) {
            return false; 
        }
    },0);
}

function ojCollapseHandler(event) {
    setTimeout(function() {
        fnCalcScreenSize();
        if (!fnEventsHandler('fnPostSection_Collapse', event.target.parentNode)) {
            return false;  
        }
    },0);
}