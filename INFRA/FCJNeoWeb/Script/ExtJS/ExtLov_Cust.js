...
//Index based search changes end
    ...
    try {
        ...
        if (objHTTP.status == 200) {
                ...
                if (LOVResponseDOM) {
                   // appendDebug(LOVResponseDOM); //Logging changes
                    ...
                    if (intTotPgs == 0) {
                       ...
                       // if (intTotPgs != "10") setInnerText(document.getElementById("ALERTTBL").tHead.rows[0].cells[1].children[0], mainWin.getItemDesc("LBL_OK"));
                    }
                    ...
                    //document.getElementById("ALERTTBL").tHead.rows[0].cells[0].children[0].className = "LovAlert"; //Data Uri change
            ...
            //fnSyncLovTableWidth();//static Header change //OJET Migration
            
           
        }
    } catch (e) {
       // setInnerText(document.getElementById("ALERTTBL").tHead.rows[0].cells[1].children[0], mainWin.getItemDesc("LBL_ERROR"));
        //document.getElementById("ALERTTBL").tHead.rows[0].cells[0].children[0].className = "LovErrorAlert"; //Data Uri Changes
    }
}
...
function lovAccessKeys(e) {
    ...
    if (evnt.keyCode == 27) {
        ...
        if (srcElement.tagName.toUpperCase() == 'oj-input-text') {
            return true;
        } else {
            fnDisableBrowserKey(evnt);
            preventpropagate(evnt);
...
function doNavigate(type) {
    ...
    case gcNAV_PREVIOUS:
        ...
        document.getElementById("goto").value = Number(getInnerText(document.getElementById("CurPage")));
        getLovResults(source, functionId, blockId, fldName, lovId);
        break;
    case gcNAV_NEXT:
        setInnerText(document.getElementById("CurPage"), Number(getInnerText(document.getElementById("CurPage"))) + 1);
...
function enableAllButtons() {//OJET Migration
...
function disableAllButtons() {//OJET Migration
...
function enableButtons() {//OJET Migration
    disableAllButtons();
...
function buildReductionCriteria() {
    ...
    for (var flds = 0; flds < rednFlds.length; flds++) { //OJET Migration
...
//        }
        if (rednFlds[flds].name == "" || rednFlds[flds].type == "hidden") {
            /*Fix for 16861079*/
            if (rednFlds[flds].tagName == 'OJ-INPUT-DATE') {
...
/* security fixes for WF starts */
             ...
             //   rednString += getPreviousSibling(getPreviousSibling(rednFlds[flds])).name + "!" + tmp_date + "|";//March22 for security changes
                ...
                if (rednFlds[flds].value ) { //OJET Migration
...
/*if(mainWin.document.getElementById("nav").children[0].children[0].children.length>0){//BUG 16374733,21623940 
    ...
    } else if (functionId == "COMMON" && mainWin.document.getElementById('BLK_ENTITY_DETAILS__ENTITY_ID')) {
    ...
    //SMSStandalone12.3 Changes Ends
		...
		//mainWin.document.getElementById("nav").children[0].children[0].focus();//HTML5 Changes 6/OCT/2016
    } else {
...
function fnHandleLovRslt(e) {
    var evnt = window.event || e;
    var srcElement = getEventSourceElement(evnt);
    var l_TableObj = document.getElementById("TableLov").tBodies[0].rows;
    if (evnt.shiftKey && evnt.keyCode == 9) {
...
function parseLovOnLoad() {
    ...
    try {
       ...
       // setInnerText(document.getElementById("ALERTTBL").tHead.rows[0].cells[1].children[0], mainWin.getItemDesc("LBL_ERROR"));
     //   document.getElementById("ALERTTBL").tHead.rows[0].cells[0].children[0].className = "LovErrorAlert"; //Data Uri change
    }
    /*var dbtime = 0;
...
function displayMiniCharInfo(){
    ...
    if(isIndexFld == "Y"){    
       var curfieldLabel = document.getElementById(cnt+1).parentNode.getElementsByTagName("OJ-LABEL")[0].innerHTML;
