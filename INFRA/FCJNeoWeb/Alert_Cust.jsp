...
<%}%>--%><!--HTML5 Changes End -->
        ...
        <script type="text/javascript" src="Script/OJET/main_alertCust.js"></script>
    ...
    <body  oncontextmenu="return false;" onkeyup="return disableCommonKeys(event);" onhelp="return false;" onclick="setParentWinActive(event);"><!-- static header change -->
    <div id="DIVif1" >
                    ...
                    <div slot="footer" id="alertFooter">
                        ...
                        if(AlertType != null) {
                         if ("C".equalsIgnoreCase(AlertType) || "O".equalsIgnoreCase(AlertType)) {
                                    ...
                                    //FCUBS11.1 CROSSBROWSER changes starts here  //fc11.1wb changes
                                      if(screenType.equalsIgnoreCase("WB") && ("N".equalsIgnoreCase(autoOvdFlag)) && ( "N".equalsIgnoreCase(ssoReq)) ) {//25402086   SSOREQ added
                                        if( "N".equalsIgnoreCase(multiAuthRequired)) {
										 ...
										 <oj-button chroming="borderless"  label=<%=StringEscapeUtils.escapeHTML(acceptLabel)%> id="BTN_ACCEPT"  onclick="callCloseAlert(event)" onkeydown="return fnHandleAlertBtn(event)" name=<%=StringEscapeUtils.escapeHTML(acceptLabel)%> title=<%=StringEscapeUtils.escapeHTML(acceptLabel)%> /></oj-button><%
                                        }else {
                                         %><oj-button chroming="solid"   class="btn-warning"  label=<%=StringEscapeUtils.escapeHTML(acceptLabel)%> title=<%=StringEscapeUtils.escapeHTML(acceptLabel)%> id="BTN_ACCEPT"  onclick="callCloseAlert(event)" onkeydown="return fnHandleAlertBtn(event)" name=<%=StringEscapeUtils.escapeHTML(acceptLabel)%> /></oj-button><%
                                        }
                            ...
                            %><oj-button chroming="<%=StringEscapeUtils.escapeHTML(okBtnChroming)%>" label=<%=StringEscapeUtils.escapeHTML(okLabel)%> class="<%=StringEscapeUtils.escapeHTML(okBtnClass)%>" title=<%=StringEscapeUtils.escapeHTML(okLabel)%> id="BTN_OK" onclick="callCloseAlert(event)"  onkeydown="return fnHandleAlertBtn(event)" name= <%=StringEscapeUtils.escapeHTML(okLabel)%> /></oj-button><%
                                }
