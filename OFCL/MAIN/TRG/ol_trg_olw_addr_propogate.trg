CREATE OR REPLACE TRIGGER "TRG_DSW_ADDR_PROPOGATE"         
/*----------------------------------------------------------------------------------------------------
**
** File Name   : ol_trg_olw_addr_propogate.trg
** Module      : 
**
...
------------------------------------------------------------------------------------------------------
*/
AFTER INSERT OR UPDATE ON OLTM_CIF_ADDL_SWIFT_ADDR
FOR EACH ROW
DECLARE
...
BEGIN
	debug.pr_debug('OL','Within ol_trg_olw_addr_propogate for ' ||
	:new.customer_no || '~' || :new.msg_type || '~' || :new.cover_reqd || '~' ||
	:new.currency_code || '~' || :new.module || '~' || :new.product);
...
WHEN OTHERS
THEN
	debug.pr_debug('**','in when others of ol_trg_olw_addr_propogate with SQLERRM=>'|| SQLERRM);
	debug.pr_debug('OL','in when others of ol_trg_olw_addr_propogate with SQLERRM=>'|| SQLERRM);
END;
/