CREATE OR REPLACE TRIGGER "TRG_MSDSWFLD_PROPOGATE"
/*----------------------------------------------------------------------------------------------------
**
** File Name : ol_trg_msdswfld_propogate.TRG
...
10-may-2016 ofcl12.2 changes sipks to sypks						
***************************************CHANGE HISTORY************************************************/

AFTER INSERT OR UPDATE OF AUTH_STAT ON OLTM_MSGTYP_FIELD_CODES
FOR EACH ROW
DECLARE
...
BEGIN
	debug.pr_debug('OL','Within ol_trg_msdswfld_propogate for ' || :new.msg_type || '~'|| :new.field_no || '~'|| :new.code || '~'|| :new.cover);
...
WHEN OTHERS
THEN
	debug.pr_debug('**','in when others of ol_trg_msdswfld_propogate with SQLERRM=>'|| SQLERRM);
	debug.pr_debug('OL','in when others of ol_trg_msdswfld_propogate with SQLERRM=>'|| SQLERRM);
END;
/