create or replace trigger oltr_lcl_holiday
after UPDATE OF HOLIDAY_LIST ON STTM_LCL_HOLIDAY
/*------------------------------------------------------------------------------------------
		...
		WHILE l_new_val IS NOT NULL LOOP
			if l_new_val <>  l_old_val then
				...
				if l_date > global.application_date then		--so this adhoc holiday date change happens only for future dates
					...
					and CHANGE_DATE = l_date
					and holiday_type='LCL'
					and holiday_key =:new.branch_code ;
...
when others then
    RAISE_APPLICATION_ERROR(-20001,
                            'bombed in trigger oltr_lcl_holiday-->' || SQLERRM);
end oltr_lcl_holiday; 
/