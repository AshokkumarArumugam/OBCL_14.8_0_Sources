CREATE OR REPLACE TRIGGER ol_trg_msdcsgrp_propogate
/*----------------------------------------------------------------------------------------------------
**
** File Name : ol_trg_msdcsgrp_propogate.trg
...
AFTER INSERT OR UPDATE OF AUTH_STAT ON OLTM_MSG_CUST_MASTER
FOR EACH ROW
DECLARE
...
BEGIN
	debug.pr_debug('OL','Within ol_trg_msdcsgrp_propogate for ' || :new.msg_group);
...
WHEN OTHERS
THEN
	debug.pr_debug('**','in when others of ol_trg_msdcsgrp_propogate with SQLERRM=>'|| SQLERRM);
	debug.pr_debug('OL','in when others of ol_trg_msdcsgrp_propogate with SQLERRM=>'|| SQLERRM);
END;
/