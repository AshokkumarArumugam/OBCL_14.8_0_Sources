CREATE OR REPLACE TRIGGER  ol_oltr_stop
/*----------------------------------------------------------------------------------------------------
**
** File Name    : ol_oltr_stop.TRG
**
** Module       : REPORTS
**
...
*/
after delete on OLTB_SERVER_REPORT
for each row



BEGIN
	--dbms_alert.signal('rptb_start_stop_alert', 'STOP' );--AutonomousDB_Changes
