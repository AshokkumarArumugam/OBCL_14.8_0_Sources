CREATE OR REPLACE PACKAGE  olpks_oldprliq_main AS
  /*-----------------------------------------------------------------------------------------------------
  **
  ** File Name  : olpks_oldprliq_main.spc
  **
...
TYPE ty_tb_v_oltbs_payrecv_paid IS TABLE OF oltb_payrecv_paid%ROWTYPE INDEX BY BINARY_INTEGER;

TYPE ty_oldprliq IS RECORD (
     v_oltbs_contract     oltb_contract%ROWTYPE,
     v_oltbs_payrecv_paid    ty_tb_v_oltbs_payrecv_paid,
     v_ontract_event_log__payrec     oltb_contract_event_log%ROWTYPE,
     v_payrecv_paid__single     oltb_payrecv_paid%ROWTYPE,
...
FUNCTION Fn_Main       (p_Source            IN     VARCHAR2,
                        ...
                        p_Multi_Trip_Id     IN  VARCHAR2,
                        p_Request_No        IN     VARCHAR2,
                        p_oldprliq          IN OUT  olpks_oldprliq_Main.Ty_oldprliq,
                        p_Status            IN OUT VARCHAR2 ,
                        p_Err_Code          IN OUT VARCHAR2,
                        p_Err_Params        IN OUT VARCHAR2)
RETURN BOOLEAN;
...
p_Wrk_oldprliq IN  olpks_oldprliq_Main.Ty_oldprliq,
p_Err_Code          IN OUT VARCHAR2,
p_Err_Params        IN OUT VARCHAR2)
RETURN BOOLEAN;
...
p_Err_Params        IN OUT VARCHAR2)
RETURN BOOLEAN;
END olpks_oldprliq_main;
/
CREATE OR REPLACE SYNONYM olpkss_oldprliq_main FOR olpks_oldprliq_main
/