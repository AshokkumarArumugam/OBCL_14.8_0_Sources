...
function fnCreateBody_LBSPRAMJ() {
    var msgxml_lbspramj = "<FCUBS_BODY>";
    msgxml_lbspramj += '    <FLD>'; 
	msgxml_lbspramj += '      <FN PARENT="" RELATION_TYPE="1" TYPE="BLK_PRAMJ">CONTRACT_REF_NO~VALUE_DATE~ENTRY_SEQ_NO~EVENT~EVENT_SEQ_NO~PROCESS_STATUS~BRANCH~PROCESS_SEQ_NO~ERR_DESC~SUM_REC_ID</FN>'; 
	msgxml_lbspramj += '    </FLD>'; 
    msgxml_lbspramj += '<REC RECID="1" TYPE="BLK_PRAMJ"><FV/></REC></FCUBS_BODY>';
    reqDom=loadXMLDoc(msgxml_lbspramj);
    var blkCdtSecNd = "";
    var blkCdtSecNd = reqDom.createCDATASection('~~~~~~~~~'+bulkPkValues);
    selectSingleNode(selectSingleNode(reqDom,"//REC[@RECID='1'][@TYPE='BLK_PRAMJ']"),"FV").appendChild(blkCdtSecNd);
...
function fn_Process(){
		...
		setNodeText(selectSingleNode(exlRequestDOM,"FCUBS_REQ_ENV/FCUBS_HEADER/FUNCTIONID"), "LBSPRAMJ");
		...
		var bodyReq = fnCreateBody_LBSPRAMJ();
