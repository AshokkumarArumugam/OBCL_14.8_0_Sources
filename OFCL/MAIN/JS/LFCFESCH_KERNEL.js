...
function fnPreLoad_CVS_PAY_SCH_KERNEL(screenArgs) {
...
function fnSchedulesUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        ...
        if (cells.length > 1) {
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'Component'.toUpperCase()) || (cells[1].toUpperCase() != 'Start Date'.toUpperCase()) || (cells[2].toUpperCase() != 'Number of Schedules'.toUpperCase()) || (cells[3].toUpperCase() != 'Frequency'.toUpperCase()) || (cells[4].toUpperCase() != 'Unit'.toUpperCase()) || (cells[5].toUpperCase() != 'Amount'.toUpperCase())) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnBackEndCall() {
    ...
    if (dbDataDOM.documentElement.nodeName != dataSrcLocationArray[0]) {
        ...
        var removeNode = selectNodes(orgDom, getXPathQuery("BLK_OLTBS_CONTRACT_SCHEDULES_FESCH"));
...
function fnSchedulesUpload(e) {
    ...
    blockId = "BLK_OLTBS_CONTRACT_SCHEDULES_MULTI";//block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_OLTBS_CONTRACT_SCHEDULES_MULTI__BTN_SCHUPLD");
    if (gAction == 'NEW' || gAction == 'MODIFY' || gAction == 'SUBSYSPKP_NEW' || gAction == 'SUBSYSPKP_MODIFY') {
...
function fnPostLoad_CVS_FEESCH_KERNEL(screenArgs) {
    var onceAuth = document.getElementById("BLK_OLTBS_CONTRACT_SCHEDULES_FESCH__ONCEAUTH_FLAG").value;
    if ((selectNodes(dbDataDOM, "//BLK_LFTBS_CONTRACT_FEE_MULTI").length > 0) && (onceAuth == 'N') && ((gAction == 'NEW') || (gAction == 'MODIFY') || (gAction == 'SUBSYSPKP_NEW') || (gAction == 'SUBSYSPKP_MODIFY'))) {
        gDisableBrowseButton = true;
    }
    else {
        gDisableBrowseButton = false;
    ...
    if (((gAction == 'NEW') || (gAction == 'MODIFY') || (gAction == 'SUBSYSPKP_NEW') || (gAction == 'SUBSYSPKP_MODIFY')) && gDisableBrowseButton) {
        fnEnableUploadBtnElement(document.getElementById("BLK_OLTBS_CONTRACT_SCHEDULES_MULTI__BTN_SCHUPLD"));
    }
    else {
        fnDisableUploadBtnElement(document.getElementById("BLK_OLTBS_CONTRACT_SCHEDULES_MULTI__BTN_SCHUPLD"));
    }
    return true;
}
