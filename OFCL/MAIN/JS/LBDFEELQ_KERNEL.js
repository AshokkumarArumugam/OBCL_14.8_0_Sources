...
**  Written by         : 
**  Date of creation   : 
**  File Name          : LBDFEELQ_KERNEL.js
**  Purpose            : 
**  Called From        : 
**  
**  CHANGE LOG
**  Last Modified By   : Srinivasulu Ch
**  Last modified on   : 16-Jul-2019
**  Change Desc        : Added to display payment versions when click on arrow button
**  Search String      : Bug#29588590 
**  CHANGE LOG
**  Last Modified By   : Revathi A
**  Last modified on   : 15-APR-2020
**  Change Desc        : Added condition On query to set the focus to main tab
**  Search String      : Bug#31127978
...
**  Last modified on   : 7-JUL-2022
**  Reason             : To Default the application date for value date and maturity date
**  Search String      : BUG#34350116
****************************************************************************************************************************/
/*var fcjResponseDOM;
var fcjRequestDOM;*/
var gPrevAction;
...
function fnPostNew_KERNEL() {
	...
	//document.getElementById("BLK_OLTBS_CONTRACT_PAY__LIMITDT").value = mainWin.AppDate;	
	//BUG#34350116 starts
	fnEnableElement(document.getElementById("BLK_OLTBS_CONTRACT_PAY__VALUE_DATE"));
	document.getElementById("BLK_OLTBS_CONTRACT_PAY__VALUE_DATE").value = mainWin.AppDate;
	fireHTMLEvent(document.getElementById("BLK_OLTBS_CONTRACT_PAY__VALUE_DATE"),"onpropertychange");
	fnEnableElement(document.getElementById("BLK_OLTBS_CONTRACT_PAY__LIMIT_DATE"));
	document.getElementById("BLK_OLTBS_CONTRACT_PAY__LIMIT_DATE").value = mainWin.AppDate;
	fireHTMLEvent(document.getElementById("BLK_OLTBS_CONTRACT_PAY__LIMIT_DATE"),"onpropertychange");
	//BUG#34350116 ends
	//OBCL_14.3_SUPPORT_BUG#31400838 CHANGES STARTS
...
function fnPostExecuteQuery_KERNEL() {
	...
	//Bug 27067718 - LBDFEELQ: SCREEN ISSUES  start
	var  l_Auth_Stat = getNodeText(selectSingleNode(fnGetDataXMLFromFCJXML(fcjResponseDOM, 1),"//BLK_OLTBS_CONTRACT_EVENT_LOG/AUTHSTAT"));
	var  l_Cont_Stat = getNodeText(selectSingleNode(fnGetDataXMLFromFCJXML(fcjResponseDOM, 1),"//BLK_OLTBS_CONTRACT_EVENT_LOG/CONTSTAT"));
	if (l_Auth_Stat == 'A'){
		 DisableToolbar_buttons('Authorize'); 
		 DisableToolbar_buttons('Delete');
	}
	if (l_Cont_Stat == 'A' && l_Auth_Stat == 'A' ){
		EnableToolbar_buttons('Reverse'); 
	}
	else{
		DisableToolbar_buttons('Reverse');
	}
	//Bug 27067718 - LBDFEELQ: SCREEN ISSUES end
	return true; 
}
...
function fnOnClick_BTN_NEXT_VER(){
    ...
    document.getElementById("BLK_OLTBS_CONTRACT_PAY__TXTVERSON").value ='Y'; //Bug#29588590
	if(verNo == VERNOCount){
...
function fnRatioUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        var cells = rows[i].split(",");
        if (cells.length > 1) {
            if (i == 0) {
                if (cells[0].toUpperCase() != '[Adhoc Fee Payment Upload]'.toUpperCase()) {
                    fnResetUpload();//reset upload parameters
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'Participant'.toUpperCase()) || (cells[1].toUpperCase() != 'Participant Name'.toUpperCase()) || (cells[2].toUpperCase() != 'Component Ratio'.toUpperCase()) || (cells[3].toUpperCase() != 'Fee Amount'.toUpperCase()) || (cells[4].toUpperCase() != 'Fee BPS Rate'.toUpperCase()) || (cells[5].toUpperCase() != 'Fee BPS Rate Applicable'.toUpperCase())) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnPartRatioUpload(e) {
    gDisableBrowseButton = false;
    blockId = "BLK_LBTBS_PARTICIPANT_RATIO";//block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_LBTBS_PARTICIPANT_RATIO__BTN_RATIO_UPLD");
        ...
        if (regex.test(fileUploadBtn.value.toLowerCase())) {
            if (typeof (FileReader) != undefined) {
                ...
                reader.onload = function (e) {
                    ...
                    var ratioUploadVal = fnRatioUploadValidation(rows);//header validations, system will validate first two rows
                    if (ratioUploadVal) {
                        fnUpdateBlockLength();
                        if (totRow > 0) {
                            csvRows = rows;
                            ...
                            showErrorAlerts("LB-UPD-016", "O", "Participant Ratio");
                            return false;
                        }
                        else {
...
function fnPostFocus_KERNEL() {
    ...
    if (((gAction == 'NEW') || (gAction == 'MODIFY')) && gDisableBrowseButton) {
        fnEnableUploadBtnElement(document.getElementById("BLK_LBTBS_PARTICIPANT_RATIO__BTN_RATIO_UPLD")); 
    }
    else {
        fnDisableUploadBtnElement(document.getElementById("BLK_LBTBS_PARTICIPANT_RATIO__BTN_RATIO_UPLD"));
    }
    return true;
}
/* OBCL_CSV_File_Upload_Changes :: Ends */