...
function fnPostLoad_CVS_AUTH_KERNEL(screenArgs) {
    ...
    if(!fnProcessAuthOnLoad(screenArgs))
        return false;
	   fnEnableElement(document.getElementById("BLK_MSG_PREVIEW_MAS__BTN_AUTH"));
	  	return true;
}
...
function fnPostExecuteQuery_KERNEL() {
	debugger;
	   ...
	   fnEnableElement(document.getElementById("BLK_MSG_PREVIEW_MAS_BTN_AUTH"));	  	 
 return true;
}
...
function Fn_OnAuth(){
    ...
    if (!fnOnlineAuthorize(subScreen)) {		
		...
		if (l_msgStat == 'SUCCESS') {		  
			fnDisableElement(document.getElementById("BLK_MSG_PREVIEW_MAS__BTN_AUTH"));        
			disableForm();
		}	
