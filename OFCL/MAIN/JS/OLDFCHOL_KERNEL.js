...
/* Bug#34958820 changes start 
var gCSSReadOnly	= "TXTro numeric";
var gCSSHoliday		= "TXTro numeric TxtHD"; */

var gCSSReadOnly = []; 
var gCSSHoliday ="calendarHoliday"; 
var gModified = "viewchanges-modified"; 
...
{
	//selectSingleNode(document, '//fieldset[@block="BLK_CITY_HOLIDAY"]').style.display = 'none';
	document.getElementById("dataContainer_BLK_CITY_HOLIDAY").style.display="none";
	document.getElementById("dataContainer_BLK_CITY_HOLIDAY").style.height = '0px';
	document.getElementById("dataContainer_BLK_CITY_HOLIDAY").style.overflow = 'hidden';
	document.getElementById("BLK_CITY_HOLIDAY").firstChild.nextSibling.style.display = 'none';
	document.getElementById("dataContainer_BLK_CITY_HOLIDAY").firstChild.firstChild.style.display = 'none';
	//document.getElementById("dataContainer_BLK_CITY_HOLIDAY").visibility=false;
	//gYearRef = getElementsByOjName("YEAR")[0];
	gYearRef=document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value;//FCUBS_12.1.0_21066833
...
function fnPostNew_KERNEL()  
{
        ...
        //getElementsByOjName("FIN_CENTRE")[0].value = mainWin.CurrentCity;
      	//getElementsByOjName("YEAR")[0].value = parent.AppDate.substr(0,4); -- Commented as part of fix for 9NT1587_FC_12.0.2_ITR1 Bug# 16995458
		// Fix for 9NT1587_FC_12.0.2_ITR1 Bug# 16995458 Starts
		document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value = parent.AppDate.substr(0,4);  
        fireHTMLEvent(document.getElementById('BLK_CITY_HOLIDAY_MASTER__YEAR'), "onpropertychange");
...
function fnPostUnlock_KERNEL() {
	fnDisableElement(getElementsByOjName('BTN_REFRESH')[0]);
	//getElementsByOjName("FIN_CENTRE")[0].readOnly = true;
	getElementsByOjName("YEAR")[0].readOnly = true;
	
	//getElementsByOjName("FIN_CENTRE")[0].disabled = true;
    //getElementsByOjName("YEAR")[0].disabled = true;
    displayCalendar();
	return true;
}
...
function fnPreSave_KERNEL() {
    ...
    setChildKeyValues("OLTMS_FINCENTRE_HOL_MASTER","FIN_CENTRE","FIN_CENTRE");
    setChildKeyValues("OLTMS_FINCENTRE_HOL_MASTER","YEAR","YEAR");
    var yearVal        = document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value;
	var gFIN_CENTRERef = document.getElementById("BLK_CITY_HOLIDAY_MASTER__FIN_CENTRE").value;
	gyearValue    = yearVal;
    gYearRef      = yearVal;
	gFIN_CENTREReference = gFIN_CENTRERef;
    displayCalendar();
    return true;
}
...
function displayCalendarOJET(){
   //var curYear2  = getElementsByOjName("YEAR")[0].value; FCUBS_12.1.0_21066833 commented
    var curYear2  = document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value; //FCUBS_12.1.0_21066833 added
    ...
    if(inYear != "")
    {
            ...
            {
		  isRefresh = true;
		  deleteAllRows("BLK_CITY_HOLIDAY");//FCUBS11.2_Cross_Browser#1
				...
				//currObj = eval(getElementsByOjName(strCell)[0]); //FCUBS11.2_Cross_Browser#1 //FCUBS_12.1.0_21066833 commented
				currObj = eval(document.getElementById("BLK_CITY_HOLIDAY_MASTER__" + monthName + "_" + row + "_" + col +  "I"));  //  FCUBS_12.1.0_21066833 added
                        currObj.value = "";
                        currObj.className = gCSSReadOnly;
                        currObj.value = dayCnt;
                        currObj.tabIndex = tabIndex;
                    ...
                    var currRow = addNewRow("BLK_CITY_HOLIDAY");
                   // currRow.cells[1].getElementsByTagName("oj-input-text")[0].value = getElementsByOjName("FIN_CENTRE")[0].value; //FCUBS_12.1.0_21066833 commented
                   currRow.cells[1].getElementsByTagName("oj-input-text")[0].value = document.getElementById("BLK_CITY_HOLIDAY_MASTER__FIN_CENTRE").value; // FCUBS_12.1.0_21066833 added
					//currRow.cells[2].getElementsByTagName("oj-input-text")[0].value = gYearRef.value;
                    //currRow.cells[2].getElementsByTagName("oj-input-text")[0].value = getElementsByOjName("YEAR")[0].value; //FCUBS_12.1.0_21066833 commented
					currRow.cells[2].getElementsByTagName("oj-input-text")[0].value = document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value;  // FCUBS_12.1.0_21066833 added
				...
				//currObj = eval(getElementsByOjName(strCell)[0]);//FCUBS11.2_Cross_Browser#1 FCUBS_12.1.0_21066833 commented
                currObj = eval(document.getElementById("BLK_CITY_HOLIDAY_MASTER__" + monthName + "_0_" + tempCol +  "I")); // FCUBS_12.1.0_21066833 added      
						currObj.value         = "";
                        currObj.className    = gCSSReadOnly;
                    }
                    ...
                    while(row <= 5)
                    {
                        for(var tempCol = col+1; tempCol < 7; tempCol++)
                        {
                           ...
                           //currObj = eval(strCell); ///FCUBS11.1 ITR1 SFR 831  //FCUBS11.2_Cross_Browser#1
				    //currObj = eval(getElementsByOjName(strCell)[0]);//FCUBS11.2_Cross_Browser#1 //FCUBS_12.1.0_21066833 commented
					currObj = eval(document.getElementById("BLK_CITY_HOLIDAY_MASTER__" + monthName + "_" + row + "_" + tempCol + "I"));// FCUBS_12.1.0_21066833 added
                            currObj.value         = "";
                            currObj.className    = gCSSReadOnly;
                        }
                        ...
                        row++;
						*/ //redwood changes 
	fnAddRow('BLK_STTMS_CCY_HOLIDAY'); //redwood chnages 
                    }
			//redwood 
            ...
            else if(gAction == "EXECUTEQUERY" || gAction == 'MODIFY' || gAction == '')
            {
                ...
                var tblRef = getTableObjForBlock("BLK_CITY_HOLIDAY").tBodies[0];
		  //var tblRef = selectNodes(document,'//table[@id="BLK_CITY_HOLIDAY"]/tbody/tr/td[3]/input');
                var dayStatus = '';
			...
			{
                        monthList =    tblRef.rows[monthCnt].cells[4].getElementsByTagName("OJ-TEXT-AREA")[0].value; //redwood 
                    }
			else
			{
                    ...
                    for(var dayCnt = 1; dayCnt <= noDays ; dayCnt++)
                    {
                        ...
                        //strCell   = monthName + "_" + row + "_" + col; //comment 9NT1587_FC_12.0.2_IUT BUGNo 16904799
						strCell   = monthName + "_" + row + "_" + col + "I";   //added 9NT1587_FC_12.0.2_IUT BUGNo 16904799 //redwood check
                        //currObj = eval(strCell); ///FCUBS11.1 ITR1 SFR 831 //FCUBS11.2_Cross_Browser#1
				currObj = eval(getElementsByOjName(strCell)[0]);//FCUBS11.2_Cross_Browser#1 
                        currObj.value = "";
                        currObj.value = dayCnt;
                        currObj.tabIndex = tabIndex;
               ...
               //currObj.attachEvent('onkeydown',fnToggleFromKey); ///FCUBS11.1 ITR1 SFR 1130
				...
				//currObj.removeAttribute('onkeydown'); //FCUBS11.2_Cross_Browser#1   //redwood changes
				addEvent(currObj, "onkeydown", "fnToggleFromKey(this,event)");  //redwood changes
                        tabIndex++;
                       if(dayStatus == 'H')
                    ...
                    {
                        //strCell   = monthName + "_0_" + tempCol; //comment 9NT1587_FC_12.0.2_IUT BUGNo 16904799
						strCell   = monthName + "_0_" + tempCol + "I"; //added 9NT1587_FC_12.0.2_IUT BUGNo 16904799 //redwood check
                        //currObj = eval(strCell); ///FCUBS11.1 ITR1 SFR 831 //FCUBS11.2_Cross_Browser#1
				//currObj = eval(getElementsByOjName(strCell)[0]);//FCUBS11.2_Cross_Browser#1 
				currObj   = eval("getElementsByOjName('" + strCell+"')[0]"); //redwood_changes
                        currObj.value         = "";
                    ...
                    while(row <= 5)
                    {
                        ...
                        {
                            //strCell   = monthName + "_" + row + "_" + tempCol;  //comment 9NT1587_FC_12.0.2_IUT BUGNo 16904799
							strCell   = monthName + "_" + row + "_" + tempCol + "I"; //added 9NT1587_FC_12.0.2_IUT BUGNo 16904799 //redwood check
                            //currObj = eval(strCell);///FCUBS11.1 ITR1 SFR 831  //FCUBS11.2_Cross_Browser#1
	   ...
	   else if (gAction == "VIEWMNTLOG") //
            {
                ...
                for(var monthCnt = 0; monthCnt<12; monthCnt++)
                {
                    ...
                    if(typeof(tblRef.rows[monthCnt].cells[4].getElementsByTagName("oj-input-text")[0]) == 'undefined')
					{
						...
						var innerHTML = tblRef.rows[monthCnt].cells[4].getElementsByTagName("OJ-TEXT-AREA")[0].getAttribute("oldInnerHTML"); //Redwood_Changes
                    ...
                    for(var dayCnt = 1; dayCnt <= noDays ; dayCnt++)
                    {
						...
						strCell   = monthName + "_" + row + "_" + col + "I";  //redwood check
                    ...
                    for(var tempCol = 0; tempCol < firstDay; tempCol++)
                    {
						...
						strCell   = monthName + "_0_" + tempCol + "I"; //redwood changes
						currObj = eval(getElementsByOjName(strCell)[0]);//FC11.2,ITR2 SFR512
                        currObj.value         = "";
						//currObj.className    = gCSSReadOnly; //Redwood_Changes
						currObj.classList.add(...gCSSReadOnly); //Redwood_Changes
                    ...
                    while(row <= 5)
                    {
                        ...
                        for(var tempCol = col+1; tempCol < 7; tempCol++)
                        {
							...
							strCell   = monthName + "_" + row + "_" + tempCol + "I"; //redwood check
							currObj = eval(getElementsByOjName(strCell)[0]);
                            currObj.value         = "";
							//currObj.className    = gCSSReadOnly; //Redwood_Changes
							currObj.classList.add(...gCSSReadOnly); //Redwood_Changes
  ...
  //  document.getElementById("BLK_CITY_HOLIDAY").tabIndex = tabIndex++;
   // document.forms[0].BTN_EXIT.tabIndex   = tabIndex++;
	 getElementsByOjName("BTN_EXIT")[0].tabIndex   = tabIndex++; //FCUBS11.1 ITR1 SFR 1130
	if(getElementsByOjName("YEAR")[0].readOnly == false) {
    getElementsByOjName("YEAR")[0].readOnly = true;
    }
}
	...
	//Redwood_Changes ends
{
    if(gAction == "NEW" || gAction == "MODIFY")
    {
        ...
        if(currObj.value != "")
        {
			...
			var loopObj          = null;
			//redwood changes starts
			//var currMonthCell    = getTableObjForBlock("BLK_CITY_HOLIDAY").tBodies[0].rows[month].cells[4].getElementsByTagName("oj-input-text")[0];
			//var inYear           = getTableObjForBlock("BLK_CITY_HOLIDAY").tBodies[0].rows[month].cells[2].getElementsByTagName("oj-input-text")[0].value;
			
            var currMonthCell;
			var inYear;
			var monthList;
			try{
				currMonthCell    = getTableObjForBlock("BLK_CITY_HOLIDAY").tBodies[0].rows[month].cells[4].getElementsByTagName("OJ-INPUT-TEXT")[0]; //redwood_changes
				 inYear          = getTableObjForBlock("BLK_CITY_HOLIDAY").tBodies[0].rows[month].cells[1].getElementsByTagName("OJ-INPUT-TEXT")[0].value; //redwood_changes
				 monthList=currMonthCell.value;
			}
			catch(e){
				//currMonthCell=getOjMeDataObject('BLK_CITY_HOLIDAY')[month]['HOLIDAY_LIST'];
				//inYear=getOjMeDataObject('BLK_CITY_HOLIDAY')[month]['YEAR'];
			}
			var firstDayInstance = new Date(inYear,month,1);
			col                  = firstDayInstance.getDay();
            
            //if(currObj.className == gCSSReadOnly) //redwood changes
			if(currObj.classList.contains(gCSSReadOnly)){ //redwood changes
            {
				//currObj.className = gCSSHoliday;  //redwood changes
				currObj.classList.add(gCSSHoliday);  //redwood changes
            ...
            {
                //currObj.className = gCSSReadOnly; //Redwood changes
				currObj.classList.add(...gCSSReadOnly); //Redwood_Changes
...
function fnResetStyle()
{
    ...
    var currObj   = null;
    gYearRef      = getElementsByOjName("YEAR")[0];
    gLblYearOf    = getElementsByOjName("YEAR")[0];
    ...
    for(var monthCnt = 0; monthCnt < 12 ;monthCnt++)
    {
        ...
        for(var rowCnt = 0; rowCnt < 6; rowCnt++)
        {
            for(var colCnt = 0; colCnt < 7;colCnt++)
            {
				...
				strCell = monthName + "_" + rowCnt + "_" + colCnt + "I";  //added 9NT1587_FC_12.0.2_IUT BUGNo 16904799 //redwood check
                //currObj = eval(strCell);      ///FCUBS11.1 ITR1 SFR 831 //FCUBS11.2_Cross_Browser#1
		    //currObj = eval(getElementsByOjName(strCell)[0]);//FCUBS11.2_Cross_Browser#1 //redwood changes
			currObj   = eval("getElementsByOjName('" + strCell+"')[0]"); //redwood_changes
                //currObj.className = gCSSReadOnly;
				currObj.classList.add(...gCSSReadOnly);
                currObj.tabIndex  = -1;
    ...
    gYearRef.tabIndex = -1;
 //   document.getElementById("BLK_CITY_HOLIDAY").tabIndex = -1;
   // document.forms[0].BTN_EXIT.tabIndex = -1; ///FCUBS11.1 ITR1 SFR 831
	getElementsByOjName("BTN_EXIT")[0].tabIndex = -1;   ///FCUBS11.1 ITR1 SFR 1130
}
...
function fnPrevYear()
{
    if (gAction == "NEW")
    {
		//if(getElementsByOjName("YEAR")[0].value == "") //FCUBS_12.1.0_21066833 commented
		if(document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value == "")  //FCUBS_12.1.0_21066833 added
		{
			alert("Please Enter valid Year");
			return false;
		}	
		// var inputYear = getElementsByOjName("YEAR")[0].value; //FCUBS_12.1.0_21066833 commented
		var inputYear = document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value;  // FCUBS_12.1.0_21066833 added
		var curYear2 = parseInt(inputYear);
        curYear2 = curYear2 - 1;
        // getElementsByOjName("YEAR")[0].value = curYear2; //FCUBS_12.1.0_21066833 commented
		document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value=curYear2; // FCUBS_12.1.0_21066833 added
		document.getElementById("BLK_CITY_HOLIDAY_MASTER__YEAR").value=curYear2;  // FCUBS_12.1.0_21066833 added
...
function fnCalHolHeading() {
    ...
    document.getElementById("BLK_CITY_HOLIDAY_MASTER").style.display="none";
    for (var i=0; i<l_hol_head_list.length; i++) {    
        getElementsByOjName(l_hol_head_list[i])[0].className = "TEXTCalHead";
    }
}
function setCss()
{
...
function fnDefaultWeeklyHolidays(){
	 try
	 {
		...
		if  (wklyHolydaysList)
		{
			...
			document.getElementById("BLK_CITY_HOLIDAY_MASTER__WEEKLY_HOLIDAYS").value=wekholsprtr.replace('-','');
			gAction = oldAction;
		}	
	  }catch(e) 
	  {
...
function fnPostLoad_CVS_MAIN_VIEWLOG() {
	 ...
	 if (codes.length > 0)
	 {
		  getElementsByOjName("FIN_CENTRE")[0].value = codes[0];
		  getElementsByOjName("YEAR")[0].value = codes[1];
	 }
	  getElementsByOjName("MODNO")[0].value = parent.screenArgs['MOD_NO'];

	  gAction = 'VIEWMNTLOG';
	  functionId = 'OLDCTYHL' ;  

	var relationArray = new Array(); 			// {Table Name} is the array index, {Parent Table Name}~{Relation} is the array value 
relationArray['BLK_CITY_HOLIDAY_MASTER'] = ""; 
relationArray['BLK_CITY_HOLIDAY'] = "BLK_CITY_HOLIDAY_MASTER~N"; 

var dataSrcLocationArray = new Array(); 	// Array of all Data Sources used in the screen 
dataSrcLocationArray[0] = "BLK_CITY_HOLIDAY_MASTER"; 
dataSrcLocationArray[1] = "BLK_CITY_HOLIDAY"; 


	//appendData(document.getElementById("TBLPageAll"));
	appendTextFieldValue(getElementsByOjName('FIN_CENTRE')[0], 1, 'BLK_CITY_HOLIDAY_MASTER');
	appendTextFieldValue(getElementsByOjName('YEAR')[0], 1, 'BLK_CITY_HOLIDAY_MASTER');	
	appendTextFieldValue(getElementsByOjName('MODNO')[0], 1, 'BLK_CITY_HOLIDAY_MASTER');	
