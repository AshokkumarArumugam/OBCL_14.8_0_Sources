...
**  Written by         : 
**  Date of creation   : 
**  File Name          : LBCPARAT_KERNEL.js
**  Purpose            : 
**  Called From        : 

  Changed By         : Ramya M
  Changed On         :20-Dec-2021
  Search String      : OBCL_14.5_Participant_BA_RATE_CHANGES
  Reason             : OBCL_14.5_Participant_BA_RATE_CHANGES.
  
  Changed By         : Ramya M
  Changed On         : 04-FEB-2021
  Search String      : OBCL_14.5_Participant_BA_RATE_CHANGES
  Reason             : OBCL_14.5_Participant_BA_RATE_CHANGES.
  
  Changed By         : Ramya M
  Changed On         : 05-FEB-2021
  Search String      : OBCL_14.5_Participant_BA_RATE_CHANGES
  Reason             : OBCL_14.5_Participant_BA_RATE_CHANGES.
  
  Changed By         : Ramya M
  Changed On         : 06-FEB-2021
  Search String      : OBCL_14.5_Participant_BA_RATE_CHANGES
  Reason             : OBCL_14.5_Participant_BA_RATE_CHANGES.
  
  Changed By         : Ramya M
  Changed On         : 06-MAY-2022
  Search String      : Disable Participant BA dtls 
  Reason             : OBCL_14.5_Participant_BA_RATE_CHANGES.
  
  ** Changed By        : Anusha K
  **Date               : 19-Jun-2023
  **Change Description :  changes done for comp pop into ssi screen
  **Search String      : OBCL_14.6_rabo_#35438441 changes
  
  ** Changed By        : Sowmya Bitra
  **Date               : 10-July-2023
  **Change Description : Changes for Settlements and Entity Upload option
  **Search String      : Bug#35546116 Changes
  
  **Changed By         : Narendra Dhaker
  **Date               : 10-Jan-2024
  **Change Description : code added to fix performance issue
  **Search String      : Bug#36146229 Changes
  
  **Changed By         : Vineeth T M
  **Date               : 23-Feb-2024
  **Change Description : Collect list of participants and pass it to parent. It will be then further passed on to 
						 participant limit screen to pick only tranche particpants in participant LOV 
...
function Fn_Default()//ramya
{
	fnSubScreenMain('LBCPARAT', 'LBCPARAT', 'CVS_PART_BA_DTLS', false);
	debugs("gAction");
    return true;
}
...
function fnPostLoad_CVS_MNEMONIC_KERNEL()
{


	fnDisableElement(document.getElementById('cmdAddRow_BLK_MNEMONIC'));
	fnDisableElement(document.getElementById('cmdDelRow_BLK_MNEMONIC'));
	return true;
}

 function Fn_SSIDefault()
{	 
	fnSubScreenMain('LBCPARAT', 'LBCPARAT', 'CVS_MNEMONIC', false);
    ...
    fcjResponseDOM = fnPost(fcjRequestDOM,servletURL,functionId);
		...
		var RecnodeList = selectNodes(fcjResponseDOM, '//REC[@TYPE="BLK_MNEMONIC"]/FV');
		
		
if (RecnodeList.length > 0) {
    var RecnodeListLen = RecnodeList.length;
   
	test(RecnodeList,RecnodeListLen);
}
...
const delay=(i)=>{
 ...
 const test=async (RecnodeList,RecnodeListLen)=>{
    console.log("start");
    for(let i=1;i<=5;i++){
		 let frame = document.getElementById("ifrSubScreen").contentWindow;
		let elem=frame.document.getElementById(`BLK_MNEMONIC__UI_CCY`);
		if(!elem){
    ...
    for (var i = 0; i < RecnodeListLen; i++) {
               ...
               try{
	       //Bug#36146229 Changes starts
	       // frame.document.getElementById(`BLK_MNEMONIC__UI_CCY${i===0?'':'RC'+i}`).value = TextContents[9]; // getting TextContents[8] value
				   var frameElement = frame.document.getElementById(`BLK_MNEMONIC__UI_CCY${i===0?'':'RC'+i}`)
				   if(frameElement) {
				   frameElement.value = TextContents[9];
				   }
               //Bug#36146229 Changes ends
			   }
			   catch(e){
				   i=i-1;
				   
				    frame.document.getElementById('nNext__BLK_MNEMONIC').click();
					setTimeout(()=>{
					console.log('waiting');
					},3000);
			   }
...
function fnCloseAlertWin_ACCEPTOVERRIDE() {
    ...
    fnUpdateBlockLength();
    var uploadSuccess = fnProcessParticipantUpload(csvRows);
    if (uploadSuccess) {
        fnUpdateBlockLength();
...
function fnPartUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        var cells = rows[i].split(",");
        if (cells.length > 1) {
            if (i == 0) {
                if (cells[0].toUpperCase() != '[Participant Details]'.toUpperCase()) {
                    fnResetUpload();//reset upload parameters
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'PARTICIPANT'.toUpperCase()) || (cells[1].toUpperCase() != 'PARTICIPANT_NAME'.toUpperCase()) || (cells[2].toUpperCase() != 'ASSETAMT'.toUpperCase()) || (cells[3].toUpperCase() != 'CURRENT_PARTICIPATION'.toUpperCase()) || (cells[4].toUpperCase() != 'ASSET_RATIO'.toUpperCase()) || (cells[5].toUpperCase() != 'SELF_PARTICIPATION'.toUpperCase())|| (cells[6].toUpperCase() != 'ORIGINATOR'.toUpperCase())||  (cells[7].toUpperCase() != 'NETTING_PREF'.toUpperCase()) ) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnBackEndCall() {
    g_prev_gAction = gAction;
   // document.getElementById("BLK_OLTBS_CONTRACT_SCHEDULES_FESCH__ORGACTCOD").value = gAction;
    gAction = 'PARTUPLD';
    appendData();
    ...
    if (dbDataDOM.documentElement.nodeName != dataSrcLocationArray[0]) {
        ...
        var removeNode = selectNodes(orgDom, getXPathQuery("BLK_MST"));
        for (var iField = 0;iField < removeNode.length;iField++) {
            removeNode[iField].parentNode.removeChild(removeNode[iField]);
        }
...
function fnProcessParticipantUpload(rows) {
    ...
    for (var i = 2 + length;i < rowsLength + length;i++) {
        ...
        if (cells.length > 1) {
            //if ((getTableObjForBlock(blockId).tBodies[0].rows[row - 1].cells[0].getElementsByTagName("oj-input-text")[0]) && (rowCheck)) {  //Bug#35546116 Commented
			  if ((document.getElementById(blockId).tBodies[0].rows[row - 1].cells[0].getElementsByTagName("INPUT")[0]) && ((rowCheck)||(multiplesPgSize >= 1))) { //Bug#35546116 Changes
...
function fnPartUpload(e) {
		try{   
		parentFunctionId = parent.functionId;
	}catch(e){}
if ((parentFunctionId == 'LBDDDONL') || (parentFunctionId == 'FCDTRONL')){
gDisableBrowseButton = true; }
 else {
    gDisableBrowseButton = false;
}
    blockId = "BLK_CON_PARAT";//block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_CON_PARAT__BTN_PARTUPLD");
        ...
        if (regex.test(fileUploadBtn.value.toLowerCase())) {
            if (typeof (FileReader) != undefined) {
                ...
                reader.onload = function (e) {
                    ...
                    var schedulesVal = fnPartUploadValidation(rows);//header validations, system will validate first two rows
                    if (schedulesVal) {
                        fnUpdateBlockLength();
                        if (totRow > 0) {
                            csvRows = rows;
                            ...
                            showErrorAlerts("LB-UPD-016", "O", "Participant details");
                            return false;
                        }
                        else {
                            var uploadSuccess = fnProcessParticipantUpload(rows);//process upload schedules
                            if (uploadSuccess) {
                                fnUpdateBlockLength();//current length of block
...
function fnEnableBaBtn(object){
	 object.disabled = false;
    object.readOnly = false;

	var noRows = getTableObjForBlock("BLK_CON_PARAT").tBodies[0].rows.length;
		for (var rowIndex = 0;rowIndex < noRows;rowIndex++) {
		//document.getElementById("BLK_CON_PARAT__BTN_PART_BA_DTLS").focus();
		//fnEnableElement(getElementsByOjName("BTN_PART_BA_DTLS")[rowIndex]);					
		getElementsByOjName("BTN_PART_BA_DTLS")[rowIndex].disabled = false;
	//	document.getElementById("BLK_CON_PARAT__BTN_PART_BA_DTLS")[rowIndex].disabled = false;
   }
	//	getElementsByOjName("BTN_PART_BA_DTLS")[rowIndex].disabled = false;
}
...
//OBCL_14.6_rabo_#35438441 changes starts
function fnEnablessiBtn(object){
	 object.disabled = false;
    object.readOnly = false;
	...
	var noRows = getTableObjForBlock("BLK_CON_PARAT").tBodies[0].rows.length;
		...
		getElementsByOjName("BTN_SSI")[rowIndex].disabled = false;
...
function fnDisableBaBtn(object){//OBCL_14.5_Participant_BA_RATE_CHANGES starts
	 object.disabled = true;
    object.readOnly = true;
	...
	var noRows = getTableObjForBlock("BLK_CON_PARAT").tBodies[0].rows.length;
		...
		getElementsByOjName("BTN_PART_BA_DTLS")[rowIndex].disabled = true;
	   }
   return true;
	
}//OBCL_14.5_Participant_BA_RATE_CHANGES ends


//OBCL_14.6_rabo_#35438441 changes starts

function fnDisablessiBtn(object){//OBCL_14.5_Participant_BA_RATE_CHANGES starts
	 object.disabled = true;
    object.readOnly = true;
	...
	var noRows = getTableObjForBlock("BLK_CON_PARAT").tBodies[0].rows.length;
		...
		getTableObjForBlock("BTN_SSI")[rowIndex].disabled = true;
	   }
   return true;
...
function fnPostFocus_KERNEL() {
    ...
    if (((gAction == 'NEW') || (gAction == 'MODIFY') || (gAction == 'SUBSYSPKP_NEW') || (gAction == 'SUBSYSPKP_MODIFY')) && gDisableBrowseButton) {
        fnEnableElement(document.getElementById("BLK_CON_PARAT__BTN_PARTUPLD"));
    }
    else {
        fnDisableElement(document.getElementById("BLK_CON_PARAT__BTN_PARTUPLD"));
    }
	if ((parent.functionId=='LBDTRONL') || (parent.functionId=='LBDDDONL'))
	{
	fnEnableBaBtn(document.getElementById("BLK_CON_PARAT__BTN_PART_BA_DTLS"));
	fnEnablessiBtn(document.getElementById("BLK_CON_PARAT__BTN_SSI")); //OBCL_14.6_rabo_#35438441 changes
	}
    else
	{
		fnDisableBaBtn(document.getElementById("BLK_CON_PARAT__BTN_PART_BA_DTLS"));
		fnDisablessiBtn(document.getElementById("BLK_CON_PARAT__BTN_SSI"));//OBCL_14.6_rabo_#35438441 changes
	}		
    return true;
}

function fnPostLoad_CVS_PARAT_KERNEL(){
	if ((parent.functionId=='LBDTRONL') || (parent.functionId=='LBDDDONL')) 
	{
	fnEnableBaBtn(document.getElementById("BLK_CON_PARAT__BTN_PART_BA_DTLS"));
	fnEnablessiBtn(document.getElementById("BLK_CON_PARAT__BTN_SSI"));//OBCL_14.6_rabo_#35438441 changes
...
/*OBCL_14.5_Participant_BA_RATE_CHANGES STARTS*/
function fnPostLoad_CVS_PART_BA_DTLS_KERNEL(){
	//document.getElementById("MESV_BLK_PART_BA_DTLS").style.visibility = "hidden";
	//document.getElementById("BLK_PART_BA_DTLS__DISCOUNT_RATEI").readOnly = false;//OBCL_14.5_Participant_BA_RATE_CHANGES
	//document.getElementById("BLK_PART_BA_DTLS__STAMPING_FEE_RATEI").readOnly = false;//OBCL_14.5_Participant_BA_RATE_CHANGES
	//var data_blk = 'BLK_PART_BA_DTLS';
	//fnAddRow(data_blk);
	//fnAddRowMESV('BLK_PART_BA_DTLS');//OBCL_14.5_Participant_BA_RATE_CHANGES
	//getElementsByOjName('BTN_PREV_BLK_PART_BA_DTLS')[0].style.visibility = 'hidden';
	//getElementsByOjName('BTN_NEXT_BLK_PART_BA_DTLS')[0].style.visibility = 'hidden';
	//getElementsByOjName('BTN_ADD_BLK_PART_BA_DTLS')[0].style.visibility = 'hidden';
	//getElementsByOjName('BTN_REMOVE_BLK_PART_BA_DTLS')[0].style.visibility = 'hidden';
return true;
}
/*
function fnPostNew_CVS_PART_BA_DTLS_KERNEL(){

	//fnAddRowMESV('BLK_PART_BA_DTLS');
	document.getElementById("BLK_PART_BA_DTLS__DISCOUNT_RATEI").readOnly = false;//OBCL_14.5_Participant_BA_RATE_CHANGES
	document.getElementById("BLK_PART_BA_DTLS__STAMPING_FEE_RATEI").readOnly = false;//OBCL_14.5_Participant_BA_RATE_CHANGES
...
function fnPostExecuteQuery_KERNEL() {
	...
	getElementsByOjName('BTN_REMOVE_BLK_PART_BA_DTLS')[0].style.visibility = 'hidden';
//OBCL_14.6_rabo_#35438441 changes STARTS
	fnEnablessiBtn(document.getElementById("BLK_CON_PARAT__BTN_SSI"));
	document.getElementById("MESV_BLK_MNEMONIC").style.visibility = "hidden";
	getElementsByOjName('BTN_PREV_BLK_MNEMONIC')[0].style.visibility = 'hidden';
	getElementsByOjName('BTN_NEXT_BLK_MNEMONIC')[0].style.visibility = 'hidden';
	getElementsByOjName('BTN_ADD_BLK_MNEMONIC')[0].style.visibility = 'hidden';
	getElementsByOjName('BTN_REMOVE_BLK_MNEMONIC')[0].style.visibility = 'hidden';			
	//OBCL_14.6_rabo_#35438441 changes ends
	}
	 else
	{
...
function fnPostAddRow_BLK_CON_PARAT_KERNEL(){
	var data_blk = "BLK_CON_PARAT";
...
//Bug#35546116 Changes Start
function fnSetlUpload(e) {  
	blockId = "BLK_SETT_CURR_DET"; //block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_SETT_CURR_DET__BTN_SETTUPLD");
    if (gAction == 'NEW' || gAction == 'MODIFY' || gAction == 'SUBSYSPKP_NEW' || gAction == 'SUBSYSPKP_MODIFY') {
        var regex = /^([a-zA-Z0-9\s_\\.\-:])+(.csv)$/;//file format parser
        if (regex.test(fileUploadBtn.value.toLowerCase())) {
            if (typeof (FileReader) != undefined) {
                ...
                reader.onload = function (e) {
                    var rows = e.target.result.split("\n");//csv file parser
                    var settlVal = fnSettlUploadValidation(rows);//header validations, system will validate first two rows
                    if (settlVal) {
                        fnUpdateBlockLength();
                        if (totRow > 0) {
                            csvRows = rows;
                            customAlertAction = "ACCEPTSETTL";
                            mask();//masking the screen
                            showErrorAlerts("LB-UPD-016", "O", "settlement details");
                            return false;
                        }
                        else {
                            var uploadSuccess = fnProcessSettlementUpload(rows);//process upload settlements
                            if (uploadSuccess) {
                                fnUpdateBlockLength();//current length of block
                                if (totRow > 0 && settlVal) {                                   
                                    fnResetUpload();//reset upload parameters										
									fnClassDefault("BLK_MST");
                                    return true; 
                                }
                            }
                            else {
...
function fnSettlUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        var cells = rows[i].split(",");
        if (cells.length > 1) {
            if (i == 0) {
                if (cells[0].toUpperCase() != '[Settlement Details]'.toUpperCase()) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'PARTICIPANT'.toUpperCase()) || (cells[1].toUpperCase() != 'CURRENCY'.toUpperCase()) || (cells[2].toUpperCase() != 'SSI_MNEMONIC'.toUpperCase()) ) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnProcessSettlementUpload(rows) {
    var rowCheck = true;//row availability check
    var rowsLength = rows.length - 1;
    ...
    for (var i = 2 + length;i < rowsLength + length;i++) {
		...
		var cells = rows[i - length].split(",");//cells split logic in rows
        if (cells.length > 1) {       
            if ((getTableObjForBlock(blockId).tBodies[0].rows[row - 1].cells[0].getElementsByTagName("INPUT")[0]) && ((rowCheck)||(multiplesPgSize >= 1))) {
                for (var j = 0;j < cells.length - 1;j++) {
                    //cells iteration of current row
                    if (cells[j] == '~END~') {  //last cell value in current row
                        continue;//if last cell value is '~END~' then, system will skip current row and continue to the next row      
                    }
                    if (cells[j] != "" && cells[j] != '~END~') {
                        var currObject = getTableObjForBlock(blockId).tBodies[0].rows[row - 1].cells[j + 1].getElementsByTagName("oj-select-single")[0];
...
function fnCloseAlertWin_ACCEPTSETTL() {
    unmask();//unmasking the screen
    fnDeleteAllRowOfUpload();
    fnUpdateBlockLength();
    var uploadSuccess = fnProcessSettlementUpload(csvRows);
    if (uploadSuccess) {
        fnUpdateBlockLength();
        if (totRow > 0 && uploadSuccess) {         
            fnResetUpload();//reset upload parameters
...
function fnExitAlertWin_ACCEPTSETTL() {
    fnUpdateBlockLength();
    fnResetUpload();//reset upload parameters
    unmask();//unmasking the screen
    customAlertAction = "";
...
function fnEntUpload(e) {	
	blockId = "BLK_ENTITY_PARAT"; //block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_ENTITY_PARAT__BTN_ENTITYUPLD");
    if (gAction == 'NEW' || gAction == 'MODIFY' || gAction == 'SUBSYSPKP_NEW' || gAction == 'SUBSYSPKP_MODIFY') {
        var regex = /^([a-zA-Z0-9\s_\\.\-:])+(.csv)$/;//file format parser
        if (regex.test(fileUploadBtn.value.toLowerCase())) {
            if (typeof (FileReader) != undefined) {
                ...
                reader.onload = function (e) {
                    var rows = e.target.result.split("\n");//csv file parser
                    var entityVal = fnEntityUploadValidation(rows);//header validations, system will validate first two rows
                    if (entityVal) {
                        fnUpdateBlockLength();
                        if (totRow > 0) {
                            csvRows = rows;
                            customAlertAction = "ACCEPTENTITY";
                            mask();//masking the screen
                            showErrorAlerts("LB-UPD-016", "O", "Entity details");
                            return false;
                        }
                        else {
                            var uploadSuccess = fnProcessEntityUpload(rows);//process upload entity details
                            if (uploadSuccess) {
                                fnUpdateBlockLength();//current length of block
                                if (totRow > 0 && entityVal) {                                   
                                    fnResetUpload();//reset upload parameters										
							    	fnClassDefault("BLK_MST");
                                    return true;
                                }
...
function fnEntityUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        var cells = rows[i].split(",");
        if (cells.length > 1) {
            if (i == 0) {
                if (cells[0].toUpperCase() != '[Entity Details]'.toUpperCase()) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'PARTICIPANT'.toUpperCase()) || (cells[1].toUpperCase() != 'ENTITY_ID'.toUpperCase()) || (cells[2].toUpperCase() != 'ENTITY_NAME'.toUpperCase()) || (cells[3].toUpperCase() != 'REMARKS'.toUpperCase()) || (cells[4].toUpperCase() != 'PRIMARY_ENTITY'.toUpperCase())) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnProcessEntityUpload(rows) {
    var rowCheck = true;//row availability check
    var rowsLength = rows.length - 1;
...
function fnCloseAlertWin_ACCEPTENTITY() {
    unmask();//unmasking the screen
    fnDeleteAllRowOfUpload();
    fnUpdateBlockLength();
    var uploadSuccess = fnProcessEntityUpload(csvRows);
    if (uploadSuccess) {
        fnUpdateBlockLength();
        if (totRow > 0 && uploadSuccess) {
            fnResetUpload();//reset upload parameters
...
function fnExitAlertWin_ACCEPTENTITY() {
    fnUpdateBlockLength();
    fnResetUpload();//reset upload parameters
    unmask();//unmasking the screen
    customAlertAction = "";
...
//Bug#35546116 Changes End
//Bug#36263557 Changes start
function fnPreSave_CVS_PARAT_KERNEL(){
	debugs("fnPreSave_CVS_PARAT_KERNEL");
	...
	parent.listTrancheParticipants = participants;
	return true;
}
