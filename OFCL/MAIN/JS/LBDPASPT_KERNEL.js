...
function fnCreateBody_LBDPASPT() {
    var msgxml_lbdpaspt = "<FCUBS_BODY>";
    msgxml_lbdpaspt += '    <FLD>'; 
	msgxml_lbdpaspt += '      <FN PARENT="" RELATION_TYPE="1" TYPE="BLK_PARTIAL_REPRICE_PROCESS">CONTRACTREFNO~SPLITSERIALNO~MATURITYDATE~BRANCHCODE~PROCESSSTATUS~PROCESSDATE~SUM_REC_ID</FN>'; 
	msgxml_lbdpaspt += '    </FLD>'; 
    msgxml_lbdpaspt += '<REC RECID="1" TYPE="BLK_PARTIAL_REPRICE_PROCESS"><FV/></REC></FCUBS_BODY>';
    reqDom=loadXMLDoc(msgxml_lbdpaspt );
    var blkCdtSecNd = "";
    var blkCdtSecNd = reqDom.createCDATASection('~~~~~~'+bulkPkValues);
    selectSingleNode(selectSingleNode(reqDom,"//REC[@RECID='1'][@TYPE='BLK_PARTIAL_REPRICE_PROCESS']"),"FV").appendChild(blkCdtSecNd);
...
function fn_Process(){
		...
		setNodeText(selectSingleNode(exlRequestDOM,"FCUBS_REQ_ENV/FCUBS_HEADER/FUNCTIONID"), "LBSPASPT");
		...
		var bodyReq = fnCreateBody_LBDPASPT();
