...
function fnPrPlSchUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        ...
        if (cells.length > 1) {
            if (i == 0) {
                if (cells[0].toUpperCase() != '[UDF Upload]'.toUpperCase()) {
                    fnResetUpload();//reset upload parameters
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'Contract Ref No'.toUpperCase()) || (cells[1].toUpperCase() != 'Field Name'.toUpperCase()) || (cells[2].toUpperCase() != 'Field Value'.toUpperCase())  || (cells[3].toUpperCase() != 'Rec Key'.toUpperCase()) ) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnPrplSchUpload(e) {
    ...
    blockId = "BLK_UDF_UPLOAD";//block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_UDF_MASTER__BTN_FILE_UPD");
	...
	document.getElementById("BLK_UDF_MASTER__FILE_NAME").value = fileUploadBtn.value.split('\\').pop();
                    ...
                    if (prplSchUploadVal) {
                        ...
                        if (totRow > 0) {
                            ...
                            showErrorAlerts("LB-UPD-016", "O", "UDF Upload");
                            return false;
                        }
                        else {
                            var uploadSuccess = fnPrplSchUpld(rows);//process upload UDFs
                            if (uploadSuccess) {
                                fnUpdateBlockLength();//current length of block
                                if (totRow > 0 && prplSchUploadVal) {
                                    var backEndCall = fnBackEndCall();//back end call to validate uploaded UDFs
                                    if (backEndCall) {
                                        fnResetUpload();//reset upload parameters
                                        return true;
                                    }
...
{
  if (document.getElementById("BLK_UDF_MASTER__PAYAU__AUTHSTAT").value == "A")
{
DisableToolbar_buttons("Authorize");
DisableToolbar_buttons("Delete");
}
  if (document.getElementById("BLK_UDF_MASTER__PAYAU__AUTHSTAT").value == "U")
{
...
function fnPostFocus_KERNEL() {
  /*  var componentFlag = true;
    ...
    if (((gAction == 'NEW') || (gAction == 'MODIFY')) && gDisableBrowseButton) {
        fnEnableUploadBtnElement(document.getElementById("BLK_UDF_MASTER__BTN_FILE_UPD")); 
    }
    else {
        fnDisableUploadBtnElement(document.getElementById("BLK_UDF_MASTER__BTN_FILE_UPD"));
    }
    return true;
}
/* OBCL_CSV_File_Upload_Changes :: Ends */