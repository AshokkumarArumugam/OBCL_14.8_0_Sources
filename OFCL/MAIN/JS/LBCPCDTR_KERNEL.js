...
****************************************************************************************************************************/
function fnPostLoad_CVS_PROCESS_CONSOL_TRNFR_HIST1_KERNEL(screenArgs) {
...
function fnPostLoad_CVS_PROCESS_TRNFR_HIST_KERNEL(screenArgs){  
	if (screenArgs['INSTRTYPE']=='Y'){
			...
			//	if (document.getElementById("DIVSubSystem").children[0].children[idx].getElementsByTagName("li")[0].id == "CVS_PROCESS_CONSOL_TRNFR_HIST1") {//BUG#34958820_OBCL_14.8_LS_REDWOOD_CHANGES
			if  (document.getElementById("subSystemConveyorBelt").getElementsByClassName("oj-conveyorbelt-content-container")[0].getElementsByTagName("oj-button")[idx].id == "CVS_PROCESS_CONSOL_TRNFR_HIST1")
				{
...
//					fnDisableSubSysButtons(document.getElementById("DIVSubSystem").children[0].children[idx].getElementsByTagName("li")[0].getElementsByClassName("Abutton")[0]);//BUG#34958820_OBCL_14.8_LS_REDWOOD_CHANGES
		...
		else
		{
			...
			for (var idx = 0; idx < len; idx++) {
				//if (document.getElementById("DIVSubSystem").children[0].children[idx].getElementsByTagName("li")[0].id == "CVS_PROCESS_CONSOL_TRNFR_HIST1") {//BUG#34958820_OBCL_14.8_LS_REDWOOD_CHANGES
					if  (document.getElementById("subSystemConveyorBelt").getElementsByClassName("oj-conveyorbelt-content-container")[0].getElementsByTagName("oj-button")[idx].id == "CVS_PROCESS_CONSOL_TRNFR_HIST1")//BUG#34958820_OBCL_14.8_LS_REDWOOD_CHANGES
					{
...
document.getElementById("cmdAddRow_BLK_PARTICIPANT_PROCESS").style.visibility = 'hidden';
document.getElementById("cmdDelRow_BLK_PARTICIPANT_PROCESS").style.visibility = 'hidden';
