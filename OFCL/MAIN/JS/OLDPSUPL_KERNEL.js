...
function fnCloseAlertWin_ACCEPTOVERRIDE() {
    ...
    fnUpdateBlockLength();
    var uploadSuccess = fnPrplSchUpload(csvRows);
    if (uploadSuccess) {
        fnUpdateBlockLength();
...
function fnPrPlSchUploadValidation(rows) {
    for (var i = 0;i < 2;i++) {
        var cells = rows[i].split(",");
        if (cells.length > 1) {
            if (i == 0) {
                if (cells[0].toUpperCase() != '[Principal Schedule Upload]'.toUpperCase()) {
                    fnResetUpload();//reset upload parameters
            ...
            if (i == 1) {
                if ((cells[0].toUpperCase() != 'Contract Ref No'.toUpperCase()) || (cells[1].toUpperCase() != 'Start Date'.toUpperCase()) || (cells[2].toUpperCase() != 'Frequency'.toUpperCase()) || (cells[3].toUpperCase() != 'Unit'.toUpperCase()) || (cells[4].toUpperCase() != 'Amount'.toUpperCase())) {
                    fnResetUpload();//reset upload parameters
                    errCode = 'LB-UPD-013';//alert("Please upload a valid CSV file.");
                    return false;
                }
...
function fnPrplSchUpload(e) {
    gDisableBrowseButton = false;
    blockId = "BLK_SCHEDULE_UPLOAD";//block id to upload the CSV file
    fileUploadBtn = document.getElementById("BLK_SCHEDULE_MASTER__BTN_FILE_UPD");
	...
	document.getElementById("BLK_SCHEDULE_MASTER__FILE_NAME").value = fileUploadBtn.value.split('\\').pop();
                    var prplSchUploadVal = fnPrPlSchUploadValidation(rows);//header validations, system will validate first two rows
                    if (prplSchUploadVal) {
                        fnUpdateBlockLength();
                        if (totRow > 0) {
                            csvRows = rows;
                            ...
                            showErrorAlerts("LB-UPD-016", "O", "Prinicipal Schedule Upload");
                            return false;
                        }
                        else {
                            var uploadSuccess = fnPrplSchUpld(rows);//process upload schedules
                            if (uploadSuccess) {
                                fnUpdateBlockLength();//current length of block
                                if (totRow > 0 && prplSchUploadVal) {
...
{
  if (document.getElementById("BLK_SCHEDULE_MASTER__PAYAU__AUTHSTAT").value == "A")
{
DisableToolbar_buttons("Authorize");
DisableToolbar_buttons("Delete");
}
  if (document.getElementById("BLK_SCHEDULE_MASTER__PAYAU__AUTHSTAT").value == "U")
{
...
function fnPostFocus_KERNEL() {
  /*  var componentFlag = true;
    ...
    if (((gAction == 'NEW') || (gAction == 'MODIFY')) && gDisableBrowseButton) {
        fnEnableUploadBtnElement(document.getElementById("BLK_SCHEDULE_MASTER__BTN_FILE_UPD")); 
    }
    else {
        fnDisableUploadBtnElement(document.getElementById("BLK_SCHEDULE_MASTER__BTN_FILE_UPD"));
    }
    return true;
}
/* OBCL_CSV_File_Upload_Changes :: Ends */