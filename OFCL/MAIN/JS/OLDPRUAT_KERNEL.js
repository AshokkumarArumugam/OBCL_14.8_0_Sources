...
**  Date of creation   : 
**  File Name          : OLDPSUAT_KERNEL.js
**  Purpose            : Payables/Receivables Upload
**  Called From        : OLDPRUAT
****************************************************************************************************************************/
var subScreen='N';
function fnPostLoad_CVS_AUTH_KERNEL(screenArgs) {
   ...
   // document.getElementById("BLK_OLTBS_CONTRACT__CALLINGFID").value=screenArgs['MASTERFNID'];
    if(!fnProcessAuthOnLoad(screenArgs))
        return false;      
	if (document.getElementById("BLK_OLTBS_PAY_RECV_UPL_MASTER_AU__REFERENCE_NO").value != null){
		document.getElementById("BLK_OLTBS_PAY_RECV_UPL_MASTER_AU__BTN_AUTH").disabled = false;
	}
...
function fnAuthorise() {
    ...
    gAction = 'AUTH';
       ...
       if (!fnOnlineAuthorize(subScreen)) {		
		...
		document.getElementById("BLK_OLTBS_PAY_RECV_UPL_MASTER_AU__BTN_AUTH").disabled = true;
