create or replace force view olvw_form_1098_details
(contract_ref_no, branch, counterparty, borrower_name, borrower_ssn, interest_paid, refund_amt)
as
/*----------------------------------------------------------------------------------------------------
**
** File Name	: olvw_form_1098_details.VW
**
** Module	: LD
**
...
----------------------------------------------------------------------------------------------------
*/
select a.contract_ref_no,a.branch,a.counterparty,b.customer_name1,b.ssn,
decode(g.contract_ref_no,a.contract_ref_no,(decode(g.tax_year,f.tax_year,g.interest_paid,0)),
...
and to_char(d.paid_date,'YYYY') = f.tax_year
...
Group by a.contract_ref_no,a.branch,a.counterparty,b.customer_name1,b.ssn,f.threshold_limit_amt,
g.contract_ref_no,g.interest_paid,g.tax_year,f.tax_year
Having decode(g.contract_ref_no,a.contract_ref_no,(decode(g.tax_year,f.tax_year,g.interest_paid,0)),
sum(nvl(d.amount_settled,0))) - sum(nvl(k.interest_waiver_amt,0)) 
+ sum((decode(to_char(i.value_date,'YYYY'),f.tax_year,nvl(i.refund_amt,0),0))) >= f.threshold_limit_amt
/
CREATE OR REPLACE SYNONYM olvws_form_1098_details FOR olvw_form_1098_details 
/