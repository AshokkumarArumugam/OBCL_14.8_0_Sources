CREATE OR REPLACE force VIEW olvw_dw_agency AS
/*----------------------------------------------------------------------------------------------------
**
** File Name	: olvw_dw_agency.vw
**
** Module	: INTERFACE
**
**
...
** 31-AUG-2012 CITIUS#14745 changes: Added for Dealtrax Data ware housing changes 
   19-OCT-2012 Flexcube V.CL Release 7.12, Retro, CITIUS#14972 Changes - DealTrax changes, New fields included
** 04-JAN-2013 FLEXCUBE Universal Banking 3.3.0.0.0CITI_R712 RETRO CITIUS#15694, Changes to include Active and Liquidated contracts for Agency instead of only Active contracts (in sync with originations).
** 04-JAN-2013 FLEXCUBE Universal Banking 3.3.0.0.0CITI_R712 RETRO CITIUS#15754, GFRN changes for Originations (reverted for Agency)
   03-JUN-2012 Oracle FLEXCUBE Universal Banking 3.3.0.0.0CITI_R713 RETRO CITIUS#16600 Changes, Uninitiated contracts also to be included
** 23-NOV-2015 Oracle FLEXCUBE Universal Banking 3.3.0.0.0CITI_R717 RETRO CITIUS#19392 Reversed contracts pickup logic is changed to align with that of Origination contracts.
--------------------------------------------------------------------------------------------------
*/
SELECT
    ...
    --, OLTB_CONTRACT_MASTER.global_tranche_amount                  --SBJ CHANGES
    ...
    ,'A' record_type
    ,OLTB_CONTRACT_MASTER.lc_fronting
    --,NULL GFRN --19-OCT-2012 Flexcube V.CL Release 7.12, Retro, CITIUS#14972 Changes--04-JAN-2013 FLEXCUBE Universal Banking 3.3.0.0.0CITI_R712 RETRO CITIUS#15754, commented
  FROM
      OLTB_CONTRACT_MASTER,
      OLTB_CONTRACT
  WHERE   OLTB_CONTRACT_MASTER.contract_Ref_no   =   OLTB_CONTRACT.contract_ref_no
  AND     OLTB_CONTRACT_MASTER.version_no     =   OLTB_CONTRACT.latest_version_no
  AND     OLTB_CONTRACT.module_code         =   'LB'
--AND     OLTB_CONTRACT.contract_status       =   'A'  -- 04-JAN-2013 FLEXCUBE Universal Banking 3.3.0.0.0CITI_R712 RETRO CITIUS#15694 commenting.
  --03-JUN-2012 Oracle FLEXCUBE Universal Banking 3.3.0.0.0CITI_R713 RETRO CITIUS#16600 Changes start here
   --AND     OLTB_CONTRACT.contract_status  NOT IN('V','H','Y')--10-DEC-2012 CITIUS#15694 Added.
  --19-NOV-2014 CITIUS#19392 change starts
   --AND     OLTB_CONTRACT.contract_status  NOT IN('V','H')
  --03-JUN-2012 Oracle FLEXCUBE Universal Banking 3.3.0.0.0CITI_R713 RETRO CITIUS#16600 Changes end here
  AND  (
	OLTB_CONTRACT.contract_status NOT IN('V','H')
	   OR
		(OLTB_CONTRACT.contract_status = 'V' AND OLTB_CONTRACT.book_date < global.application_date
		AND (OLTB_CONTRACT.latest_event_date BETWEEN TRUNC(global.application_date,'MON') AND global.application_date)
		)
	)  
  --19-NOV-2014 CITIUS#19392 change ends.
/
CREATE OR REPLACE SYNONYM olvws_dw_agency
FOR olvw_dw_agency
/