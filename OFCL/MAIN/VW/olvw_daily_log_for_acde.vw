CREATE OR REPLACE FORCE VIEW olvw_daily_log_for_acde
AS
/*----------------------------------------------------------------------------------------------------
**
** File Name	: olvw_daily_log_for_acde.VW
**
...
AND	a.module		IN ('AC','OL')
AND	a.cust_gl		= 'G'
AND	a.ac_branch		= b.branch_code
AND	a.trn_ref_no		= b.unit_ref_no
AND	b.unit_type		= 'R'
AND	a.trn_ref_no		= c.reference_no(+)
AND	a.event_sr_no		= NVL(c.evnt_seq_no,a.event_sr_no)
/
CREATE OR REPLACE SYNONYM olvws_daily_log_for_acde FOR olvw_daily_log_for_acde
/