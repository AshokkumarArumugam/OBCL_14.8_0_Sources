...
function MenuDetails() {
	loadXML("AnnotationXML/FCUBSServiceOperationList.xml");
...
function getServices(xmlDoc) {
	...
	for (var i = 0; i < x.length; i++) {
		...
		if (next.nodeName == "SERVICE_DESCRIPTION") {
			Service_Description = next.childNodes[0].nodeValue;
	...
	menuContent += "</ul></br>";
	menuContent += "</div><br><br><br><div id='footer'><legend>FLEXCUBE UBS Integration Gateway © 2016, Oracle and/or	its affiliates. All rights reserved.</legend></div>";
...
function getOperations(Service) {
	...
	for (var i = 0; i < x.length; i++) {
		...
		if (value == Service) {
			...
			menuContent += "<p>";
			if (next.nodeName == "SERVICE_DESCRIPTION") {
				menuContent += next.childNodes[0].nodeValue;
			...
			while (next.nodeName != "EndOfList") {
				...
				var Desc = next.nextSibling.nextSibling;
			...
			menuContent += "</ul>";
			menuContent += "</div><br><br><div id='footer'><legend>FLEXCUBE UBS Integration Gateway © 2016, Oracle and/or	its affiliates. All rights reserved.</legend></div>";
			break;
		}
	}
		...
		for (var i = 0; i < x.length; i++) {
			if (x[i].getAttribute("name") == selectedVal[0]) {
				...
				if (next.getAttribute("name") == selectedVal[1]) {
					...
					tableContent += "</fieldset> <br><br><div id='footer'><legend>FLEXCUBE UBS Integration Gateway © 2016, Oracle and/or	its affiliates. All rights reserved.</legend></div>";
					document.getElementById('divOperations').style.display = "none";
					document.getElementById('divListOfXSDs').style.display = "block";
...
function getXSDDescription(XSDName) {
	...
	if (xmlDoc != null) {
		xmlDoc.async = false;
		loadXML("AnnotationXML/"+File[0] + ".xml");
...
function getDetails(XSDName) {
	...
	var mainWindow = window.open("Load1.htm", "",
...
function getData(xmlDoc) {
	...
	CountNodes = 0;
	for (var i = 0; i < x.length; i++) {
		var y = x[i].getElementsByTagName("XSD_NODE_1")[0].nextElementSibling;
...
function getOtherElements(xmlDoc, name, num, parent) {
	...
	if (x.length > 0) {
		for (var i = 0; i < x.length; i++) {
			var next = x[i].nextElementSibling;
			var previous = x[i].previousElementSibling;

			if (next == null || next.nodeName == "EndOfList") {
				subString = '';
				if (previous.childNodes[0].nodeValue == name) {
					if (num > 4) {
						var pre = previous.previousElementSibling;
...
function selectSingleNode(xmlDoc, strXMLPath) {
	...
	var oResult = oEvaluator.evaluate(strXMLPath, xmlDoc, null,
...
function getExtNode(EXTNodes, parent) {
	var nodeTmp;
	for(i=0;i<EXTNodes.length; i++) {
		console.log(EXTNodes[i].childNodes[EXTNodes[i].childNodes.length-2].nodeValue);
		if(getNodeText(EXTNodes[i].childNodes[EXTNodes[i].childNodes.length-2])==parent) {
			nodeTmp = EXTNodes[i];
			break;
		}
...
function getNodeDetails(value) {
	...
	if (xmlDoc != null) {
		xmlDoc.async = false;
		loadXML("AnnotationXML/"+File + ".xml");
	...
	var EXTNodes = selectNodes(xmlDoc, ("//NodeDetails[NodeName='"
			+ SelectedNode[1] + "']"));

	var EXTNode = getExtNode(EXTNodes, SelectedNode[2]);
	
	if (selectSingleNode(EXTNode, "Comment") != null) {
		Comment = getNodeText(selectSingleNode(EXTNode, "Comment")).trim();
