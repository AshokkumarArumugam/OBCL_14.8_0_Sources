...
<xsl:template name="sprtHandler">
    ...
    <xsl:template name="MultipleHandler_tmp">
        ...
        <tfoot>
            <tr><td colspan="{$spanCnt}" tabindex="0"><span class="LBLinv"><xsl:value-of select = "$end_table"/><xsl:value-of select = "$end_table"/></span><xsl:text disable-output-escaping="yes">&#160;</xsl:text></td></tr>
        </tfoot>
    </xsl:template>
    ...
    <xsl:template match="FIELD/TYPE[text()='BUTTON']" mode="template">
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="../../@TYPE != 'ME' or ../../@VIEW = 'SE'">              
                    <div class="DIVText" onclick="mainWin.fnUpdateScreenSaverInterval();"><label class="LBLstd"></label><!-- 12.0.2 ScreenSaver Changes -->
                        <xsl:call-template name="dispButtonField_tmp"/>
                    </div>
                </xsl:if>
            </xsl:otherwise>
    ...
    <xsl:template match="TYPE[text()='TEXT' or text()='AMOUNT' or text()='PRODUCT' or text()='DATE' or text()='ACCOUNT' or text()='BRANCH' or text()='CURRENCY' or text()='CUSTOMER' or text()='MASK' or text()='RESTRICTED_TEXT']" mode="template_fldset">
    ...
    <xsl:template  match="SECTION">
        ...
        <xsl:value-of select = "$section_class"/>
            ...
            <span class="sectionToggle">
				<button class="BTNicon" title="$expand_section" id = "BtnSectionNav" onclick="expandCollapseSection(this, 'EXPAND');"><span class="sectionExpand"></span>
				</button>
            </span>
...
<xsl:template name="dispCheckboxField_tmp">
    ...
    <xsl:if test="(../../@TYPE = 'ME' and ../../@VIEW != 'SE') or (../../@TYPE = 'ME' and count(../../@VIEW) = 0)">
        <label class="LBLauto">
            ...
            </xsl:attribute><span class="LBLinv">
                <xsl:value-of select="../NAME"></xsl:value-of>
            </span>
            <input type="checkbox" class="CHKstd" disabled="true">
                <xsl:call-template name="ATTR_Handler_tmp">
