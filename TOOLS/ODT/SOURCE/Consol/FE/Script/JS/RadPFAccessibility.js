...
function shortcut(e) {
    ...
    if (Preobjec != "") {
        ...
        var Txtar = document.getElementById(formName).getElementsByTagName("TEXTAREA");
        if (evnt.shiftKey == true && evnt.keyCode == 9) {
            if (srcElement.id == "RADPFINDEXCLOSE") {
                if (Txtar.length >= 1) {
                    Txtar[Txtar.length - 1].focus();
            ...
            if (srcElement.id.substring(srcElement.id.length - 3, srcElement.id.length) == "_TE") {
                document.getElementById("RADPFINDEXCLOSE").focus();
                fnDisableBrowserKey(e);
                preventpropagate(e);
                return;
            }
            ...
            if (Txtar.length >= 1) {
                if (srcElement.id == Txtar[Txtar.length - 1].id) {
                    document.getElementById("RADPFINDEXCLOSE").focus();
                    fnDisableBrowserKey(e);
                    preventpropagate(e);
                    return;
    ...
    else if (evnt.keyCode == 9 && srcElement.id == "TPFD" && evnt.shiftKey == false) {
        document.getElementById("RADPFINDEXCLOSE").focus();
        fnDisableBrowserKey(evnt);
        preventpropagate(evnt);
        return;
    }
    else if (evnt.keyCode == 9 && evnt.shiftKey == true && srcElement.id == "RADPFINDEXCLOSE") {
        document.getElementById("TPFD").focus();
        fnDisableBrowserKey(evnt);
        preventpropagate(evnt);
        return;
    }
