...
function FnShowTabs(divid) {
    ...
    var tablist = "DIV_DBF~DIV_CB~DIV_SUMORDER~DIV_EVENTS~DIV_CUST_ATTRS~DIV_BIND_VRBLS~DIV_RTN_FLD~DIV_OFF_LINE_BIND_VRBLS~DIV_OFF_LINE_RTN_FLD~DIV_DASHBOARD_LINK~DIV_AMOUNTTAB~DIV_PRG_BUS_FLTR_D~DIV_PRG_EXE_FLTR_D~DIV_PRG_FF_FLTR_D~DIV_PATTERNS~DIV_CRISEARCH~DIV_BLKFLDS~DIV_MENUTABLS";
...
function EnableFlds(tableName) {
    ...
    for (var k = 1;k <= 11;k++) {
        if (document.getElementById('FUNCTION_TYPE').value == "S")
            document.getElementById(tableName).rows[k].cells[5].getElementsByTagName("BUTTON")[0].disabled = true;
    }
...
function fnCheckFnCatgry(tableName) {
    ...
    if (funCtgry == "MAINTENANCE") {
        for (var i = 8;i <= 11;i++) {
            ...
            document.getElementById(tableName).rows[i].cells[2].getElementsByTagName("INPUT")[0].disabled = true;
            document.getElementById(tableName).rows[i].cells[5].getElementsByTagName("BUTTON")[0].disabled = true;
            document.getElementById(tableName).rows[i].cells[2].getElementsByTagName("INPUT")[0].value = "";
            document.getElementById(tableName).rows[i].cells[3].getElementsByTagName("INPUT")[0].disabled = true;
        }
    }
        ...
        for (var i = 4;i <= 11;i++) {
            document.getElementById(tableName).rows[i].cells[3].getElementsByTagName("INPUT")[0].disabled = false;
            if (i > 7) {
                document.getElementById(tableName).rows[i].cells[5].getElementsByTagName("BUTTON")[0].disabled = false;
            }
        }
    }
}
...
function EnableScrArgs(flag) {
	...
	if(document.getElementById("MAIN_SCREEN").checked == false){
	document.getElementById("SCREEN_QUERYREQ").cheked = false;
	}
}
...
function checkNewCriteria() {
			...
			var loopbreakflag=false;
                if (tablen > 1) {
                for (var i = 0;i < tablen;i++) {
                if (tab.rows[i].cells[5].getElementsByTagName('INPUT')[0].value != ""){
				       loopbreakflag=true;
						break;					   
            }
        }
