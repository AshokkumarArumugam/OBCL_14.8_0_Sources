		...
		if (document.getElementById('BLK_MULTI_RECORD').value == "Y") {
				...
				for (var j = 0; j < dtscr.length; j++) {
					if (selectSingleNode(dtscr[j], "DATASRC_NAME") != null) {
						if ((getNodeText(selectSingleNode(dtscr[j], 'MULTI_RECORD')) == "Y")
								&& getNodeText(selectSingleNode(dtscr[j], 'DATASRC_TYPE')) == "SUMMARY") {
			...
			for (var i = 0; i < dtblks.length; i++) {
				if (selectSingleNode(dtblks[i], 'BLK_DATASOURCES') != null) {
					...
					var BlkDtscr = selectNodes(dtblks[i],
							("//RAD_DATA_BLOCKS[BLOCK_NAME='" + blkName + "']/BLK_DATASOURCES"));
        	...
        	for (var k = 0; k < dtscr.length; k++) {
				if (getNodeText(selectSingleNode(dtscr[k], 'MULTI_RECORD')) == "N") {
					...
					for (var d = 0; d < dbs; d++) {
						if (getNodeText(selectSingleNode(dtscr[k], "DATASRC_NAME")) == document
								.getElementById('DBLK_DATASOURCE_NAME').options[d].value) {
							dns = true;
							break;
						}
					}
					if (dns == false) {
						if (blkTyp == "SUMMARY") {
							if (getNodeText(selectSingleNode(dtscr[k], 'DATASRC_TYPE')) == "SUMMARY") {
								addOption(document.getElementById('DATASOURCE_NAME'), getNodeText(selectSingleNode(
										dtscr[k], "DATASRC_NAME")), getNodeText(selectSingleNode(dtscr[k],
										"DATASRC_NAME")), false);
							}
						} else {
							addOption(document.getElementById('DATASOURCE_NAME'), getNodeText(selectSingleNode(
									dtscr[k], "DATASRC_NAME")),
									getNodeText(selectSingleNode(dtscr[k], "DATASRC_NAME")), false);
						}
					}
				}
			}
    ...
    if (node == 'RAD_SECTIONS') {
        ...
        for (var j = 0;j < dtscrlen;j++) {
            if (selectNodes(dom, "//RAD_SCREENS[@ID='" + l_val1 + "']/" + tab_type + "[@ID='" + tab_type + "']" + "/RAD_TABS[@ID='" + l_val2 + "']/" + node)[j]) {
                ...
                var sel_Multiple = getNodeText(selectSingleNode(selectNodes(dom, ("//RAD_SCREENS[@ID='" + l_val1 + "']/" + tab_type + "[@ID='" + tab_type + "']" + "/RAD_TABS[@ID='" + l_val2 + "']/" + node))[j], "MULTIPLE_SEC"));
                ...
                if (visibletab == "Y") {
					if (sel_Multiple == "Y") {
						...
						} else if (document.getElementById("VIEW_TYPE").value == "MULTIPLE") {
						} else {
							break;
						}
					}
...
function fn_Chekc_RadXml(NewDOM) {
	...
	if(selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RELEASE_TYPE")){
	if (parent.relType == "CLUSTER") {
		if (getNodeText(selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RELEASE_TYPE")) == "KERNEL") {
			if (alertMessage("You are loading KERNEL RAD in CLUSTER Release", "I")) {
				return true;
			}
			//reloadForm();
			//return false;
		...
		if (getNodeText(selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RELEASE_TYPE")) == "KERNEL") {
			if (alertMessage("You are loading KERNEL RAD in CUSTOM Release", "I")) {
				return true;
			}
			//reloadForm();
			//return false;

		} else if (getNodeText(selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RELEASE_TYPE")) == "CLUSTER") {
			if (alertMessage("You are loading CLUSTER RAD in CUSTOM Release", "I")) {
				return true;
			}
			//reloadForm();
			//return false;
    ...
    if (parent.relType == "KERNEL") {
        if (selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_CLUSTER") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_CUSTOM") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_CUSTOMER") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_CHILD_CLUSTER") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_CHILD_CUSTOM") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_CHILD_CUSTOMER") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_SCRCHLD_CLUSTER") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_SCRCHLD_CUSTOM") != null || selectSingleNode(NewDOM, "//RAD_FUNCTIONS/RAD_SCRCHLD_CUSTOMER") != null) {
            alertMessage("Radxml Had Been Modified in Other Release", "O");
            reloadForm();
            return false;
...
function FnCheckSecVal() {
    if (document.getElementById('SECTION_LABEL').value == "" && document.getElementById('COLLAPSE_SEC').checked == true) {
...
function fnPopulatedataype() {
    ...
    if ((multRec[0] != "")) {
        ...
        document.getElementById("DATATYPE_DT").value = res[0];//26374789 
        document.getElementById("MAX_LENGTH").value = res[1];

        if (document.getElementById("BLOCK_NAME_DT").value != "") { //26374789 
            var blname = document.getElementById("BLOCK_NAME_DT").value;
...
function fnGatewayvalidation(node) {
	if (node == "GW_FUNCTION") {
		if (document.getElementById('GW_FUNCTION').checked == true) {
			document.getElementById("ELCM_FUNCTION").disabled = true;
			document.getElementById("ELCM_FUNCTION").checked = false;

		} else {
			document.getElementById("ELCM_FUNCTION").disabled = false;
		}
	} else {
		if (document.getElementById('ELCM_FUNCTION').checked == true) {
			document.getElementById("GW_FUNCTION").disabled = true;
			document.getElementById("GW_FUNCTION").checked = false;

		} else {
			document.getElementById("GW_FUNCTION").disabled = false;
		}
...
function fnRestActionsUI(){
	 if (document.getElementById('REST_ENABLED').checked == true) {
         ...
         document.getElementById("REST_CACHE_ENABLED").disabled = false; // Bug#33914847	
       }
		else {
       document.getElementById("REST_MICRO_ENABLED").disabled = true; 
		...
		document.getElementById("REST_CACHE_ENABLED").disabled = true; // Bug#33914847 
		document.getElementById("REST_CACHE_ENABLED").checked = false; // Bug#33914847 
      }
	 ...
	 if (document.getElementById('REST_ENABLED').checked == true && document.getElementById('REST_SERVICE_NAME').value!="") {
		 for (var k = 1;k < 13;k++) {  
			 if(document.getElementById("OPERATION_CODE"+k).value!=""){
...
function fnPopulateLOVdataype() {
    /* for fetch columns from the table selected   */
	var funcN=document.getElementById("FUNCTION_ID").value;
	var lovid=document.getElementById("LOV_NAME_QUERY_LOVDTLS").value; 
	var queryString = "FNPOPULATELOVTYPE";
    //var queryString = "FETCH@SELECT DATA_TYPE , DECODE(DATA_TYPE,'CHAR', CHAR_LENGTH, 'VARCHAR2', CHAR_LENGTH, 'NUMBER', NVL(DATA_PRECISION,DATA_LENGTH), DATA_LENGTH) DATA_LENGTH FROM user_tab_cols  WHERE TABLE_NAME ='" + tabname + "' and COLUMN_NAME='" + clname + "'";
    var WhereString = "where b.function_id='" + funcN + "' and b.lov_id='" + lovid + "'  AND b.function_id = A.function_id  AND b.LOV_ID = A.LOV_ID";
    parent.gReqType = "APP";
    parent.gReqCode = parent.gAction;
    var radReqDOM = parent.buildRADXml();
    var bodyNode = selectSingleNode(radReqDOM, "//RAD_REQ_ENV/RAD_HEADER");
    ...
    if ((multRec[0] != "")) {
        ...
        var res = multRec[0].split("~");
        document.getElementById("LOV_STATUS_LOVDTLS").value = res[0];
        document.getElementById("EXTERNAL_LOV_NAME_LOVDTLS").value = res[1];
        document.getElementById("LOV_FUNC_NAME_LOVDTLS").value = res[2];
        var bind=res[3];
        bind= replaceAll(bind,",",",BIND");
        document.getElementById("COMBINED_DTLS_LOVDTLS").value = "~BIND"+bind+","; 
        document.getElementById("NO_DATA_FRM_EXT_SYS_PREF_LOVDTLS").value = res[4];
        document.getElementById("LOV_ID_OTHER_LOVDTLS").value = res[5];
...
function fnCombinedButton(){
	if(document.getElementById("LOV_STATUS_LOVDTLS").value=="C"){
		document.getElementById("POPLT_CMBD").style.display = "block";
		
	}else{
		document.getElementById("POPLT_CMBD").style.display = "none";
	}
}