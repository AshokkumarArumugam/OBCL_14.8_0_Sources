...
var restCallform="";
function fnRestArgs(e) {
	var evnt = window.event || e;
    var srcElement = getEventSourceElement(evnt);
    var act = srcElement.parentNode.parentNode.getElementsByTagName("INPUT")[1].value;
    actVal = act;
    parent.actVal = actVal;
    restCallform= srcElement.parentNode.parentNode.getElementsByTagName("INPUT")[6].value;
    loadSubScreenDIV("ChildWin", "RadRestArgs.jsp?Title=" + act);
}
...
function setTempHeaderNodes(dom) {
    ...
    //Setting Action Node To LOAD Starts
    setNodeText(selectSingleNode(dom, "//RAD_FUNCTIONS/ACTION"), "LOAD");
    //Setting Action Node To LOAD Ends
    ...
    if (getNodeText(selectSingleNode(dom, "//RAD_FUNCTIONS/FUNCTION_TYPE")) == "C" || getNodeText(selectSingleNode(dom, "//RAD_FUNCTIONS/FUNCTION_TYPE")) == "S") {
...
function TemproaryToModifyActions(dom) {
    ...
    for (var actN = 0;actN < actionNodes.length;actN++) {
		...
		if (selectSingleNode(actionNodes[actN], 'REST_OPERATION_ENABLED') == null) {
            var actStgElemnt = dom.createElement("REST_OPERATION_ENABLED");
            actionNodes[actN].appendChild(actStgElemnt);
        }
...
function fn_basic_valids(dom) {
	...
	var RadActions = selectNodes(dom, "//RAD_KERNEL//RAD_ACTIONS");
	if(RadActions.length > 0) {
		var nodevalue = getNodeText(selectSingleNode(RadActions[0], "OPERATION_ID"));
		var TypeXsd = getNodeText(selectSingleNode(RadActions[0], "RAD_XSD_TYPE_NAME"));
		if (nodevalue || TypeXsd) {
			try {
				// Validation for Xsd duplicates.
				var xsdname = fnValidateXSDCount();
				if (xsdname) {
					errMsg += "\n XSD Name Already Used in "+ xsdname +" ~";
				}
			} catch (e) {
			}
		}
...
var sumFunc_ID=funC_ID.substring(0,2)+"S"+funC_ID.substring(3);
var XsdNameQuery = "XSDNAMEQUERY"; 
var WhereString = "WHERE FUNCTION_ID NOT IN ('"+funC_ID+"','"+sumFunc_ID+"')  AND XSD_NAME IN ( " + Xsd_names + " )";
    try {
...
function fnCollectGlobalLov(dom) {
	...
	if (gLOVList == "") {
		...
		var lovs = gLOVList.substring(0, gLOVList.length - 1);
		gLOVList = " WHERE LOV_ID IN (" + lovs + ") AND FUNCTION_ID='COMMON'";
		parent.gReqCode = 'UICONTROLLER';
		parent.gReqType = "APP";
		var radReqDOM = parent.buildRADXml();
		var response = parent.fnPost(getXMLString(radReqDOM) + parent.gBodySeparator + "R019" + parent.gBodySeparator
				+ gLOVList, "RADClientHandler");
		if (response != "") {
			...
			alertMessage(lovs + " global LOVS are missing in Schema", "E");
		}
...
function loadSubScreenDIV(divId, src) {
	//Tool hangs if any duplicate field is added issue-starts here  bug#32499898
	try {
		var masker = document.getElementById("masker");
		if (masker.children[0].childElementCount > 0) {
			unmask();
			var chWin = document.getElementById("ChildWin");
			chWin.children[0].src = "";
			masker.children[0].removeChild(chWin);
		}
	} catch (ex) {
	}
	// End here
    ...
    if (src != "RadGenerate.jsp" && src != "RadMissingLabels.jsp" && src !="RadMissingComments.jsp") {
        if (document.getElementById("Dtop"))
            winObj.style.top = document.getElementById("Dtop").offsetHeight + 25 + "px";
        else 
            winObj.style.top = "190px"; // After generating the RAD, success message cannot be seen,It goes out of screen Issue. bug#32499898
        if (document.getElementById("Dleft"))
            winObj.style.left = document.getElementById("Dleft").offsetWidth + "px";
        else 
            winObj.style.left = "190px"; // After generating the RAD, success message cannot be seen,It goes out of screen Issue. bug#32499898
    }
    else {
...
function fnChkUnChkFlds(fldGroup) {
    if (fldGroup == "Front-End Files") {
        ...
        var incFlsList = "MENU_DETAILS~LABEL_DETAILS~AMEND_DETAILS~SUMMARY_DETAILS~SCREEN_DETAILS~LOV_DETAILS~BLOCK_PK_COLS~CALL_FORM_DETAILS~BLOCK_DETAILS~DATASCR_DETAILS~FUNCTION_CALL_FORMS~GATEWAY_DETAILS~VARIABLE_MAPPING~NOTIFICATION_DETAILS~FUNCTION_PARAMETERS~NOTIFICATION_TRIGGER~XSD_DETAILS";
...
function fnSetDefaultvals(NewDOM, mainNode) {
    ...
    var relNodeTemp = selectNodes(NewDOM, "//" + mainNode);
    if (relNodeTemp && relNodeTemp.length > 0) {
        ...
        for (var bkn = 0;bkn < blkKerNodes.length;bkn++) {
            ...
            for (var bkf = 0;bkf < blkkerblkflds.length;bkf++) {
				...
				//VINIT
				 ...
				 if (selectSingleNode(blkkerblkflds[bkf], "MAX_VAL") == null) {
	                    var blkfldann = NewDOM.createElement("MAX_VAL");
	                    blkkerblkflds[bkf].appendChild(blkfldann);
	                    setNodeText(selectSingleNode(blkkerblkflds[bkf], "MAX_VAL"), "");
	                }
				//AMIT
				 if (selectSingleNode(blkkerblkflds[bkf], "HOTKEYREQJA") == null) {
                    var blkfldann = NewDOM.createElement("HOTKEYREQJA");
                    blkkerblkflds[bkf].appendChild(blkfldann);
                    setNodeText(selectSingleNode(blkkerblkflds[bkf], "HOTKEYREQJA"), "N");
                }
				//AMIT                                
                ...
                if (selectSingleNode(blkkerblkflds[bkf], "CHK_TRIM") == null) {
                    var blkfldann = NewDOM.createElement("CHK_TRIM");
                    blkkerblkflds[bkf].appendChild(blkfldann);
                    setNodeText(selectSingleNode(blkkerblkflds[bkf], "CHK_TRIM"), "N");
                }
		...
		var fstFlds_inv = selectNodes(NewDOM, "//" + mainNode + "/RAD_FIELDSETS[FIELDSET_VISIBLE='N']/FIELDSET_FIELDS");
		 for (var ffl = 0;ffl < fstFlds_inv.length;ffl++) {
		 try{
		 setNodeText(selectSingleNode(fstFlds_inv[ffl], "FIELD_ORDER"), "0");
		 setNodeText(selectSingleNode(fstFlds_inv[ffl], "ACTIVE"), "N");
		 }
		 catch(e){}
		 }
        ...
        for (var scn = 0;scn < scrns.length;scn++) {
			...
			if (selectSingleNode(scrns[scn], "SCREEN_QUERYREQ") == null) {
                var scrVis = NewDOM.createElement("SCREEN_QUERYREQ");
                scrns[scn].insertBefore(scrVis, selectSingleNode(scrns[scn], "HEADER"));
                setNodeText(selectSingleNode(scrns[scn], "SCREEN_QUERYREQ"), "Y");
            }
			if (selectSingleNode(scrns[scn], "IMGFLD_QUERYREQ") == null) {
                var scrVis = NewDOM.createElement("IMGFLD_QUERYREQ");
                scrns[scn].insertBefore(scrVis, selectSingleNode(scrns[scn], "HEADER"));
                setNodeText(selectSingleNode(scrns[scn], "IMGFLD_QUERYREQ"), "N");
            }
        ...
        if (funcPrfs.length > 0) {
			...
			if (selectSingleNode(funcPrfs[0], "SRC_BLOCK_NAME") == null) {
                var vcsFld = NewDOM.createElement("SRC_BLOCK_NAME");
                funcPrfs[0].insertBefore(vcsFld, selectSingleNode(funcPrfs[0], "TANK_MODIFICATIONS"));
            }
            if (selectSingleNode(funcPrfs[0], "SRC_FIELD_NAME") == null) {
                var vcsFld = NewDOM.createElement("SRC_FIELD_NAME");
                funcPrfs[0].insertBefore(vcsFld, selectSingleNode(funcPrfs[0], "TANK_MODIFICATIONS"));
            }
	    	...
	    	if (selectSingleNode(funcPrfs[0], "JAVA_PAYEMNTS_FUNCTION") == null) {
                var vcsFld = NewDOM.createElement("JAVA_PAYEMNTS_FUNCTION");
                funcPrfs[0].insertBefore(vcsFld, selectSingleNode(funcPrfs[0], "TANK_MODIFICATIONS"));
            }
...
//vinit 
            ...
            if (selectSingleNode(lnfrm[0], "LOV_STATUS") == null) {
                var relTyp = NewDOM.createElement("LOV_STATUS");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "LOV_STATUS"), "I");
            }
            if (selectSingleNode(lnfrm[0], "EXTERNAL_LOV_NAME") == null) {
                var relTyp = NewDOM.createElement("EXTERNAL_LOV_NAME");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "EXTERNAL_LOV_NAME"), "");
            }
            if (selectSingleNode(lnfrm[0], "LOV_FUNC_NAME") == null) {
                var relTyp = NewDOM.createElement("LOV_FUNC_NAME");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "LOV_FUNC_NAME"), "");
            }
            if (selectSingleNode(lnfrm[0], "COMBINED_QUERY_DTLS") == null) {
                var relTyp = NewDOM.createElement("COMBINED_QUERY_DTLS");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "COMBINED_QUERY_DTLS"), "");
            }
            if (selectSingleNode(lnfrm[0], "COMBINED_DTLS") == null) {
                var relTyp = NewDOM.createElement("COMBINED_DTLS");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "COMBINED_DTLS"), "");
            }
            if (selectSingleNode(lnfrm[0], "NO_DATA_FRM_EXT_SYS_PREF") == null) {
                var relTyp = NewDOM.createElement("NO_DATA_FRM_EXT_SYS_PREF");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "NO_DATA_FRM_EXT_SYS_PREF"), "");
            }
            if (selectSingleNode(lnfrm[0], "LOV_ID_OTHER") == null) {
                var relTyp = NewDOM.createElement("LOV_ID_OTHER");
                lnfrm[0].appendChild(relTyp);
                setNodeText(selectSingleNode(lnfrm[0], "LOV_ID_OTHER"), "");
            }
        ...
        for (var j = 0;j < screens.length;j++) {
            ...
            if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/UPLOAD_TABLE") == null) {
                ...
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
			...
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_BLK_NAME") == null) {
                var lname = NewDOM.createElement("REST_BLK_NAME");
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
                //selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_BLK_NAME"), "");
            }
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_RELATION_TYPE") == null) {
                var lname = NewDOM.createElement("REST_RELATION_TYPE");
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
                //selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_RELATION_TYPE"), "");
            }
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_PK_COLS") == null) {
                var lname = NewDOM.createElement("REST_PK_COLS");
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
                //selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_PK_COLS"), "");
            }
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_PK_TYPES") == null) {
                var lname = NewDOM.createElement("REST_PK_TYPES");
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
                //selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_PK_TYPES"), "");
            }
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_RELATION_WITH_PARENT") == null) {
                var lname = NewDOM.createElement("REST_RELATION_WITH_PARENT");
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
                //selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_RELATION_WITH_PARENT"), "");
            }if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_XSD_NODE") == null) {
                var lname = NewDOM.createElement("REST_XSD_NODE");
                screens[j].insertBefore(lname, selectSingleNode(screens[j], "DATASRC_TYPE"));
                //selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_DATASOURCES[DATASRC_NAME='" + datasrcnm + "']/REST_XSD_NODE"), "");
            }
		...
		if (mainNode == "RAD_KERNEL" && selectSingleNode(NewDOM, "//" + "RAD_KERNEL").childNodes.length > 0 || mainNode != "RAD_KERNEL" && selectSingleNode(NewDOM, "//" + "RAD_KERNEL").childNodes.length == 0) {
			if (selectSingleNode(NewDOM, "//" + mainNode) != null
					&& selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS") == null) {
				var lname = NewDOM.createElement("RAD_ACTIONS");
				selectSingleNode(NewDOM, "//" + mainNode).appendChild(lname);
				setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS"), "");
			}
			if (selectSingleNode(NewDOM, "//" + mainNode) != null
					&& selectSingleNode(NewDOM, "//" + mainNode + "/RAD_REST_ACTIONS") == null) {
				var lname = NewDOM.createElement("RAD_REST_ACTIONS");
				selectSingleNode(NewDOM, "//" + mainNode).appendChild(lname);
			}
			if (selectSingleNode(NewDOM, "//" + mainNode) != null
					&& selectSingleNode(NewDOM, "//" + mainNode + "/RAD_REST_ACTIONS/REST_SERVICE_NAME") == null) {
				var lname = NewDOM.createElement("REST_SERVICE_NAME");
				selectSingleNode(NewDOM, "//" + mainNode + "/RAD_REST_ACTIONS").appendChild(lname);
			}
		}
        ...
        for (var j = 0;j < screens.length;j++) {
			...
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS/REST_SERVICE_NAME") == null) {
                var lname = NewDOM.createElement("REST_SERVICE_NAME");
                selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS/REST_SERVICE_NAME"), "");
            }
			...
			if (selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS/XSD_MODULE_FOLDER") == null) {
                var lname = NewDOM.createElement("XSD_MODULE_FOLDER");
                selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS").appendChild(lname);
                setNodeText(selectSingleNode(NewDOM, "//" + mainNode + "/RAD_ACTIONS/XSD_MODULE_FOLDER"), "");
            }
...
function checkIdentifier(identifier, fieldIn) {
	if( identifier && !identifier.match(/^[a-zA-Z_]+\w*$/)){
		alertMessage("No special characters are allowed and first letter should be alphabet in "+fieldIn, "E");
	}
}
