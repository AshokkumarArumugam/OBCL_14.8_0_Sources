...
function Loadradxml() {
    ...
    if (parent.vwChg == 'Y') {
		...
		if (document.getElementsByName("FUNCTION_CATEGORY")[0].value == "MASTER") {
				fnAllowUI_MasterOnly();
			} 
			if (document.getElementsByName("FUNCTION_CATEGORY")[0].value == "SUBSYSTEM") {
				fnAllowUI_SubSystemOnly();
			} 
    }
    return true;
}
        ...
        if (resDtScr == "1") {
            ...
            if (parent.chngUIFlg != "Y") {
				...
				if (document.getElementById("ELCM_FUNCTION").checked)  
                  response = CallCodeGenerator(radReqDOM, getXMLString(Elcm_dom), frntndFiles); 
				else
