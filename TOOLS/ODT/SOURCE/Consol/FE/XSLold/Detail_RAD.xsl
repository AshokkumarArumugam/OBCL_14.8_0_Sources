...
<xsl:template match="SCREEN">
	...
	<xsl:apply-templates select="HEADER/PAGE"></xsl:apply-templates>
      <!--added some space after the Header incase of absoulte position. !-->
      <DIV id="space" style="width:97%;height:2px;"></DIV>
       ...
       <DIV CLASS="absDIVPage" ID="TBLPage{@ID}">
            ...
            <xsl:if test="position()!=1">
                <xsl:attribute name="style">
                    <xsl:text>display:none;width:90%</xsl:text>
                </xsl:attribute>
	    	</xsl:if>
...
<xsl:template name="AbsolutePageHandler">
    <xsl:param name="curr_page" select="." />
 	...
 	<xsl:apply-templates select="/FORM/BLOCK[@SCREEN=$screen and @TYPE !='Audit Entry' and ID != 'BLK_BLOCK_BUTTONS']" >
...
<xsl:param name="curr_blk" select="." />
    ...
    <!--<DIV CLASS="DIVMultiple" NAME="dataContainer" ID="SYS_{$curr_blk/ID}"  STYLE="overflow:scroll;width:{$curr_blk/WIDTH};height:{$curr_blk/HEIGHT};">--><!--ugi commented-->

    <DIV STYLE="overflow:hidden;height:150px;">

    <DIV CLASS="DIVMultiple" NAME="dataContainer" id="DIV_{$curr_blk/ID}"  STYLE="overflow:scroll;width:{$curr_blk/WIDTH};height:{$curr_blk/HEIGHT};">
    <!--ugi added for the multiple entry abs pos in layout controls-->
   		<xsl:if test="$gPosition='absolute'">
			<xsl:attribute name="STYLE">
				<xsl:text>{position:absolute;overflow:scroll;left:</xsl:text>
				<xsl:value-of select="$col" />
				<xsl:text>px;top:</xsl:text>
				<xsl:value-of select="$row" />
				<xsl:text>px;width:800px;</xsl:text>
...
<xsl:template match="BLOCK" mode="AuditEntry">
   	<NOBR>
        ...
        <!--<DIV CLASS="DivAudit" id="SYS_{ID}">ugi added-->
	<DIV CLASS="DivAudit" id="DIV_{ID}"><!--ugi added for audit entry in layout controls-->
			<xsl:attribute name="STYLE">
...
-->
  ...
  <xsl:template match="HEADER/PAGE">
    <!-- Call page handler hardcoded as "Header" that displays fields in all tab pages as header -->
    <xsl:if test="$gPosition='absolute'">
      <DIV CLASS="absDIVHdr" ID="TBLPage{@ID}"
           style="HEIGHT:{@HEIGHT};left:10px;top:0px;">
        <TABLE style="width:97%;height:100%;border-bottom:2px solid #3C5494;"
    ...
    <xsl:if test="$gPosition!='absolute'">
      <table style="width:100%">
        <tr>
          <td>
            <TABLE CLASS="colTABLEHdr" ID="TBLPage{@ID}" cellpadding="2"
                   cellspacing="0">
              <xsl:call-template name="ColumnPageHandler">
                <xsl:with-param name="curr_page">
                  <xsl:value-of select="@NAME"/>
        ...
        <tr bgColor="#3C5494" style="width:100%;height:3px;">
          <td>
          </td>
        </tr>
      </table>
