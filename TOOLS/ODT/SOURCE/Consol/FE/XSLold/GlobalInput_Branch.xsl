...
<xsl:template match="FIELD/TYPE[text()='TEXT' or text()='AMOUNT' or text()='DATE' or text()='ACCOUNT' or text()='BRANCH' or text()='CURRENCY' or text()='CUSTOMER' or text()='MASK']">
   	...
   	<xsl:if test="$gPosition!='absolute'">
	    <TD CLASS="colTDText">	
		<!-- yugandhar added this for hidden fields-->
	    	<xsl:if test="count(../HIDDEN)&gt;=1 and ../HIDDEN = -1">
	    		<xsl:attribute name="CLASS">
...
<xsl:template name="dispAmountField">
	...
	<INPUT TYPE="TEXT" CLASS="TextAmount" onactivate="acceptInputAmount('{../NAME}', '{../RELATED_FIELD}')" onbeforedeactivate="validateInputAmount('{../NAME}', '{../RELATED_FIELD}')" >
...
<xsl:template name="dispDateField">
	...
	<INPUT TYPE="TEXT" CLASS="TextDate"  onactivate="acceptInputDate('{../NAME}')" onbeforedeactivate="validateInputDate('{../NAME}')" >
	    ...
	    <!-- Jijesh added for REFFIELD -->
		<xsl:if test="../REFFIELD !=''">
  			<xsl:attribute name="REFFIELD">
				<xsl:value-of select="../REFFIELD" />
   			</xsl:attribute>
		</xsl:if>
...
<xsl:template name="dispMaskField">
    ...
    <INPUT TYPE="TEXT" CLASS="TextNormal" mask="{../MASK}" onactivate="acceptInputValue('{../NAME}')" onbeforedeactivate="validateInputValue('{../NAME}');">
	    ...
	    <xsl:if test="count(../POPUPEDIT) &gt; 0 ">
		    ...
		    <BUTTON CLASS="BTNPopupEdit" onmouseover="this.className='BTNPopupEditOver'" onmouseout="this.className='BTNPopupEdit'">
		        <xsl:if test="count(../HIDDEN) &gt; 0 and ../HIDDEN = -1">
		  		    <xsl:attribute name="CLASS">hidden</xsl:attribute>
		        </xsl:if>
...
<xsl:template name="dispTextField">
	...
	<xsl:if test="../TYPE='TEXT'and ../DTYPE='NUMERIC'">
		<xsl:attribute name="style">
		...
		<xsl:if test="count(../UPPERCASE) &gt; 0 and ../UPPERCASE = -1">
...
<xsl:template match="FIELD/TYPE[text()='SELECT']">
   	...
   	<xsl:if test="$gPosition!='absolute'">
	    <TD CLASS="colTDList" >
			<!-- yugandhar added this for hidden select fields-->
	    	<xsl:if test="count(../HIDDEN)&gt;=1 and ../HIDDEN = -1">
	    		<xsl:attribute name="CLASS">
	    			<xsl:text>DispNone</xsl:text>
	    		</xsl:attribute>
...
<xsl:template match="FIELD/TYPE[text()='RADIO']">
   	...
   	<xsl:if test="$gPosition!='absolute'">
	    <TD CLASS="colTDRadio" >
		<!-- yugandhar added this for hidden radio fields-->
	    	<xsl:if test="count(../HIDDEN)&gt;=1 and ../HIDDEN = -1">
	    		<xsl:attribute name="CLASS">
	    			<xsl:text>DispNone</xsl:text>
	    		</xsl:attribute>
...
<xsl:template match="FIELD/TYPE[text()='CHECKBOX']">
	...
	<xsl:attribute name="CLASS">colTDTextmultiple</xsl:attribute></xsl:if>
		<!-- yugandhar added this for hidden checkbox fields-->
	    	<xsl:if test="count(../HIDDEN)&gt;=1 and ../HIDDEN = -1">
	    		<xsl:attribute name="CLASS">
	    			<xsl:text>DispNone</xsl:text>
	    		</xsl:attribute>
...
<xsl:template match="FIELD/TYPE[text()='TEXTAREA']">
   	...
   	<xsl:if test="$gPosition!='absolute'">
	    <TD CLASS="colTDTextarea" >
		<!-- yugandhar added this for hidden text area fields-->
	    	<xsl:if test="count(../HIDDEN)&gt;=1 and ../HIDDEN = -1">
	    		<xsl:attribute name="CLASS">
	    			<xsl:text>DispNone</xsl:text>
	    		</xsl:attribute>
...
<xsl:param name="curr_fld" select="." />
        ...
        <xsl:if test="count($curr_fld/READ_ONLY) &gt; 0 and $curr_fld/READ_ONLY = -1">
      	    ...
      	    <xsl:if test="$curr_fld/TYPE[text() = 'DATE']">
      	    	<xsl:attribute name="style">
      	    		<xsl:text>text-align: right;</xsl:text>
      	    	</xsl:attribute>
      	    </xsl:if>
	 ...
	 <!-- Ravi - Tab index, Required property must be set for visible field -->
...
<xsl:template name="LovHandler">
    ...
    <xsl:if test="count($curr_fld/LOV) &gt; 0 ">
		<!-- Display Button -->
	    <!--<BUTTON TABINDEX="-1" CLASS="ButtonLov" ONCLICK="{$curr_fld/LOV/NAME}.show_lov('{$curr_fld/LOV/RET_FLDS}','{$curr_fld/LOV/FORM_NAME}','{$curr_fld/LOV/BIND_VARS}', '{$curr_fld/LOV/TITLE}', '{$curr_fld/LOV/COL_HEADING}', '{$curr_fld/LOV/REDUCTION_FLD_LABELS}')">-->
	     
	    <!--BUTTON TABINDEX="-1" CLASS="ButtonLov" ONCLICK="{$curr_fld/LOV/NAME}.show_lov('{$curr_fld/LOV/RET_FLDS}','{$curr_fld/LOV/FORM_NAME}','{$curr_fld/LOV/BIND_VARS}', '{$curr_fld/LOV/TITLE}', '{$curr_fld/LOV/COL_HEADING}', '{$curr_fld/LOV/REDUCTION_FLD_LABELS}')" 
	    	onmouseover="this.className='ButtonLovOver'" onmouseout="this.className='ButtonLov'" -->
	    
	    <BUTTON TABINDEX="-1" CLASS="ButtonLov" ONCLICK="LovHandler({$curr_fld/LOV/NAME}_ONLINE,{$curr_fld/LOV/NAME}_OFFLINE, '{$curr_fld/LOV/RET_FLDS}','{$curr_fld/LOV/FORM_NAME}','{$curr_fld/LOV/BIND_VARS}', '{$curr_fld/LOV/TITLE}', '{$curr_fld/LOV/COL_HEADING}', '{$curr_fld/LOV/REDUCTION_FLD_LABELS}')" 
	    	onmouseover="this.className='ButtonLovOver'" onmouseout="this.className='ButtonLov'" >
	    <!-- Three new parameters for the LOV override added. Senthil L 02 Mar 2005-->
