...
<!--  sundar May 5...for alias integration -->
      ...
      <!-- Data XML Start -->
      <xsl:if test="count($dsNode/DATA_ROWNO) &gt;  0">
      <xsl:if test="$dsNode/RELATION_TYPE != 'N'">              
      <xsl:apply-templates select="./RAD_BLK_FIELDS[FIELD_TYPE != 'LABEL']"/>      
      </xsl:if>     
     <xsl:if test="$dsNode/RELATION_TYPE = 'N'">       
      <xsl:for-each select="$dsNode/DATA_ROWNO">
       <DATA_ROWNO>
      <xsl:apply-templates select="//RAD_BLK_FIELDS[FIELD_TYPE != 'LABEL']"/>
       </DATA_ROWNO>      
      </xsl:for-each>  
...
<!--
	...
	<LABEL_LINK>
		   <xsl:if test="normalize-space($currNode/LABEL_LINK) != ''">              
        <xsl:value-of select="$currNode/LABEL_LINK"/>
      </xsl:if>
      <xsl:if test="normalize-space($currNode/LABEL_LINK) = ''">
        <xsl:if test="normalize-space(//RAD_FIELDS[@Identifier=$currNode/@Identifier]/LABEL_LINK) != ''">
          <xsl:variable name="labelLink" select="//RAD_FIELDS[@Identifier=$currNode/@Identifier]/LABEL_LINK"/>
          <xsl:value-of select="$labelLink"/>
        </xsl:if>
      </xsl:if>
	</LABEL_LINK>
...
<!--            </xsl:if> -->
  ...
  <!-- sundar ends here -->    
            <xsl:if test="normalize-space($relatedFld) != ''">
              <RELATED_FIELD>
...
<!--              <DEF_VALUE><xsl:value-of select="//RAD_FIELDS[@Identifier=$Identifier]/DEFAULT_VALUE"/></DEF_VALUE> -->
            ...
            <xsl:variable name="dataSrcNode"
                          select="//RAD_DATASOURCES[RAD_FIELDS[@Identifier=$Identifier]]"/>
            <xsl:variable name="aliasName" select="$dataSrcNode/DATASRC_ALIAS"/>
<!--            
