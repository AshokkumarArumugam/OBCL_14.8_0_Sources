...
if [ -e GenericDBCompileRunChk.flg ]
then
...
(
	...
	export CLASSPATH=./GenericExecDBCompile.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.generic.Generic_ExecDBCompile "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
)
