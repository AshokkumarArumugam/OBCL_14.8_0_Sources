...
(
	if [ -f "SchedulerEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./SchedulerEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.scheduler.Scheduler_InfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
