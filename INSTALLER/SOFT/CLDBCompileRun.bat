...
IF EXIST CLDBCompileChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the CLDBCompileChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	set CLASSPATH=CLExecDBCompile.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.cl.CL_ExecDBCompile %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file CLDBCompileChk.flg.
	DEL CLDBCompileChk.flg
)
