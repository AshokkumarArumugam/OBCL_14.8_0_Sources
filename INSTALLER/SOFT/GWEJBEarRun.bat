...
(
	...
	IF EXIST GWEJBEarCreation.jar (
	set CLASSPATH=GWEJBEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.Gateway.GWEJB_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GWEJBEarRunChk.flg.
	DEL GWEJBEarRunChk.flg
	) ELSE (
