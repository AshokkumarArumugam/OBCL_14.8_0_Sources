...
if [ -e GWMDBEarRunChk.flg ]
then
	echo "Installer is already running."
	echo "Please manually delete the GWMDBEarRunChk.flg file in order to continue."
	echo "."	
	exit
fi
...
(
    if [ -f "GWMDBEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./GWMDBEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.Gateway.GWMDB_InfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
	echo "Error!! GWMDBEarCreation.jar does not exist.It is required proceed further."
	EXITINSTALLER
	fi
)
...
(
	echo "in exit"
	echo "Deleting the Flag file GWMDBEarRunChk.flg."
	rm GWMDBEarRunChk.flg
