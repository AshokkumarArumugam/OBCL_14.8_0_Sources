...
IF EXIST GenericEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the GenericEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST GenericEarCreation.jar (
	set CLASSPATH=GenericEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.generic.Generic_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GenericEarRunChk.flg.
	DEL GenericEarRunChk.flg
	) ELSE (
	ECHO Error!! GenericEarCreation.jar does not exist.It is required to proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER  )
)
