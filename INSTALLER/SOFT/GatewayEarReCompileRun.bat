...
IF EXIST GatewayReCompileEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the GatewayReCompileEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
REM. > GatewayReCompileEarRunChk.flg
...
(
	...
	set CLASSPATH=GatewayReCompileEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.Gateway.GatewayReCompile %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GatewayReCompileEarRunChk.flg.
	DEL GatewayReCompileEarRunChk.flg
)
...
:EXITINSTALLER
	...
	ECHO @@@@@@ Restarting the INSTALLER @@@@@@@
	cd %~dp0
	CALL GatewayReCompileEarRun.bat
	)
