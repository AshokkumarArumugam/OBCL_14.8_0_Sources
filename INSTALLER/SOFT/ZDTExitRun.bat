...
(
	...
	IF EXIST ZDTExitRun.jar (
	set CLASSPATH=ZDTExitRun.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.common.ZDTExitRun %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file ZDTExitRunChk.flg.
	DEL ZDTExitRunChk.flg
		) ELSE (
