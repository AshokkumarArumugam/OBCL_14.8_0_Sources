...
IF EXIST ZDTLoginSwitchRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the ZDTLoginSwitchRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST ZDTLoginSwitchRun.jar (
	set CLASSPATH=ZDTLoginSwitchRun.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.common.ZDTLoginSwitchRun %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file ZDTLoginSwitchRunChk.flg.
	DEL ZDTLoginSwitchRunChk.flg
		) ELSE (
	ECHO Error!! ZDTLoginSwitchRun.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
