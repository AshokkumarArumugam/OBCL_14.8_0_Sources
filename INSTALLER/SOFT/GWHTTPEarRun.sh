...
if [ -e GWHTTPEarRunChk.flg ]
then
	echo "Installer is already running."
	echo "Please manually delete the GWHTTPEarRunChk.flg file in order to continue."
	echo "."	
	exit
fi
...
(
    if [ -f "GWHTTPEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./GWHTTPEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.Gateway.GWHTTP_InfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
	echo "Error!! GWHTTPEarCreation.jar does not exist.It is required proceed further."
	EXITINSTALLER
	fi
)
...
(
	echo "in exit"
	echo "Deleting the Flag file GWHTTPEarRunChk.flg."
	rm GWHTTPEarRunChk.flg
