CREATE OR REPLACE PROCEDURE pr_instlr_post_import(p_err_code  OUT VARCHAR2
...
BEGIN
    ...
    LOOP
        l_sql := 'Alter Table ' || rec.table_name || ' Enable all triggers';
        BEGIN
    ...
    --Cyclic Compilation
    pr_instlr_cyclic_compile(p_err_code,p_err_param);
