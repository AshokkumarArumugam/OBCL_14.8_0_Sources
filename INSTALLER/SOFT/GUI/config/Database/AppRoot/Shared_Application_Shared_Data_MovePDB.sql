/*  Script for Shared Application + Shared Data excluding PDB functions */
SET VERIFY ON
SET HEAD ON
SET FEEDBACK 1
SET ARRAY 1
	...
	UPDATE smtb_menu
	 ...
	 WHERE smtb_menu.function_id IN
		   (SELECT function_id
			 ...
			 WHERE function_id NOT IN ('&FUNCTIONID_GROUP')	--> based on the input
			UNION
			SELECT summary_fn_id
			  FROM cstm_approot_functions_menu
			 WHERE function_id NOT IN ('&FUNCTIONID_GROUP')	--> based on the input
			   AND summary_fn_id IS NOT NULL
			);
END;
/
BEGIN
  FOR I IN (SELECT 'BEGIN ' || chr(10) || 'DBMS_PDB.SET_DATA_LINKED(''&P_APPROOT_USER''' ||
			...
			WHERE object_name IN (SELECT DISTINCT a.object_name
                                ...
                                WHERE sharing = 'DL'
								AND function_id NOT IN ('&FUNCTIONID_GROUP')	--> based on the input
