...
IF EXIST ODTDBCompileChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the ODTDBCompileChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST ODTExecDBCompile.jar (
	set CLASSPATH=ODTExecDBCompile.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.tools.ODT_ExecDBCompile %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file ODTDBCompileChk.flg.
	DEL ODTDBCompileChk.flg
		) ELSE (
	ECHO Error!! ODTExecDBCompile.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
