...
if [ -e COREGatewayRestEarRunChk.flg ]
then
	echo "Installer is already running."
	echo "Please manually delete the COREGatewayRestEarRunChk.flg file in order to continue."
	echo "."	
	exit
fi
#echo "Creating a new flag file....."
> COREGatewayRestEarRunChk.flg
...
#Bug#31876599 
ojdbc=$( ls |find ojdbc*[0-9].jar|sort -k1.6n|tail -1)
cd $CURRDIR
...
(
	if [ -f "COREGatewayRestEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./COREGatewayRestEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.core.CORE_GatewayRestInfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
	echo "Error!! COREGatewayRestEarCreation.jar does not exist.It is required proceed further."
	EXITINSTALLER
	fi
)
...
(
	echo "in exit"
	echo "Deleting the Flag file COREGatewayRestEarRunChk.flg."
	rm COREGatewayRestEarRunChk.flg
