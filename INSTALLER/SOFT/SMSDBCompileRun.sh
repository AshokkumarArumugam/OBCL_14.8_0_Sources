...
(	
	...
	if [ -f $ORACLE_HOME/jdbc/lib/$ojdbc ]; then
	export CLASSPATH=$ORACLE_HOME/jdbc/lib/$ojdbc:$JAVA_HOME/lib/tools.jar:$CLASSPATH
	fi
...
(
	...
	export CLASSPATH=./SMSExecDBCompile.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.sms.SMS_ExecDBCompile "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
)
