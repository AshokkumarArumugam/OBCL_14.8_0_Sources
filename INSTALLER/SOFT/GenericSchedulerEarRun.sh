...
if [ -e GenericSchedulerEarRunChk.flg ]
then
...
(
	if [ -f "GenericSchedulerEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./GenericSchedulerEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.generic.GenericScheduler_InfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
...
(
	...
	rm GenericSchedulerEarRunChk.flg
