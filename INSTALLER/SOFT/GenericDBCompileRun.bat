...
IF EXIST GenericDBCompileRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the GenericDBCompileRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
REM. > GenericDBCompileRunChk.flg
...
(
	...
	IF EXIST GenericExecDBCompile.jar (
	set CLASSPATH=GenericExecDBCompile.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.generic.Generic_ExecDBCompile %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GenericDBCompileRunChk.flg.
	DEL GenericDBCompileRunChk.flg
		) ELSE (
	ECHO Error!! GenericExecDBCompile.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
