...
if [ -e CLGatewayRestEarRunChk.flg ]
then
...
(
	if [ -f "CLGatewayRestEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./CLGatewayRestEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.cl.CL_GatewayRestInfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
