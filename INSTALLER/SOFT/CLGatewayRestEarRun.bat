...
IF EXIST CLGatewayRestEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the CLGatewayRestEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
REM. > CLGatewayRestEarRunChk.flg
...
(
	...
	IF EXIST CLGatewayRestEarCreation.jar (
	set CLASSPATH=CLGatewayRestEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.cl.CL_GatewayRestInfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file CLGatewayRestEarRunChk.flg.
	DEL CLGatewayRestEarRunChk.flg
		) ELSE (
	ECHO Error!! CLGatewayRestEarCreation.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
