...
IF EXIST ZDTStageUpdateRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the ZDTStageUpdateRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST ZDTStageUpdateRun.jar (
	set CLASSPATH=ZDTStageUpdateRun.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.common.ZDTStageUpdateRun %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file ZDTStageUpdateRunChk.flg.
	DEL ZDTStageUpdateRunChk.flg
		) ELSE (
	ECHO Error!! ZDTStageUpdateRun.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
