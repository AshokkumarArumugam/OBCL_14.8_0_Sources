...
IF EXIST SchedulerEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the SchedulerEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST SchedulerEarCreation.jar (
	set CLASSPATH=SchedulerEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.scheduler.Scheduler_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file SchedulerEarRunChk.flg.
	DEL SchedulerEarRunChk.flg
		) ELSE (
	ECHO Error!! SchedulerEarCreation.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
