...
if [ -e GWEJBEarRunChk.flg ]
then
	echo "Installer is already running."
	echo "Please manually delete the GWEJBEarRunChk.flg file in order to continue."
	echo "."	
	exit
fi
...
(
    if [ -f "GWEJBEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./GWEJBEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.Gateway.GWEJB_InfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
	echo "Error!! GWEJBEarCreation.jar does not exist.It is required proceed further."
	EXITINSTALLER
	fi
)
...
(
	echo "in exit"
	echo "Deleting the Flag file GWEJBEarRunChk.flg."
	rm GWEJBEarRunChk.flg
