...
IF EXIST CLGatewayEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the CLGatewayEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	set CLASSPATH=CLGatewayEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.cl.CL_GatewayInfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file CLGatewayEarRunChk.flg.
	DEL CLGatewayEarRunChk.flg
)
