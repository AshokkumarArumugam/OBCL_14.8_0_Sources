...
if [ -e COREGatewayEarRunChk.flg ]
then
	echo "Installer is already running."
	echo "Please manually delete the COREGatewayEarRunChk.flg file in order to continue."
	echo "."	
	exit
fi
#echo "Creating a new flag file....."
> COREGatewayEarRunChk.flg
...
if [ "$USERDIR" == '' ]
then
echo "Copying env.properties to currDir/logs NOt Required"
else
echo "Copying env.properties to currDir/logs"
echo $USERDIR
cp $USERDIR logs
fi
echo "."
...
(
    if [ -f "COREGatewayEarCreation.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./COREGatewayEarCreation.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.core.CORE_GatewayInfraBuild "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
	echo "Error!! COREGatewayEarCreation.jar does not exist.It is required proceed further."
	EXITINSTALLER
	fi
)
...
(
	echo "in exit"
	echo "Deleting the Flag file COREGatewayEarRunChk.flg."
	rm COREGatewayEarRunChk.flg
