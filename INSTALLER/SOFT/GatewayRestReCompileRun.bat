...
IF EXIST GatewayRestReCompileRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the GatewayRestReCompileRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
REM. > GatewayRestReCompileRunChk.flg
...
(
	...
	set CLASSPATH=GatewayRestReCompile.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.Gateway.GatewayRestReCompile %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GatewayRestReCompileRunChk.flg.
	DEL GatewayRestReCompileRunChk.flg
)
...
:EXITINSTALLER
	...
	ECHO @@@@@@ Restarting the INSTALLER @@@@@@@
	cd %~dp0
	CALL GatewayRestReCompileRun.bat
	)
