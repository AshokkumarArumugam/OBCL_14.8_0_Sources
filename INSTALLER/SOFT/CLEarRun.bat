...
IF EXIST CLEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the CLEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	set CLASSPATH=CLEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.cl.CL_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file CLEarRunChk.flg.
	DEL CLEarRunChk.flg
)
