...
IF EXIST ZDTInitRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the ZDTInitRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST ZDTInitRun.jar (
	set CLASSPATH=ZDTInitRun.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.common.ZDTInitRun %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file ZDTInitRunChk.flg.
	DEL ZDTInitRunChk.flg
		) ELSE (
	ECHO Error!! ZDTInitRun.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
