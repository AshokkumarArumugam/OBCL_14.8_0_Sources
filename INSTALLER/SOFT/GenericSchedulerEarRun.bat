...
IF EXIST GenericSchedulerEarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the GenericSchedulerEarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
REM. > GenericSchedulerEarRunChk.flg
...
(
	...
	IF EXIST GenericSchedulerEarCreation.jar (
	set CLASSPATH=GenericSchedulerEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.generic.GenericScheduler_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GenericSchedulerEarRunChk.flg.
	DEL GenericSchedulerEarRunChk.flg
		) ELSE (
	ECHO Error!! GenericSchedulerEarCreation.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
