...
IF EXIST ODTWarRunChk.flg (
	ECHO Installer is already running.
	ECHO Please manually delete the ODTWarRunChk.flg file in order to continue.
	ECHO.
	PAUSE
	EXIT
)
...
(
	...
	IF EXIST ODTWarCreation.jar (
	set CLASSPATH=ODTWarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.tools.ODT_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file ODTWarRunChk.flg.
	DEL ODTWarRunChk.flg
		) ELSE (
	ECHO Error!! ODTWarCreation.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
