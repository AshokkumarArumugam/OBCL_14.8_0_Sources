...
IF EXIST COREGatewayRestEarRunChk.flg (
...
REM. > COREGatewayRestEarRunChk.flg
...
(
	...
	IF EXIST COREGatewayRestEarCreation.jar (
	set CLASSPATH=COREGatewayRestEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.core.CORE_GatewayRestInfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file COREGatewayRestEarRunChk.flg.
	DEL COREGatewayRestEarRunChk.flg
		) ELSE (
...
:EXITINSTALLER
	...
	ECHO @@@@@@ Restarting the INSTALLER @@@@@@@
	cd %~dp0
	CALL COREGatewayRestEarRun.bat
	)
