...
(
	...
	IF EXIST GWHTTPEarCreation.jar (
	set CLASSPATH=GWHTTPEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.Gateway.GWHTTP_InfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file GWHTTPEarRunChk.flg.
	DEL GWHTTPEarRunChk.flg
	) ELSE (
