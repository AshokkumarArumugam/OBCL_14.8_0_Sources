...
IF EXIST COREGatewayEarRunChk.flg (
...
(   
	...
	IF EXIST COREGatewayEarCreation.jar (
	set CLASSPATH=COREGatewayEarCreation.jar;%CLASSPATH%
	...
	java %JAVA_DEBUG% com.ofss.genericsoftinstaller.core.CORE_GatewayInfraBuild %InstallationShipment% %ApplicationType% "%ConfigPath%" "%JAVA_HOME%" "%APPSERVER_HOME%" "%ORACLE_HOME%" "%WEBLOGIC_HOME%" %LOAD_ENV_VARIABLES% 
	
	ECHO Deleting the Flag file COREGatewayEarRunChk.flg.
	DEL COREGatewayEarRunChk.flg
	) ELSE (
	ECHO Error!! CLExecDBCompile.jar does not exist.It is required proceed further
	ECHO .
	PAUSE
	GOTO :EXITINSTALLER )
)
