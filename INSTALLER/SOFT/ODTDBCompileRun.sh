...
(
	if [ -f "ODTExecDBCompile.jar" ]
	then
	##echo $InstallationShipment
	export CLASSPATH=./ODTExecDBCompile.jar:$CLASSPATH
	echo "Running in Silent Mode"
	java com.ofss.genericsoftinstaller.tools.ODT_ExecDBCompile "$InstallationShipment" "$ApplicationType" "$ConfigPath" "$JAVA_HOME" "$APPSERVER_HOME" "$ORACLE_HOME" "$WEBLOGIC_HOME" "$LOAD_ENV_VARIABLES"
	EXITINSTALLER
	else
