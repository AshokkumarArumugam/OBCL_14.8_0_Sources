CREATE OR REPLACE PACKAGE BODY aapks_extract_ol_contract_txns AS
  /*-----------------------------------------------------------------------------------------------------
  **
  ** File Name  : aapks_extract_ol_contract_txns.sql
  **
  ** Module     : OL
  **
  ...
  */
  ...
  BEGIN
    IF debug.pkg_debug_on <> 2 THEN
      debug.pr_debug('OL', 'AAPKS_EXTRACT_OL_CONTRACT_TXNS->' || p_msg);
    END IF;
  END dbg;
  ...
  *************************************************************************************************/
    ...
    CURSOR cr_stg_app_master(p_extraction_date IN DATE, prev_extraction_date IN DATE, p_branch IN sttms_core_branch.branch_code%type) IS
      SELECT a.contract_ref_no,
       ...
       WHERE branch = p_branch
         AND module_code = 'OL' --Module code restrictions
         AND EXISTS (SELECT 1
  ...
  BEGIN
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE AATB_STG_OL_LOAN_CONTRACT_TXNS';
    ...
    BEGIN
      SELECT ord_of_ext
       ...
       WHERE table_name = 'AATB_STG_OL_LOAN_CONTRACT_TXNS'
    ...
    BEGIN
      SELECT param_value
        ...
        FROM oltb_eis_cs_param
       WHERE param_name = 'OFSAA_PREV_EXT_OL_LOAN_CONTRACT_TXNS';
    EXCEPTION
      WHEN OTHERS THEN
            ...
            FOR lr_eve_ent IN cr_stg_acc_eve_entries(lr_eve_dia.contract_ref_no,
              ...
              FOR lr_acvw IN cr_stg_acvw_all_ac(lr_eve_dia.contract_ref_no,
                ...
                BEGIN
                  ...
                  END;
                  BEGIN
                    INSERT INTO AATB_STG_OL_LOAN_CONTRACT_TXNS
                      (v_txn_teller_id, --1
                       d_execution_time, --2
...
END aapks_extract_ol_contract_txns;
/