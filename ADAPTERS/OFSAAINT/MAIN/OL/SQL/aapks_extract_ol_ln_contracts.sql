create or replace PACKAGE BODY aapks_extract_ol_ln_contracts AS
  /*-----------------------------------------------------------------------------------------------------
  **
  ** File Name  : aapks_extract_ol_ln_contracts.sql
  **
  ** Module     : OL
  **
  ...
  ** India
  ...
  Change Description : Added to extract 10 additional fields to OFSAA
  Search String	     : OBCL_OFSAA_Changes 
  Modified on        : 25-Feb-2021  
  
    
  Changed By         : Revathi Dharmalingam
  Change Description : Added to code to populate 1 additional fields F_CAPITAL_MARKET_EXPOSURE in AAZB_STG_OL_LOAN_CONTRACTS table 
                      for OFSAA integration
  Search String	     : OBCL_14.5_Support_35317044_Changes
  Modified on        : 21-Apr-2023 
  
  Changed By         : Akhila Samson
  Change Description : Added to condition to fetch the payment schedules from oltbs_contract_schedules.
  Search String	     : Bug#35794581_RETRO_BUG  
  ...
  */
  ...
  BEGIN
    IF debug.pkg_debug_on <> 2 THEN
      debug.pr_debug('OL', 'AAPKS_EXTRACT_OL_LN_CONTRACTS->' || p_msg);
    END IF;
  END dbg;
  ...
  FUNCTION Fn_Stg_Load_Loan_Contracts(p_Extraction_Date IN DATE,
    ...
    CURSOR cr_stg_contract_master(p_extraction_date IN DATE, prev_extraction_date IN DATE, p_branch IN sttms_core_branch.branch_code%type) IS
      SELECT A.contract_ref_no,
             ...
             A.product_type,
             B.main_comp,
			 B.status_since --OBCL_OFSAA_Changes added
       ...
       WHERE A.branch = p_branch
         ...
         --AND A.latest_event_seq_no = B.event_seq_no
	...
	l_Capital_Market_Exposure  Aatb_Stg_Ol_Loan_Contracts.F_Capital_Market_Exposure%TYPE; --OBCL_14.5_Support_35317044_Changes
  BEGIN
	...
	--OBCL_14.5_Support_35317044_Changes Starts
   -- EXECUTE IMMEDIATE 'TRUNCATE TABLE AATB_STG_OL_LOAN_CONTRACTS';
     EXECUTE IMMEDIATE 'TRUNCATE TABLE AAZB_STG_OL_LOAN_CONTRACTS';
	 --OBCL_14.5_Support_35317044_Changes Ends
    ...
    BEGIN
      SELECT ord_of_ext
       ...
       WHERE table_name = 'AATB_STG_OL_LOAN_CONTRACTS'
    ...
    BEGIN
      SELECT param_value
        ...
        FROM oltb_eis_cs_param
       WHERE param_name = 'OFSAA_PREV_EXT_OL_LOAN_CONTRACTS';
    EXCEPTION
      WHEN OTHERS THEN
					...
					SELECT event_date
					 ...
					 WHERE event_code ='MRGI'
					   and contract_Ref_no = ( select c.contract_Ref_no 
												 from OLTB_CONTRACT_MERGE_DETAIL c
												where c.child_Ref_no =lr_rec.contract_ref_no --'000AB10170910004'
      ...
      --l_total_margin
		...
		*/	
        ...
        BEGIN
		  ...
		  SELECT base_rate_floor
            INTO l_rate_floor_rate
            FROM oltms_contract_flrclg_rate a
           WHERE contract_ref_no = lr_rec.contract_ref_no
			 AND component IN (SELECT L.component
						  FROM lftm_product_iccf L
						 WHERE L.product = lr_rec.product_code
			 ...
			 AND eff_date=(SELECT MAX(b.eff_date)
                                  FROM oltms_contract_flrclg_rate b
                                 WHERE b.contract_ref_no = lr_rec.contract_ref_no
				                   AND b.component = a.component);
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
        ...
        BEGIN
		  ...
		  SELECT frequency
           ...
           WHERE contract_ref_no = lr_rec.contract_ref_no
		     AND Schedule_type = 'P' --Bug#35794581_RETRO_BUG
		     AND component IN (SELECT L.component
                                 FROM lftm_product_iccf L
                                WHERE L.product = lr_rec.product_code
                                  AND L.shown_in_contract_main_screen = 'Y')
        ...
        EXCEPTION
            WHEN OTHERS THEN -- WHEN NO_DATA_FOUND THEN --Bug#35794581_RETRO_BUG
            l_schedule_frequency := NULL;
			Dbg('Query the record from oltbs_contract_schedules ::' ||SQLERRM); --Bug#35794581_RETRO_BUG
        END;	
		--OBCL_OFSAA_Changes Ends 
		--OBCL_14.5_Support_35317044_Changes Starts
          l_run_point := '137';   
			BEGIN
			SELECT  Field_Val_1
			INTO  l_Capital_Market_Exposure
			FROM  Oltm_Contract_Userdef_Fields
			WHERE   contract_ref_no = lr_rec.contract_ref_no    
			  AND  version_no       =  ( SELECT MAX(version_no) 
										   FROM Oltm_Contract_Userdef_Fields b
										   WHERE b.contract_ref_no = lr_rec.contract_ref_no    
										);
			EXCEPTION
			WHEN OTHERS  THEN
			  l_capital_market_exposure := NULL;
			END;		
		--OBCL_14.5_Support_35317044_Changes Ends
		
        BEGIN
          INSERT INTO AATB_STG_OL_LOAN_CONTRACTS
            (fic_mis_date,
             v_account_number, --1
             v_branch_code, --2
             v_ccy_code, --3
			 ...
			 --OBCL_OFSAA_Changes Ends	
			 F_Capital_Market_Exposure	--312	 --OBCL_14.5_Support_35317044_Changes			 
			 )
          VALUES
            (p_extraction_date,
			 ...
			 --OBCL_OFSAA_Changes Ends
			 l_Capital_Market_Exposure --312 --OBCL_14.5_Support_35317044_Changes
			 );
        EXCEPTION
...
END aapks_extract_ol_ln_contracts;
/
